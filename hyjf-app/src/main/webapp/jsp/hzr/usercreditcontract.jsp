...
<html>
	...
	<body class="bg_grey">
		<div class="specialFont response">
			...
			<div class="contract_item bg_white">
				<div>
					...
					<div class="contract_item_content">
						<p><span>合同编号</span>${creditResult.data.creditTender.assignNid}</p>
					</div>
				</div>
			</div>
			<!--item end-->
			...
			<div class="contract_item bg_white">
				<div>
					<p class="contract_item_title">原债权人（转让方，以下简称甲方）：${creditResult.data.usersInfoCredit.truename}</p>
					<div class="contract_item_content">
						<p><span>身份证(护照)号码</span>${creditResult.data.usersInfoCredit.idcard}</p>
						<p><span>汇盈金服用户名</span>${creditResult.data.usersCredit.username}</p>
					</div>
				</div>
			</div>
			<!--item end-->
			...
			<div class="contract_item bg_white">
				<div>
					<p class="contract_item_title">新债权人（受让方，以下简称乙方）： ${creditResult.data.usersInfo.truename} </p>
					<div class="contract_item_content">
						<p><span>身份证(护照)号码：</span>${creditResult.data.usersInfo.idcard}</p>
						<p><span>汇盈金服用户名：</span>${creditResult.data.users.username}</p>
					</div>
				</div>
			</div>
			<!--item end-->
							...
							<div class="margin0">
								<div class="item">
									<table border="0" cellspacing="0" cellpadding="0" class="borderNone hzr-rule-table">
										...
										<tr>
                                            <td>预计还款日</td>
											<td>自 ${creditResult.data.creditTender.addTime} 月起，每月  ${creditResult.data.creditTender.addip} 日（24:00前）</td>
                                        </tr>
										<tr>
											<td>借款期限</td>
									...
									<table border="0" cellspacing="0" cellpadding="0" class="borderNone hzr-rule-table">
										...
										<tr>
											<td>转让手续费</td>
											<td>￥${creditResult.data.creditTender.creditFee}</td>
										</tr>
										<tr>
											<td>转让剩余期限</td>
