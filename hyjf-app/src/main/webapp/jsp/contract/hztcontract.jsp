...
<html>
	...
	<div class="specialFont response">
			...
			<div class="contract_item bg_white">
				<div>
					<div class="contract_item_content">
						<p><span>编号：${borrow.borrowNid}</span></p> 
						<p><span>签订日期：${borrow.signDate}</span></p>
						<p>甲方（出借方/债权受让方）：</p>
					</div>
					<div class="fuwujian-contact-overflow">
						<table border="1" cellspacing="0" cellpadding="0" class="borderNone hzr-rule-table">
							<tr><td>用户名</td><td> ${borrow.tenderRealName}${borrow.tenderIdCard}</td></tr>
							<tr><td>投资金额</td><td><c:out value="${borrow.invest}"></c:out>元</td></tr>
							<tr><td>投资期限</td><td>${borrow.borrowPeriod }</td></tr>
							<tr><td>历史年回报率</td><td>${borrow.borrowApr }%</td></tr>
							<tr><td>投资开始日</td><td>${borrow.investStartDate }</td></tr>
							<tr><td>投资到期日</td><td>${borrow.investEndDate }</td></tr>
							<tr><td>借款方还款方式</td><td>${borrow.repayStyle }</td></tr>
							<tr><td>总还款本金及收益</td><td><c:out value="${borrow.account}"></c:out>元</td></tr>
						</table>
					</div>
				</div>
			</div>
			...
			<div class="contract_item bg_white">
				<div class="contract_item_content">
					<p>乙方（借款方/债权转让方）：<c:out value="${borrow.borrowUserName}"></c:out></p>
					<p><span class="fl">说明：</span><span>为保护其隐私权，因此未显示全部乙方的信息，若出现乙方违反本协议的情况，丙方有权向甲方披露其全部信息，包括但不限于乙方的姓名、身份证号码及住址等信息。</span></p>
					<p><span class="fl">投资项目：</span><span>详见平台网站www.hyjf.com的项目内容。</span></p>
					<p><span class="fl">丙方（居间人）：</span><span>惠众商务顾问（北京）有限公司</span></p>
					<p><span class="fl">经营地址：</span><span><b>上海市长宁区延安西路2299号世贸大厦24楼</b></span></p>
					<p>鉴于：</p>
					<p>1、丙方是一家在北京合法成立并有效存续的有限责任公司，拥有www.hyjf.com网站（以下简称“本网站”）的经营权，提供信用咨询，为交易提供信息服务；</p>
					<p>2、甲、乙双方已在本网站注册，并承诺其提供给丙方的信息是完全真实的；</p>
					<p>3、甲方为出借方，承诺对本协议涉及的投资款具有完全的支配能力，是其自有闲置资金，且为合法所得；</p>
					<p>4、乙方为借款方，拟通过本网站平台借款/转让债权，进行借款。</p>
