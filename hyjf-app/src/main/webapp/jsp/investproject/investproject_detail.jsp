	...
	<body class="bg_grey">
		<div class="specialFont  cal-main">
                ...
                <section class="new-form-item  bg_white my-invest-item">
					...
					<c:if test="${investType eq 'null' or empty investType}">
					<div>
                		<!-- 代金券 优惠券 -->
                		<c:if test="${repayDetail.couponType eq '1' or repayDetail.couponType eq '2'}">
                			<span>预期用券收益（元）</span>
                			<span class="hyjf-color format-num">${repayDetail.interest}</span>
                		</c:if>
                		<c:if test="${repayDetail.couponType eq '3' }">
                			<span>预期用券收益（元）</span>
