 ...
 */
@Service("appTwoelevenActivityServiceImpl")
...
//        }
    ...
    public JSONObject seckillAward(String couponId,String activity, Integer userId){
        ...
        logger.info("【双十一秒杀活动】APP用户秒杀 userId="+userId+",couponId="+couponId);
        ...
        try{
            ...
            if (RedisUtils.lockWithTimeout(rediskey,token,6000L)) {
                ...
                int awardTime = twoelevenCustomizeMapper.getUserAwardTime(userId);
                if(awardTime>=2){
                    logger.info("【双十一秒杀活动】APP用户剩余秒杀次数不足 userId="+userId+",awardTime="+awardTime);
                    //1=加息先行 ,2=红包到达战场
                ...
                logger.info("【双十一秒杀活动】APP秒杀剩余张数(记得减一) rest="+mapCoupon.get("rest")+",couponId="+couponId +",userId="+userId);
