 ...
 */
@Service
public class AppTenderCreditServiceImpl extends BaseServiceImpl implements AppTenderCreditService {

	Logger _log = LoggerFactory.getLogger(AppTenderCreditServiceImpl.class);
	...
	@Override
	@Cached(name="appHomeHZRCountCache-", expire = CustomConstants.HOME_CACHE_LIVE_TIME, cacheType = CacheType.BOTH)
	@CacheRefresh(refresh = 10, stopRefreshAfterLastAccess = 600, timeUnit = TimeUnit.SECONDS)
	...
	@Override
	@Cached(name="appHomeHZRCache-", expire = CustomConstants.HOME_CACHE_LIVE_TIME, cacheType = CacheType.BOTH)
	@CacheRefresh(refresh = 10, stopRefreshAfterLastAccess = 600, timeUnit = TimeUnit.SECONDS)
	 ...
	 * @return
	 * @see com.hyjf.app.user.credit.AppTenderCreditService#selectUserCreditContract(com.hyjf.app.user.credit.AppTenderCreditAssignedBean)
	 */
	@Override
	...
	public Integer insertTenderToCredit(int userId, int nowTime, AppTenderBorrowCreditCustomize appTenderBorrowCreditCustomize, String platform) throws Exception {
		...
		if (borrowRecoverList != null && borrowRecoverList.size() > 0) {
			...
			if (tenderToCreditList != null && tenderToCreditList.size() > 0) {
				...
				borrowCredit.setBidApr(new BigDecimal(tenderToCreditList.get(0).getBorrowApr()));// 原标历史年回报率
				...
				if(!CollectionUtils.isEmpty(debtConfig)){
					borrowCredit.setCreditFee(creditCreateMap.get("assignPay").multiply(debtConfig.get(0).getAttornRate().divide(new BigDecimal(100))).setScale(2, BigDecimal.ROUND_DOWN));// 服务费
				}else{
...
//					borrowCredit.setCreditStatus(2);
				...
				if (creditTender.getCreditFee().compareTo(BigDecimal.ZERO) > 0) {
					...
					if (webListCount == 0) {
						...
						params.put("money", creditCapitalAssigned);
				...
				if (borrowRecoverList != null && borrowRecoverList.size() == 1) {
					...
					// add by liuyang 发送生成法大大PDF MQ处理 20180226 start
					this.sendPdfMQ(userId,borrowNid,assignOrderId,creditNid, creditTender.getCreditTenderNid());
					// add by liuyang 发送生成法大大PDF MQ处理 20180226 end
...
//		}
		...
		_log.info(borrowRecover.getBorrowNid()+" APP承接结束债权  借款人: "+borrowUserId+"-"+borrowUserAccount.getAccount()+" 出借人: "+tenderUserId+"-"+tenderAccountId+" 授权码: "+borrowRecover.getAuthCode()+" 原始订单号: "+borrowRecover.getNid());
...
//		record.setTenderUsername(tenderUsername);
		...
		if (borrowCreditList != null && borrowCreditList.size() > 0) {
			...
			if (borrowRecoverList != null && borrowRecoverList.size() > 0) {
				...
				if (borrowList != null && borrowList.size() > 0) {
					...
					_log.info("getAssignPay:"+tenderToCreditAssign.getAssignPay().replaceAll(",", ""));
					_log.info("getBankBalance:"+accountList.get(0).getBankBalance());
	 ...
	 * @param orderId
	 * @return
	 * @see com.hyjf.app.user.credit.AppTenderCreditService#getIncreaseInterestInvestByOrdId(java.lang.String)
	 */
    @Override
    ...
    public IncreaseInterestInvest getIncreaseInterestInvestByTenderNid(String orderId) {
        IncreaseInterestInvestExample example = new IncreaseInterestInvestExample();
        IncreaseInterestInvestExample.Criteria cra = example.createCriteria();
        cra.andTenderNidEqualTo(orderId);
        List<IncreaseInterestInvest> list = increaseInterestInvestMapper.selectByExample(example);
        if (list != null && list.size() > 0) {
            return list.get(0);
        }
