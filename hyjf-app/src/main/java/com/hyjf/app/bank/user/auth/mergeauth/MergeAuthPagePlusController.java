package com.hyjf.app.bank.user.auth.mergeauth;
...
public class MergeAuthPagePlusController extends BaseController {
	...
	public ModelAndView page(HttpServletRequest request,
		...
		String retUrl = PropUtils.getSystem(CustomConstants.HYJF_WEB_URL) + request.getContextPath() 
				...
				+ MergeAuthPagePlusDefine.RETURL_SYN_ACTION + ".do?authType="+AuthBean.AUTH_TYPE_MERGE_AUTH+ "&sign=" + sign+"&platform="+platform;
		// 异步调用路
	...
	public ModelAndView mergeAuthReturn(HttpServletRequest request,
        ...
        logger.info("合并授权同步回调调用查询接口查询状态结束  结果为:" + (retBean == null ? "空" : retBean.getRetCode())+"授权状态为："+retBean.getPaymentAuth());
	...
	private ModelAndView getErrorMV(String msg, BaseMapBean baseMapBean,String sign,String platform) {
		...
		baseMapBean.set("authType", AuthBean.AUTH_TYPE_MERGE_AUTH);
