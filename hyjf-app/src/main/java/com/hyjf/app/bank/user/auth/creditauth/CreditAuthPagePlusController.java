package com.hyjf.app.bank.user.auth.creditauth;
...
public class CreditAuthPagePlusController extends BaseController {
	...
	public ModelAndView page(HttpServletRequest request,
		...
		String retUrl = PropUtils.getSystem(CustomConstants.HYJF_WEB_URL) + request.getContextPath() 
				...
				+ CreditAuthPagePlusDefine.RETURL_SYN_ACTION + ".do?authType="+AuthBean.AUTH_TYPE_AUTO_CREDIT+"&sign="+sign+"&platform="+platform;
		// 异步调用路
		...
		try {
			...
			modelAndView = new ModelAndView(
					CreditAuthPagePlusDefine.USER_AUTH_ERROR_PATH);
	...
	public ModelAndView creditAuthReturn(HttpServletRequest request,
		...
		LogUtil.startLog(THIS_CLASS, CreditAuthPagePlusDefine.RETURL_SYN_ACTION, "[自动债转授权同步回调开始]");
        BaseMapBean baseMapBean=new BaseMapBean();
        bean.convert();
        String sign = request.getParameter("sign");
		String platform = request.getParameter("platform");
        ...
        // 出借人签约状态查询
        logger.info("自动债转授权同步回调调用查询接口查询状态");
        BankCallBean retBean=authService.getTermsAuthQuery(Integer.parseInt(bean.getLogUserId()),BankCallConstant.CHANNEL_APP);
        logger.info("自动债转授权同步回调调用查询接口查询状态结束  结果为:" + (retBean == null ? "空" : retBean.getRetCode())+"授权状态为："+retBean.getPaymentAuth());
        // 返回失败
     	if (retBean != null&& 
            ...
            if("1".equals(retBean.getPaymentAuth())){
                ...
                return getSuccessMV("用户自动债转授权成功！",baseMapBean,Integer.parseInt(bean.getLogUserId())); 
            }else{
	...
	public BankCallResult creditAuthBgreturn(HttpServletRequest request,
		...
		LogUtil.startLog(CreditAuthPagePlusDefine.THIS_CLASS,
				CreditAuthPagePlusDefine.RETURL_ASY_ACTION, "[自动债转授权异步回调开始]");
		bean.convert();
	...
	private ModelAndView getErrorMV(String msg, BaseMapBean baseMapBean,String sign,String platform) {
		...
		baseMapBean.set("authType", AuthBean.AUTH_TYPE_AUTO_CREDIT);
        ...
        LogUtil.endLog(THIS_CLASS, CreditAuthPagePlusDefine.CREDIT_AUTH_ACTION);
        return modelAndView;
    }
