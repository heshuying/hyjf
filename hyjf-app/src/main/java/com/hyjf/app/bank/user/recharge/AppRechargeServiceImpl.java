 ...
 */
@Service
public class AppRechargeServiceImpl extends BaseServiceImpl implements AppRechargeService {
	...
	public JSONObject handleRechargeInfo(BankCallBean bean, Map<String, String> params) {
		...
		// 失败信息
		String errorMsg = bean.getRetMsg() == null ? "" : bean.getRetMsg();
		// ip
		...
		if (BankCallStatusConstant.RESPCODE_SUCCESS.equals(retCode)) {
			...
			if (accountRecharge != null) {
				...
				if (accountRecharge.getStatus() == RECHARGE_STATUS_SUCCESS) {
					...
					try {
						...
						accountRecharge.setStatus(RECHARGE_STATUS_SUCCESS);// 充值状态:
						...
						accountList.setTradeStatus(1);// add by cwyang 对应数据字段充值成功
						// 插入交易明细
			...
			// 根据返回码获取错误信息
			errorMsg = this.getBankRetMsg(retCode);
			// 更新订单信息
			AccountRechargeExample example = new AccountRechargeExample();
			example.createCriteria().andNidEqualTo(orderId);
