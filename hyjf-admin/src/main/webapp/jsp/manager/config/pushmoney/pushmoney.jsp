...
<shiro:hasPermission name="pushmoney:VIEW">
	<tiles:insertTemplate template="/jsp/layout/listLayout.jsp" flush="true">
		<%-- 画面的标题 --%>
		<tiles:putAttribute name="pageTitle" value="提成设置" />
		
		<%-- 画面主面板的标题块 --%>
		<tiles:putAttribute name="pageFunCaption" type="string">
			<h1 class="mainTitle">提成设置</h1>
			<span class="mainDescription">本功能可以增加修改删除提成。</span>
		</tiles:putAttribute>
					...
					<div class="col-md-12">
						<div class="search-classic">
							...
							<div class="well">
								...
								<shiro:hasPermission name="pushmoney:ADD"> 
									<a class="btn btn-o btn-primary btn-sm hidden-xs fn-Add"
											data-toggle="tooltip" data-placement="bottom" data-original-title="添加新提成">添加 <i class="fa fa-plus"></i></a>
 								</shiro:hasPermission> 
							...
							<table id="equiList" class="table table-striped table-bordered table-hover">
								...
								<thead>
									<tr>
										...
										<th class="center">是否发放提成</th>
										<th class="center">天标提成比例</th>
										<th class="center">月标提成比例</th>
										...
										<c:otherwise>
											<c:forEach items="${pushmoneyForm.recordList }" var="record" begin="0" step="1" varStatus="status">
												<tr>
													...
													<td><c:out value="${record.projectType eq 1? '汇直投' : '汇计划'}"></c:out></td>
													<td><c:out value="${record.type }"></c:out></td>
													<td><c:out value="${record.rewardSend eq 0? '不发放' : '发放'}"></c:out></td>
													<td align="right"><c:out value="${record.dayTender }"></c:out></td>
													<td align="right"><c:out value="${record.monthTender }"></c:out></td>
													<td><c:out value="${record.remark }"></c:out></td>
													<td class="center">
														<div class="visible-md visible-lg hidden-sm hidden-xs">
															<shiro:hasPermission name="pushmoney:MODIFY">
															...
															<%-- <shiro:hasPermission name="pushmoney:DELETE">
							...
							<%-- 分页栏 --%>
							<hyjf:paginator id="equiPaginator" hidden="paginator-page" action="init" paginator="${pushmoneyForm.paginator}"></hyjf:paginator>
							<br/><br/>
						</div>
					</div>
				</div>
		...
		<tiles:putAttribute name="pageJavaScript" type="string">
			<script type='text/javascript' src="${webRoot}/jsp/manager/config/pushmoney/pushmoney.js"></script>
		</tiles:putAttribute>
