...
<shiro:hasPermission name="finhxfmancharge:VIEW">
	<tiles:insertTemplate template="/jsp/layout/listLayout.jsp" flush="true">
		<%-- 画面的标题 --%>
		<tiles:putAttribute name="pageTitle" value="汇消费融资管理费" />
		
		<%-- 画面主面板的标题块 --%>
		<tiles:putAttribute name="pageFunCaption" type="string">
			<h1 class="mainTitle">汇消费融资管理费</h1>
		...
		<div class="container-fluid container-fullw bg-white">
			...
			<ul class="nav nav-tabs" id="myTab">
				...
				<shiro:hasPermission name="finsercharge:VIEW">
		      	...
		      	<shiro:hasPermission name="finhxfmancharge:VIEW">
		      		<li class="active"><a href="${webRoot}/manager/config/finhxfmancharge/init">汇消费融资管理费</a></li>
		      	</shiro:hasPermission>
			...
			<div class="tab-content">
				<div class="tab-pane fade in active">
					<%-- 功能栏 --%>
					<div class="well">
						<c:set var="jspPrevDsb" value="${finhxfmanchargeForm.paginator.firstPage ? ' disabled' : ''}"></c:set>
						<c:set var="jspNextDsb" value="${finhxfmanchargeForm.paginator.lastPage ? ' disabled' : ''}"></c:set>
						<a class="btn btn-o btn-primary btn-sm hidden-xs fn-Prev${jspPrevDsb}"
								data-toggle="tooltip" data-placement="bottom" data-original-title="上一页"><i class="fa fa-chevron-left"></i></a>
						<a class="btn btn-o btn-primary btn-sm hidden-xs margin-right-15 fn-Next${jspNextDsb}"
								data-toggle="tooltip" data-placement="bottom" data-original-title="下一页"><i class="fa fa-chevron-right"></i></a>
						<shiro:hasPermission name="finhxfmancharge:ADD">
					...
					<table id="equiList" class="table table-striped table-bordered table-hover">
						...
						<thead>
							<tr>
								...
								<th class="center">管理费率（上限）</th>
								<th class="center">管理费率（下限）</th>
						...
						<tbody id="roleTbody">
							<c:choose>
								<c:when test="${empty finhxfmanchargeForm.recordList}">
									<tr>
										<td colspan="8">暂时没有数据记录</td>
									</tr>
								</c:when>
								<c:otherwise>
									<c:forEach items="${finhxfmanchargeForm.recordList }" var="record" begin="0" step="1" varStatus="status">
										<tr>
											<td class="center"><c:out value="${status.index+((finhxfmanchargeForm.paginatorPage - 1) * finhxfmanchargeForm.paginator.limit) + 1 }"></c:out></td>
											<td><c:out value="${record.chargeTimeType }"></c:out></td>
											<td class="center"><c:out value="${record.manChargeType }"></c:out></td>
											<td align="right"><c:out value="${record.chargeTime }"></c:out><c:out value="${record.chargeTimeType eq 'endday' ? 'N天' : '月' }"></c:out></td>
											<td align="right"><c:out value="${record.manChargePer }"></c:out></td>
											<td align="right"><c:out value="${record.manChargePerEnd }"></c:out></td>
											<td class="center"><c:out
													value="${record.status == '0' ? '启用' : '禁用'  }"></c:out></td>
											<td><c:out value="${record.remark }"></c:out></td>
											<td class="center">
												<div class="visible-md visible-lg hidden-sm hidden-xs">
													<shiro:hasPermission name="finhxfmancharge:MODIFY">
													...
													<shiro:hasPermission name="finhxfmancharge:DELETE">
					...
					<%-- 分页栏 --%>
					<hyjf:paginator id="equiPaginator" hidden="paginator-page" action="init" paginator="${finhxfmanchargeForm.paginator}"></hyjf:paginator>
					<br /> <br />
				</div>
			</div>
			</div>
		...
		<tiles:putAttribute name="searchPanels" type="string">
			...
			<input type="hidden" name="paginatorPage" id="paginator-page" value="${finhxfmanchargeForm.paginatorPage}" />
		</tiles:putAttribute>
		...
		<tiles:putAttribute name="pageJavaScript" type="string">
			<script type='text/javascript' src="${webRoot}/jsp/manager/config/borrow/finhxfmancharge/finhxfmancharge.js"></script>
		</tiles:putAttribute>
