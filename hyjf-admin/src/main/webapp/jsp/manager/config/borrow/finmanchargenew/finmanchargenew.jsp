					...
					<table id="equiList" class="table table-striped table-bordered table-hover">
						...
						<thead>
							<tr>
								...
								<th class="center">服务费收取方式</th>
								<%-- upd by LSY START --%>
								<%-- <th class="center">服务费率</th>
								
								<th class="center">管理费率</th> --%>
								<th class="center">放款服务费率</th>
								<th class="center">还款服务费率</th>
								<%-- upd by LSY END --%>
								<th class="center">收益差率</th>
								<th class="center">逾期利率</th>
								<th class="center">逾期免息天数</th>
								
								<th class="center">服务费总额</th>
								<th class="center">是否自动还款</th>
								...
								<c:otherwise>
									<c:forEach items="${finmanchargeForm.recordList }" var="record" begin="0" step="1" varStatus="status">
										<tr>
											...
											<td class="center"><c:out value="${record.chargeMode== '1' ? '按比例' : '按金额' }"></c:out></td>
											...
											<td class="center"><c:out value="${record.lateInterest }"></c:out></td>
											<td class="center"><c:out value="${record.lateFreeDays }"></c:out></td>
											<%-- <td class="center"><c:out value="${record.serviceCharge }"></c:out></td> --%>
											<td align="center"><fmt:formatNumber value="${record.serviceFeeTotal}" type="number" pattern="#,##0.00#" /> </td>											
											<td class="center"><c:out value="${record.autoRepay== '1' ? '是' : '否' }"></c:out></td>
