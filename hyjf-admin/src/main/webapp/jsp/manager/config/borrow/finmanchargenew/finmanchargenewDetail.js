...
$.extend(Page, {
	...
	initEvents : function() {
		...
		$(".chargeOn").change(Events.chargeModeOnchangeAct);
		/*$("#chargeMode").change(Events.chargeModeOnchangeAct);*/
	},
	// 画面布局
	doLayout : function() {
		// 初始化下拉框
		...
		//
		$('#chargeOn').on("change", function(evnet, d) {
			$('#repayTimeConfigArea').hide();
		}),
		$('#chargeOff').on("change", function(evnet, d) {
