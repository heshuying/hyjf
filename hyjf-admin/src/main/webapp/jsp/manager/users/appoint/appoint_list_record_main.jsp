	...
	<tiles:insertTemplate template="/jsp/layout/listLayout.jsp"
		...
		<div class="container-fluid container-fullw bg-white">
			...
			<div class="tab-content">
				<div class="tab-pane fade in active">
								...
								<c:choose>
									...
									<c:forEach items="${recordList}" var="record"
										...
										<td>${vs.count }</td>
												...
												<td>
												${record.recodTruncateTime}
												</td>
												<td>
													<c:if test="${not empty record.addTime}">
													<a href="${webRoot}/manager/users/appoint/appointlistrecord.do?username=${record.username}">明细</a>
													</c:if>
												</td>
										</tr>
										</c:forEach>
					...
					<%-- 分页栏 --%>
					<hyjf:paginator id="equiPaginator" hidden="paginator-page" action="appointlistrecordmainSearchAction" paginator="${form.paginator}"></hyjf:paginator>
					<br /> <br />
				</div>
			</div>
			</div>
		...
		<tiles:putAttribute name="searchPanels" type="string">
			...
			<input type="text" name="username" id="username" class="form-control input-sm underline"  maxlength="20" value="${form.username}" />
				...
				<div class="form-group">
				<label>电话</label> 
			<input type="text" name="mobile" id="mobile" class="form-control input-sm underline"  maxlength="20" value="${form.mobile}" />
			</div>
			<div class="form-group">
				<label>操作时间</label>
		...
		<tiles:putAttribute name="pageJavaScript" type="string">
			<script type='text/javascript' src="${webRoot}/jsp/manager/users/appoint/appoint_list_record_main.js"></script>
		</tiles:putAttribute>
