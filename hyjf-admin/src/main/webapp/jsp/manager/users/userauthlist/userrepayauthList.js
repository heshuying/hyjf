...
Action = {
	...
	// 页面查询
	searchAction : "userrepayauthlist",
	// 导出的Action
	exportAction : "exportrepayauth",
	//解约	
	repaycancel : "repaycancel",
	queryAuth : "userrepayauthquery"
...
Events = {
	...
	repaycancelAct:function(event){
        if(event) {
            Page.primaryKey.val($(this).data("userid"));
        }
        Page.confirm("还款授权解约", "确认执行此操作吗？",  function(isConfirm) {
            if (isConfirm) {
                var param = {};
                param.userId = Page.primaryKey.val();
                Page.coverLayer("正在操作数据，请稍候...");
                $.ajax({
                    url : Action.repaycancel,
                    type : "GET",
                    async : true,
                    data : param,
                    ...
                    error : function(err) {
                        Page.coverLayer();
                        Page.primaryKey.val("");
                        Page.notice("还款授权解约发生错误,请重新操作!", "","error");
                    }
                });
            }
        });
...
/*	repaycancelAct:function(event){
        ...
        var isConfirm = confirm('"还款授权解约", "确认执行此操作吗？');
            if (isConfirm) {
                var param = {};
                param.userId = Page.primaryKey.val();
                Page.coverLayer("正在操作数据，请稍候...");
	 ...
	 queryAuthAct : function(event) {
	        ...
	        Page.confirm("还款授权查询", "确认执行此操作吗？",  function(isConfirm) {
	            if (isConfirm) {
	                var param = {};
	                param.userId = Page.primaryKey.val();
	                Page.coverLayer("正在操作数据，请稍候...");
	                $.ajax({
	                    ...
	                    error : function(err) {
	                        Page.coverLayer();
	                        Page.primaryKey.val("");
	                        Page.notice("还款授权权查询发生错误,请重新操作!", "","error");
	                    }
	                });
	            }
	        });
...
//        });
	...
	initEvents : function() {
		...
		$(".fn-repaycancel").click(Events.repaycancelAct),
