...
<shiro:hasPermission name="userauthlist:VIEW">
	<tiles:insertTemplate template="/jsp/layout/listLayout.jsp" flush="true">
		<%-- 画面的标题 --%>
		<tiles:putAttribute name="pageTitle" value="自动投标授权" />

		<%-- 画面主面板标题块 --%>
		<tiles:putAttribute name="pageFunCaption" type="string">
			<h1 class="mainTitle">自动投标授权</h1>
			<span class="mainDescription">本功能可以查询相应的会员授权信息。</span>
		</tiles:putAttribute>
				...
				<div class="tabbable">
					<ul class="nav nav-tabs" id="myTab"> 
						<shiro:hasPermission name="userauthlist:SEARCH">
				      		<li  class="active"><a href="${webRoot}/manager/userauth/userauthlist">授权状态</a></li>
				      	</shiro:hasPermission>
				      	<shiro:hasPermission name="userauthlist:SEARCH">
										...
										<table id="equiList" class="table table-striped table-bordered table-hover">
											...
											<thead>
												<tr>
													...
													<th class="center">自动投标交易金额</th>
													<th class="center">自动投标到期日</th>
													<th class="center">自动投标授权状态</th>
													<th class="center">自动债转交易金额</th>
													<th class="center">自动债转到期日</th>
													...
													<c:otherwise>
														<c:forEach items="${userauthListForm.recordList }" var="record" begin="0" step="1" varStatus="status">
															<tr>
																...
																<td class="center"><c:out value="${record.invesMaxAmt }"></c:out></td>
																<td class="center"><c:out value="${record.autoInvesEndTime }"></c:out></td>
																<td class="center"><c:out value="${record.autoInvesStatus }"></c:out></td>
																<td class="center"><c:out value="${record.creditMaxAmt }"></c:out></td>
																<td class="center"><c:out value="${record.autoCreditEndTime }"></c:out></td>
																<td class="center"><c:out value="${record.autoCreditStatus }"></c:out></td>
																<td class="center"><c:out value="${record.autoCreateTime }"></c:out></td>

																<td class="center">
																	<a class="fn-queryAuth" data-userid="${record.userId }">查询</a>
																	<%--<a class="fn-invesCancel" data-userid="${record.userId }">自动投标解约</a>
																	<a class="fn-creditCancel" data-userid="${record.userId }">自动债转解约</a>--%>
																</td>
															</tr>
														</c:forEach>					
													</c:otherwise>
		...
		<script type="text/javascript"> var webRoot = "${webRoot}";</script>
			<script type='text/javascript' src="${webRoot}/jsp/manager/users/userauthlist/userauthList.js"></script>
		</tiles:putAttribute>
