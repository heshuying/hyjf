...
Action = {
	...
	// 页面查询
	searchAction : "userpayauthlist",
	// 导出的Action
	exportAction : "exportpayauth",
	//解约	
	paycancel : "paycancel",
	queryAuth : "userpayauthquery"
...
Events = {
                ...
                $.ajax({
                    ...
                    success : function(data) {
                        ...
                        if (data.success == "0") {
                            ...
                            setTimeout(function () {Page.confirm("",data.msg,"success",{showCancelButton: false}, function(){Events.refreshClkAct()})} ,100);
                        } else {
                            // Page.confirm("",data.msg,"error",{showCancelButton: false}, function(){Events.refreshClkAct()});
                            setTimeout(function () {Page.confirm("",data.msg,"error",{showCancelButton: false}, function(){Events.refreshClkAct()})} ,100);
                    ...
                    error : function(err) {
                        Page.coverLayer();
                        Page.primaryKey.val("");
                        Page.notice("缴费授权解约发生错误,请重新操作!", "","error");
                    }
                });
            }
        });
    ...
    queryAuthAct : function(event) {
        ...
        Page.confirm("缴费授权查询", "确认执行此操作吗？",  function(isConfirm) {
            if (isConfirm) {
                var param = {};
                param.userId = Page.primaryKey.val();
                Page.coverLayer("正在操作数据，请稍候...");
                $.ajax({
                    ...
                    success : function(data) {
                        ...
                        if (data.success == "0") {
                            //Page.confirm("",data.msg,"success",{showCancelButton: false}, function(){Events.refreshClkAct()});
                            setTimeout(function () {Page.confirm("",data.msg,"success",{showCancelButton: false}, function(){Events.refreshClkAct()})} ,500);
                        } else {
                            setTimeout(function () {Page.confirm("",data.msg,"error",{showCancelButton: false}, function(){Events.refreshClkAct()})} ,500);
                        }
                    },
                    error : function(err) {
                        Page.coverLayer();
                        Page.primaryKey.val("");
                        Page.notice("缴费授权权查询发生错误,请重新操作!", "","error");
                    }
                });
            }
        });
...
$.extend(Page, {
	...
	doLayout: function() {
        ...
        // 日历选择器
        $('#sign-start-date-time').datepicker({
            autoclose: true,
            todayHighlight: true,
            endDate:new Date()
        });
        $('#sign-end-date-time').datepicker({
            autoclose: true,
            todayHighlight: true,
            endDate:new Date()
        });
...
//        });
        var day30 = 30 * 24 * 60 * 60 * 1000;
        $("#sign-start-date-time").on("changeDate", function(ev) {
            var now = new Date();
            var selectedDate = new Date(ev.date.valueOf());
            var endDate = $('#sign-end-date-time').datepicker("getDate");
            var finalEndDate = +selectedDate + day30 >= +now ? now : new Date(+selectedDate + day30);
            $('#sign-end-date-time').datepicker("setStartDate", selectedDate);
            $('#sign-end-date-time').datepicker("setEndDate", finalEndDate);
            //如果end值范围超过30天，设置end最大结束时间
            if (endDate != null && (+endDate - selectedDate >= day30)) {
                $('#sign-end-date-time').datepicker("setDate", finalEndDate);
            }
        });
        if($("#sign-start-date-time").val() != ''){
            $('#sign-start-date-time').datepicker("setDate", $("#sign-start-date-time").val());
        }
	},
	// 初始化画面事件处理
	initEvents : function() {
		...
		$(".fn-paycancel").click(Events.paycancelAct),
		// 自动授权查询
