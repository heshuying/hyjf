...
<shiro:hasPermission name="userauthlist:VIEW">
	<tiles:insertTemplate template="/jsp/layout/listLayout.jsp" flush="true">
		<%-- 画面的标题 --%>
		<tiles:putAttribute name="pageTitle" value="还款授权" />

		<%-- 画面主面板标题块 --%>
		<tiles:putAttribute name="pageFunCaption" type="string">
			<h1 class="mainTitle">还款授权</h1>
			<span class="mainDescription">本功能可以查询相应的会员还款授权信息。</span>
		</tiles:putAttribute>
				...
				<div class="tabbable">
					<ul class="nav nav-tabs" id="myTab"> 
						<shiro:hasPermission name="userauthlist:SEARCH">
				      	...
				      	<shiro:hasPermission name="userauthlist:SEARCH">
				      		<li  class="active"><a href="${webRoot}/manager/userauth/userrepayauthlist">还款授权</a></li>
				      	</shiro:hasPermission>
				    </ul>
													...
													<c:otherwise>
														<c:forEach items="${obj.recordList }" var="record" begin="0" step="1" varStatus="status">
															<tr>
																...
																<td class="center"><c:out value="${record.repayMaxAmt}"></c:out></td>
																...
																<td class="center">
<%--                                										<a href="${webRoot}/manager/userauth/userrepayauthquery?id=${record.userid}" class="fn-Info">查询</a> --%>
																	<a class="fn-queryAuth" data-userid="${record.userid }">查询</a>
<%--                                										<a href="${webRoot}/manager/userauth/userrepayauthdis?id=${record.userid}" class="fn-Info">解约</a> --%>
																	<%--<a class="fn-repaycancel" data-userid="${record.userid }">解约</a>--%>
																</td>
															</tr>
														</c:forEach>					
													</c:otherwise>
										...
										<%-- 分页栏 --%>
										<hyjf:paginator id="equiPaginator" hidden="paginator-page" action="userrepayauthlist" paginator="${obj.paginator}"></hyjf:paginator>
										<br/><br/>
									</div>
								</div>
							</div>
		...
		<script type="text/javascript"> var webRoot = "${webRoot}";</script>
			<script type='text/javascript' src="${webRoot}/jsp/manager/users/userauthlist/userrepayauthList.js"></script>
		</tiles:putAttribute>
