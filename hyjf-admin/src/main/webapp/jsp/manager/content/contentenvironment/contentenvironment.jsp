...
<shiro:hasPermission name="contentenvironment:VIEW">
	<tiles:insertTemplate template="/jsp/layout/listLayout.jsp"
		flush="true">
		<%-- 画面的标题 --%>
		<tiles:putAttribute name="pageTitle" value="公司环境设置" />

		<%-- 画面主面板的标题块 --%>
		<tiles:putAttribute name="pageFunCaption" type="string">
			<h1 class="mainTitle">公司环境设置</h1>
			<span class="mainDescription">本功能可以增加修改删除公司环境。</span>
		</tiles:putAttribute>
						...
						<div class="tab-pane fade in active">
							...
							<div class="well">
								<c:set var="jspPrevDsb"
									value="${environmentForm.paginator.firstPage ? ' disabled' : ''}"></c:set>
								<c:set var="jspNextDsb"
									value="${environmentForm.paginator.lastPage ? ' disabled' : ''}"></c:set>
								<a
								...
								<shiro:hasPermission name="contentenvironment:ADD">
							...
							<table id="equiList"
								...
								<tbody id="roleTbody">
									<c:choose>
										<c:when test="${empty environmentForm.recordList}">
											<tr>
												<td colspan="8">暂时没有数据记录</td>
											</tr>
										</c:when>
										<c:otherwise>
											<c:forEach items="${environmentForm.recordList }"
												var="record" begin="0" step="1" varStatus="status">
												<tr>
													<td class="center"><c:out value="${status.index+((environmentForm.paginatorPage - 1) * environmentForm.paginator.limit) + 1 }"></c:out></td>
													<td><c:out value="${record.name }"></c:out></td>
													<td class="center"><a
														href="${fileDomainUrl}${record.imgurl}" target="_blank"
														class="thumbnails-wrap" data-toggle="popover"
														data-placement="right" data-trigger="hover"
														data-html="true" data-title="图片预览"
														data-content="<img src='${fileDomainUrl}${record.imgurl}' style='max-height:350px;'/>">
															<img src="${fileDomainUrl}${record.imgurl}" />
													</a></td>
													<td class="center"><c:out
															value="${record.imgtype == '0' ? '横图' : '竖图' }"></c:out></td>
													<td align="right"><c:out value="${record.orderNum }"></c:out></td>
													<td class="center"><c:out
															value="${record.status == '0' ? '关闭' : '启用' }"></c:out></td>
													<td class="center"><fmt:formatDate value="${record.createTime }" pattern="yyyy-MM-dd HH:mm:ss"/> </td>
													<td class="center">
														<div class="visible-md visible-lg hidden-sm hidden-xs">
															<shiro:hasPermission name="contentenvironment:MODIFY">
															...
															<shiro:hasPermission name="contentenvironment:MODIFY">
																<a
																	class="btn btn-transparent btn-xs tooltips fn-UpdateBy"
																	data-id="${record.id }" data-toggle="tooltip"
																	tooltip-placement="top"
																	data-original-title="${record.status== '0' ? '启用' : '关闭' }"><i
																	class="fa fa-lightbulb-o fa-white"></i></a>
															</shiro:hasPermission>
															<shiro:hasPermission name="contentenvironment:DELETE">
							...
							<hyjf:paginator id="equiPaginator" hidden="paginator-page"
								action="init" paginator="${environmentForm.paginator}"></hyjf:paginator>
							<br /> <br />
						</div>
					</div>
				</div>
		...
		<tiles:putAttribute name="searchPanels" type="string">
			...
			<input type="hidden" name="paginatorPage" id="paginator-page"
				value="${environmentForm.paginatorPage}" />
			<div class="form-group">
				<label>公司环境名称</label> <input type="text" name="name"
					class="form-control input-sm underline"
					value="${environmentForm.name}" />
			</div>
			<div class="form-group">
				<label>图片类型</label> <select name="imgtype"
			...
			<div class="form-group">
				<label>状态</label> <select name="status"
					...
					<option value="">全部</option>
					<option value="0"
						<c:if test="${environmentForm.status==0}">selected="selected"</c:if>>关闭</option>
					<option value="1"
						<c:if test="${environmentForm.status==1}">selected="selected"</c:if>>启用</option>
				</select>
			</div>
			<div class="form-group">
				<label>添加时间</label>
...
.popover img {
	max-width: 460px;
		...
		<tiles:putAttribute name="pageJavaScript" type="string">
			<script type='text/javascript'
				src="${webRoot}/jsp/manager/content/contentenvironment/contentenvironment.js"></script>
		</tiles:putAttribute>
