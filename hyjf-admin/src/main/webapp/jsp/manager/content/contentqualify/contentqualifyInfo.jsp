...
<tiles:insertTemplate template="/jsp/layout/dialogLayout.jsp"
	...
	<tiles:putAttribute name="mainContentinner" type="string">
		<c:set var="jspEditType"
			value="${empty contentqualifyForm.id ? '添加' : '修改'}"></c:set>
		<div class="panel panel-white" style="margin: 0">
			<div class="panel-body" style="margin: 0 auto">
				<div class="panel-scroll height-430">
					<form id="mainForm"
						action="${empty contentqualifyForm.id ? 'insertAction' : 'updateAction'}"
						method="post" role="form" class="form-horizontal">
						<%-- 列表一览 --%>
						<input type="hidden" name="id" id="id"
							value="${contentqualifyForm.id }" /> <input type="hidden"
							name="pageToken" value="${sessionScope.RESUBMIT_TOKEN}" /> <input
							type="hidden" id="success" value="${success}" />
						<div class="form-group">
							<label class="col-xs-2 control-label padding-top-5" for="type"> <span
								class="symbol required"></span>资质名称
							</label>
							<div class="col-xs-10">
								<input type="text" placeholder="名称" id="name" name="name"
									value="${contentqualifyForm.name}" class="form-control"
									datatype="s2-20" errormsg="名称只能是字符汉字，长度2~20个字符！">
						...
						<div class="form-group">
							...
							<div class="fileinput fileinput-new col-xs-6" data-provides="fileinput">
								...
								<div class="fileinput-new thumbnail">
									<img width="160" height="120" src="${fileDomainUrl}${contentqualifyForm.imgurl}" alt="">
								</div> 
								<!-- 显示图片 -->
								...
								<div class = "purMargin">
									<input type="text" readonly="readonly"
										name="imgurl" id="imgurl" value="${contentqualifyForm.imgurl}" placeholder="上传图片路径"/>
								</div>
								<!-- 按钮管理 -->
						...
						<div class="form-group">
							<label class="col-xs-2 control-label padding-top-5" for="describe"> <span
								class="symbol"></span>资质描述
							</label>
							<div class="col-xs-10">
								<textarea maxlength="200" placeholder="办公描述" id="describe"
									name="describe" class="form-control limited">${contentqualifyForm.describe}</textarea>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-xs-10">
								<input type="text" placeholder="排序" id="orderNum"
									name="orderNum" value="${contentqualifyForm.orderNum}"
							...
							<div class="col-xs-10">
								<div class="radio clip-radio radio-primary ">
									<input type="radio" id="statusOn" name="status" datatype="*"
										...
										${contentqualifyForm.status == '1' ? 'checked' : ''}>
									<label for="statusOn"> 启用 </label> <input type="radio"
										id="statusOff" name="status" datatype="*" value="1"
										class="event-categories"
										${contentqualifyForm.status == '0' ? 'checked' : ''}>
									<label for="statusOff"> 关闭 </label>
								</div>
							</div>
	...
	<tiles:putAttribute name="pageJavaScript" type="string">	
		<script type='text/javascript'
			src="${webRoot}/jsp/manager/content/contentqualify/contentqualifyInfo.js"></script>
	</tiles:putAttribute>
