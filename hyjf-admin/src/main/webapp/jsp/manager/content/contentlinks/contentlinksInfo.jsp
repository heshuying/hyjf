...
<tiles:insertTemplate template="/jsp/layout/dialogLayout.jsp"
	flush="true">
	<%-- 画面的标题 --%>
	<tiles:putAttribute name="pageTitle" value="友情链接管理" />
	
	<%-- 画面主面板 --%>
	<tiles:putAttribute name="mainContentinner" type="string">
		<c:set var="jspEditType"
			value="${empty contentlinksForm.id ? '添加' : '修改'}"></c:set>
		<div class="panel panel-white" style="margin: 0">
			<div class="panel-body" style="margin: 0 auto">
				<div class="panel-scroll height-430">
					<form id="mainForm"
						action="${empty contentlinksForm.id ? 'insertAction' : 'updateAction'}"
						method="post" role="form" class="form-horizontal">
						<%-- 友情链接列表一览 --%>
						<input type="hidden" name="id" id="id"
							value="${contentlinksForm.id }" /> <input type="hidden"
						...
						<div class="form-group">
							<label class="col-xs-2 control-label padding-top-5" for="webname"> <span
								class="symbol required"></span>网站名称
							</label>
							<div class="col-xs-10">
								<input type="text" placeholder="网站名称" id="webname"
									name="webname" value="${contentlinksForm.webname}"
									class="form-control" datatype="s1-20"
									errormsg="名称 只能是字符汉字，长度1~20个字符！">
								<hyjf:validmessage key="webname" label="名称"></hyjf:validmessage>
							</div>
						</div>
						
						<div class="form-group">
							<label class="col-xs-2 control-label padding-top-5" for="url"> <span
								class="symbol required"></span>网站链接
							</label>
							<div class="col-xs-10">
								<input type="text" placeholder="网站url" id="url" name="url"
									value="${contentlinksForm.url}" class="form-control"
									datatype="*1-60" errormsg="url 长度1~60个字符！">
						...
						<div class="form-group">
							<label class="col-xs-2 control-label padding-top-5" for="logo"> <span
								class="symbol required"></span>友情连接图片
							</label>
							<div class="fileinput fileinput-new col-xs-6" data-provides="fileinput">
								<!-- 缺省图片 -->
								<div class="fileinput-new thumbnail">
									<img width="160" height="120" src="${fileDomainUrl}${contentlinksForm.logo}" alt="">
								</div> 
								<!-- 显示图片 -->
								...
								<div class = "purMargin">
									<input type="text" readonly="readonly" 
										name="logo" id="logo" value="${contentlinksForm.logo}" placeholder="上传图片路径"/>
								</div>
								<!-- 按钮管理 -->
						...
						<div class="form-group">
							<label class="col-xs-2 control-label padding-top-5" for="content"> <span
								class="symbol"></span>友情连接内容
							</label>
							<div class="col-xs-10">
								<textarea placeholder="友情连接说明" id="summary" name="summary"
									class="form-control limited">${contentlinksForm.summary}</textarea>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-xs-10">
								<input type="text" placeholder="排序只能是数字" id="order" name="order"
									value="${contentlinksForm.order}" class="form-control"
							...
							<div class="col-xs-10">
								<div class="radio clip-radio radio-primary ">
									<input type="radio" id="statusOn" name="status" datatype="*"
										...
										${contentlinksForm.status == '0' ? 'checked' : ''}> <label
										for="statusOn"> 关闭 </label> <input type="radio" id="statusOff"
										name="status" datatype="*" value="1" class="event-categories"
										${contentlinksForm.status == '1' ? 'checked' : ''}> <label
										for="statusOff"> 启用 </label>
								</div>
							</div>
							<hyjf:validmessage key="status" label="友情链接状态"></hyjf:validmessage>
						</div>
	...
	<tiles:putAttribute name="pageJavaScript" type="string">
		...
		<script type='text/javascript'
			src="${webRoot}/jsp/manager/content/contentlinks/contentlinksInfo.js"></script>
	</tiles:putAttribute>
