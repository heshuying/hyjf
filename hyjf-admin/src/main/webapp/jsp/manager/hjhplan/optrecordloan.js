...
/* 对应JAVA的Controllor的@RequestMapping */
	Action = {
        ...
        // 运营记录-出借明细标签Action
        optTenderFlagAction : webRoot + "/manager/borrow/borrowinvest/optActionSearch?isOptFlag=1",
		// 运营记录-承接明细标签Action
...
/* 事件动作处理 */
	Events = {
		...
		tenderInfoClkAct: function(event) {
			...
			$("#end-date-time").val("");
			$("#planNidSrch").val("");
			var value = $("#planNidTemp").val();
            $("#myPlanNid").val(value);
			Page.submit(Action.optTenderFlagAction);
		},
		// 刷新按钮的单击动作事件
		...
		optCreditTenderClkAct : function(selection, cds) {
			...
			//var planNid = $("#planNidSrch").val();
			//Page.primaryKey2.val(planNid);
			Page.primaryKey3.val($("#assignPlanNid").val());
			$("#myPlanNid").val($("#planNidTemp").val());
...
$.extend(Page, {
	...
	primaryKey2 : $("#planNidNew"), // 债转标的传参
	primaryKey3 : $("#assignPlanNid"), // 承接明细传参
	primaryKey9 : $("#planNidTemp"),   //  全局传参数用
	// 画面布局
	doLayout: function() {
		// 条件下拉框
...
//		$("#pageUrl").val(window.location.href);
		// 边界面板按钮的单击事件绑定
			...
			// 标签栏点击事件绑定
		    $(".ysbd").click(Events.searchClkAct),
