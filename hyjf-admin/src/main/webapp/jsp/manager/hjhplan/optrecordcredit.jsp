...
<shiro:hasPermission name="borrow:VIEW">
	<tiles:insertTemplate template="/jsp/layout/listLayout.jsp" flush="true">
					...
					<div class="tab-content">
							...
							<div class="col-md-12">
								<div class="search-classic">
											...
											<c:otherwise>
												<c:forEach items="${recordList }" var="record" begin="0" step="1" varStatus="status">
													<tr>
														...
														<td class="center"><c:out value="${record.liquidatesPeriod }/${record.borrowPeriod }"/></td>
														...
														<td class="center">
                                                            <shiro:hasPermission name="hjhdebtcredit:INFO">
															<a class="btn btn-transparent btn-xs tooltips fn-Info" data-creditnid="${record.creditNid }"
															   data-toggle="tooltip" tooltip-placement="top" data-original-title="承接明细">承接明细</a>
														</shiro:hasPermission>
														</td>
													</tr>
												</c:forEach>
									...
									<shiro:hasPermission name="borrow:SEARCH">
										<hyjf:paginator id="equiPaginator" hidden="paginator-page" action="optCreditActionSearch" paginator="${hjhDebtCreditForm.paginator}"></hyjf:paginator>
									</shiro:hasPermission>
									<br/><br/>
								</div>
							</div>
							</div>
							<input type="hidden" id="planNidTemp" name ="planNidTemp" value="${hjhDebtCreditForm.planNidTemp}">
							<%-- 债转标的检索面板 (ignore) --%>
							<tiles:putAttribute name="searchPanels" type="string">
								<shiro:hasPermission name="hjhdebtcredit:SEARCH">
									<input type="hidden" name="export" id="export" />
									<input type="hidden" name="paginatorPage" id="paginator-page" value="${hjhDebtCreditForm.paginatorPage}" />
									<input type="hidden" name="assignPlanNid" id="assignPlanNid" value="${hjhDebtCreditForm.planNid}">
									<input type="hidden" id="myPlanNid" name="planNidTemp"  value="${hjhDebtCreditForm.planNidTemp}">
									...
									<div class="form-group">
										...
										<input type="hidden" name="planNidSrch" id="planNidSrch" value="${hjhDebtCreditForm.planNid}">
									</div>
									<div class="form-group">
									...
									<div class="form-group">
										<label>清算后智投编号</label>
										<input type="text" name="planNidNew" id="planNidNew" class="form-control input-sm underline" value="${hjhDebtCreditForm.planNidNew}" />
									</div>
									<div class="form-group">
										<label>出让人</label>
									...
									<%--预期年化承接收益率 start--%>
									<div class="form-group">
										<label>预计承接出借利率</label>
										<div class="input-group input-daterange datepicker">
											<span class="input-icon">
												<input type="text" name="actualAprStartSrch" id="actualAprStartId"  class="form-control underline" value="${hjhDebtCreditForm.actualAprStartSrch}" onkeyup="value=value.replace(/[^\d.]/g,'').replace(/^\./g,'').replace(/\.{2,}/g,'.').replace('.','$#$').replace(/\./g,'').replace('$#$','.').replace(/^(\-)*(\d+)\.(\d\d).*$/,'$1$2.$3')"/>
												<i class="ti-calendar"></i>
											</span>
											<span class="input-group-addon no-border bg-light-orange">~</span>
											<input type="text" name="actualAprEndSrch"  id="actualAprEndId" class="form-control underline" value="${hjhDebtCreditForm.actualAprEndSrch}" onkeyup="value=value.replace(/[^\d.]/g,'').replace(/^\./g,'').replace(/\.{2,}/g,'.').replace('.','$#$').replace(/\./g,'').replace('$#$','.').replace(/^(\-)*(\d+)\.(\d\d).*$/,'$1$2.$3')" />
											<label>%</label>
										</div>
									</div>
									<%--预期年化承接收益率 end--%>
						...
						<span class="input-icon">
							...
							<i class="ti-calendar"></i> </span>
									...
									<div class="form-group">
										<label>转让标签</label>
										<input type="text" name="labelName" maxlength = "10" class="form-control input-sm underline" value="${hjhDebtCreditForm.labelName}" />
									</div>
								</shiro:hasPermission>
							</tiles:putAttribute>
						</div>
		...
		<tiles:putAttribute name="pageJavaScript" type="string">
			<script type='text/javascript' src="${webRoot}/jsp/manager/hjhplan/optrecordcredit.js"></script>
		</tiles:putAttribute>
