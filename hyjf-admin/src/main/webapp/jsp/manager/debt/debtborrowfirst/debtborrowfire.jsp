...
<tiles:insertTemplate template="/jsp/layout/dialogLayout.jsp" flush="true">
					...
					<div class="panel panel-white">
						<div class="panel-body panel-table">
								...
								<tr>
									...
									<td align="left"> 									
										<div class="radio clip-radio radio-primary radio-inline">
											<input type="radio" id="verifyStatus1" class="fn-Radio" name="verifyStatus" <c:if test="${ ( fireInfo.verifyStatus eq '1' ) }">checked="checked"</c:if> <c:if test="${canUpdate}">disabled="disabled"</c:if> value="1">
											<label for="verifyStatus1"> 定时发标 </label>
											<input type="radio" id="verifyStatus2" class="fn-Radio" name="verifyStatus" <c:if test="${ ( fireInfo.verifyStatus eq '2' ) }">checked="checked"</c:if> <c:if test="${canUpdate}">disabled="disabled"</c:if> value="2">
											<label for="verifyStatus2"> 立即发标 </label>
											<input type="radio" id="verifyStatus3" class="fn-Radio" name="verifyStatus" <c:if test="${ ( fireInfo.verifyStatus eq '3' ) or ( fireInfo.verifyStatus eq '0' ) }">checked="checked"</c:if> <c:if test="${canUpdate}">disabled="disabled"</c:if> value="3">
									...
									<tr id="ontimeCtrl2">
										<c:if test="${projectType==11}">
										<!-- 只有是尊享汇时才显示 -->
										<td align="right" class="tlabel"><span class="symbol required"></span>预约开始时间：</td>
										<td align="left">
											<span class="input-icon input-icon-right">
												<input type="text" name="bookingBeginTime" id="bookingBeginTime" class="form-control input-sm"<c:if test="${canUpdateTime}">disabled="disabled"</c:if> 
													value="<c:out value="${ bookingBeginTime }" />"  maxlength="16" datatype="*1-16" nullmsg="未填写预约开始时间"
													onclick="WdatePicker({skin:'twoer', dateFmt:'yyyy-MM-dd HH:mm', errDealMode: 1})"/>
												<i class="fa fa-calendar"></i>
											</span>
									...
									<tr id="ontimeCtrl3">
										<c:if test="${projectType==11}">
										<!-- 只有是尊享汇时才显示 -->
										<td align="right" class="tlabel"><span class="symbol required"></span>预约截止时间：</td>
										<td align="left">
											<span class="input-icon input-icon-right">
												<input type="text" name="bookingEndTime" id="bookingEndTime" class="form-control input-sm" <c:if test="${canUpdateTime}">disabled="disabled"</c:if>
													value="<c:out value="${ bookingEndTime }" />"  maxlength="16" datatype="*1-16" nullmsg="未填写预约截止时间"
													onclick="WdatePicker({skin:'twoer', dateFmt:'yyyy-MM-dd HH:mm', errDealMode: 1})"/>
												<i class="fa fa-calendar"></i>
											</span>
							...
							<table class="center" style="width:95%">
								...
								<tr>
									<td align="right" class="tlabel">出借利率率：</td>
	...
	<tiles:putAttribute name="pageJavaScript" type="string">
		...
		<script type='text/javascript' src="${webRoot}/jsp/manager/debt/debtborrowfirst/debtborrowfire.js"></script>
	</tiles:putAttribute>
