...
<shiro:hasPermission name="activitylist:VIEW">
    <tiles:insertTemplate template="/jsp/layout/listLayout.jsp" flush="true">
            ...
            <div class="container-fluid container-fullw bg-white">
                <div class="tabbable">
                    <ul class="nav nav-tabs" id="myTab">
                        ...
                        <shiro:hasPermission name="activitylist:VIEW">
                            <li class="active"><a href="${webRoot}/manager/activity/qixiactivity/awardinit">奖励明细</a></li>
                        </shiro:hasPermission>
                    </ul>
                            ...
                            <table id="equiList" class="table table-striped table-bordered table-hover">
                                ...
                                <tr>
                                    ...
                                    <th class="center">
                                        ...
                                        <c:choose>
                                            <c:when test="${ !empty qixiactivityListForm.sort and qixiactivityListForm.col eq 'qx.update_time' }">
                                                <a href="#" class="fn-SortTwo" data-col="qx.update_time" data-sort="${qixiactivityListForm.sort}" style="color:black;">发放时间</a>&nbsp;<i class="fa fa-sort"></i>
                                            </c:when>
                                            <c:otherwise>
                                                <a href="#" class="fn-SortTwo" data-col="qx.update_time" data-sort="DESC" style="color:black;">发放时间</a>&nbsp;<i class="fa fa-sort"></i>
                                            </c:otherwise>
                                        </c:choose>
                                        <%--出借时间--%>
                                    </th>
                                    ...
                                    <c:otherwise>
                                        <c:forEach items="${qixiactivityListForm.recordList }" var="record" begin="0" step="1" varStatus="status">
                                            <tr>
                                                ...
                                                <td class="center"><c:out value="${record.awardName }"></c:out></td>
                                                ...
                                                <td class="center">
                                                    <c:if test="${record.rewardStatus == '1'}">
                                                        <c:out value=" ${fn:substring(record.distributionTime, 0, 19)}"></c:out>
                                                    </c:if>
                                                </td>
                                            </tr>
                                        </c:forEach>
                                ...
                                <%-- 分页栏 --%>
                            <hyjf:paginator id="equiPaginator" hidden="paginator-page" action="awardinit" paginator="${qixiactivityListForm.paginator}"></hyjf:paginator>
                            <br/><br/>
                        </div>
                    </div>
                </div>
        ...
        <tiles:putAttribute name="searchPanels" type="string">
            ...
            <input type="hidden" name="colTwo" id="colTwo" value="${qixiactivityListForm.colTwo}" />
            <input type="hidden" name="sortTwo" id="sortTwo" value="${qixiactivityListForm.sortTwo}" />
            <div class="form-group">
                <label>奖励类型</label>
                <select name="awardType" class="form-control underline form-select2">
                    <option value="">全部</option>
                        <%--<option value="0" >汇直投</option>--%>
                        <%--<option value="1">汇计划</option>--%>
                    <option value="加息券" <c:if test="${qixiactivityListForm.awardType == '加息券'}">selected="selected"</c:if>>加息券</option>
                    <option value="实物" <c:if test="${qixiactivityListForm.awardType == '实物'}">selected="selected"</c:if>>实物</option>
                </select>
            </div>
            <div class="form-group">
                <label>奖励批号</label>
                <input type="text" name="awardNum" id="awardNum" class="form-control underline" value="${qixiactivityListForm.awardNum}" />
            </div>
            <div class="form-group">
                <label>发放方式</label>
                <select name="grandWay" class="form-control underline form-select2">
                    <option value="">全部</option>
                    <option value="0" <c:if test="${qixiactivityListForm.grandWay == '0'}">selected="selected"</c:if>>系统发放</option>
                    <option value="1" <c:if test="${qixiactivityListForm.grandWay == '1'}">selected="selected"</c:if>>手动发放</option>
                </select>
            </div>
            <div class="form-group">
                <label>账户名</label>
            ...
            <div class="form-group">
                ...
                <select name="status" class="form-control underline form-select2">
                    <option value="">全部</option>
                    <option value="0" <c:if test="${qixiactivityListForm.status == '0'}">selected="selected"</c:if>>待发放</option>
                    <option value="1" <c:if test="${qixiactivityListForm.status == '1'}">selected="selected"</c:if>>已发放</option>
                </select>
            </div>
        </tiles:putAttribute>
        ...
        <tiles:putAttribute name="pageJavaScript" type="string">
            <script type='text/javascript' src="${webRoot}/jsp/manager/activity/qixiactivity2018/qixiactivityaward.js"></script>
        </tiles:putAttribute>
