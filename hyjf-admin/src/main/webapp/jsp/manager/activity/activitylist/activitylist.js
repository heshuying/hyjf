...
Action = {
	...
	// 详细列表的Action
	listInfoAction : "getActivityInfoAction",
},
/* 事件动作处理 */
Events = {
	...
	addClkAct : function() {
		$.colorbox({
			overlayClose: false,
			href: "#urlDialogPanel",
			title: "<i class=\"fa fa-plus\"></i> 添加活动配置",
	...
	// 边界面板查询按钮的单击事件
	searchListInfoAct : function(event) {
		var action = jQuery(this).attr("action");
		$("#paginator-page").val(1);
		Page.submit(webRoot + action);
...
$.extend(Page, {
	...
	initEvents : function() {
		...
		// 重置按钮的单击事件绑定
		$(".fn-Info").click(Events.searchListInfoAct);
	}
});
