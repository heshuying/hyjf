...
<c:set var="searchAction" value="#" scope="request"></c:set>
	...
	<tiles:insertTemplate template="/jsp/layout/listLayout.jsp" flush="true">
				...
				<div class="tab-content">
					<div class="tab-pane fade in active">
										...
										<c:otherwise>
											<c:forEach items="${recordList }" var="record" begin="0" step="1" varStatus="status">
												<tr>
													...
													<td class="center"><c:out value="${record.activity eq '0' ? '68元代金券' : '1.5%加息券' }"></c:out></td>
							...
							<shiro:hasPermission name="activitylist:SEARCH">
								<hyjf:paginator id="equiPaginator" hidden="paginator-page" action="searchAction" paginator="${newUserListForm.paginator}"></hyjf:paginator>
							</shiro:hasPermission>
							<br/><br/>
					</div>
				</div>
			...
			<shiro:hasPermission name="activitylist:SEARCH">
				...
				<div class="form-group">
					...
					<select name="activitySrch" class="form-control underline form-select2">
						<option value=""></option>
						<option value="0" <c:if test="${newUserListForm.activitySrch=='0'}">selected</c:if>>68元代金券</option>
						<option value="1" <c:if test="${newUserListForm.activitySrch=='1'}">selected</c:if>>1.5%加息券</option>
					</select>
				</div>
				<div class="form-group">
					<label>发放状态</label>
		...
		<tiles:putAttribute name="pageJavaScript" type="string">
			<script type='text/javascript' src="${webRoot}/jsp/manager/activity/newuser/newuser.js"></script>
		</tiles:putAttribute>
