...
<shiro:hasPermission name="worldCupActivity:VIEW">
    <tiles:insertTemplate template="/jsp/layout/listLayout.jsp" flush="true">
            ...
            <div class="container-fluid container-fullw bg-white">
                <div class="tabbable">
                    <ul class="nav nav-tabs" id="myTab">
                        <shiro:hasPermission name="worldCupActivity:VIEW">
                            <li class="active"><a href="${webRoot}/manager/activity/worldCupActivity/init">竞猜输赢</a></li>
                        </shiro:hasPermission>
                        <shiro:hasPermission name="worldCupActivity:VIEW">
                            <li><a href="${webRoot}/manager/activity/worldCupActivity/winningInit">竞猜冠军</a></li>
                        </shiro:hasPermission>
                    </ul>
                        ...
                        <div class="tab-pane fade in active">
                            ...
                            <table id="equiList" class="table table-striped table-bordered table-hover">
                                ...
                                <tr>
                                    ...
                                    <th class="center">竞猜场次</th>
                                    <th class="center">用户的竞猜</th>
                                    <th class="center">比赛结果</th>
                                    <th class="center">竞猜结果</th>
                                    <th class="center">第几次竞猜</th>
                                    <th class="center">猜中场数</th>
                                    <%--<th class="center">猜中场数排行</th>--%>
                                    ...
                                    <c:otherwise>
                                        <c:forEach items="${worldCupActivityForm.recordList }" var="record" begin="0" step="1" varStatus="status">
                                            <tr>
                                                ...
                                                <td class="center"><c:out value="${record.batchName}"></c:out></td>
                                                <td class="center"><c:out value="${record.teamName}"></c:out></td>
                                                <td class="center" >
                                                        <c:out value="${record.matchResult}"></c:out>
                                                </td>
                                                <td class="center" >
                                                    <c:if test="${record.guessingMatchResult ==null}">${record.guessingMatchResult }</c:if>
                                                    <c:if test="${record.guessingMatchResult !=null}">
                                                        <c:out value="${record.guessingMatchResult ==1? '猜中':'未猜中' }"></c:out>
                                                    </c:if>
                                                </td>
                                                <td class="center"><c:out value="${record.guessingMatchNum }"></c:out></td>
                                                <td class="center"><c:out value="${record.guessingFieldNum }"></c:out></td>
                                                <%--<td class="center"><c:out value="${record.guessingRankings }"></c:out></td>--%>
                                ...
                                <%-- 分页栏 --%>
                            <hyjf:paginator id="equiPaginator" hidden="paginator-page" action="init" paginator="${worldCupActivityForm.paginator}"></hyjf:paginator>
                            <br/><br/>
                        </div>
                    </div>
                </div>
        ...
        <tiles:putAttribute name="pageJavaScript" type="string">
            <script type='text/javascript' src="${webRoot}/jsp/manager/activity/worldCupActivity/worldCupActivity.js"></script>
        </tiles:putAttribute>
