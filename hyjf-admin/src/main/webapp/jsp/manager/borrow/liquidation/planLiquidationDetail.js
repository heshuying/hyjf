...
Action = {
	...
	// 导出的Action
	exportAction : "infoExportAction",
	// 清算的Action
	liquidationAction : "liquidationAction",
	// 转让记录的Action
	transferRecordAction : webRoot + "/manager/borrow/plancredit/init.do",
...
Events = {
	...
	updateClkAct : function(event) {
		...
		var param = {};
		param.plannid = $(this).data("plannid");
		
		Page.confirm("", "确定要对"+plannid+",进行还款吗？", function(isConfirm) {
			if (isConfirm) {
	...
	// 修改按钮的单击动作事件
	liquidationClkAct: function(event) {
		Page.primaryKey.val($(this).data("plannid"));
		$("#accedeOrderId").val($(this).data("plannid"));
			Page.submit(Action.liquidationAction);
	},
	
	transferRecordClkAct: function(event) {
//		Page.primaryKey.val($(this).data("planOrderId"));
		$("#planOrderId").val($(this).data("planorderid"));
		Page.submit(Action.transferRecordAction);
	},
...
$.extend(Page, {
	...
	initEvents : function() {
		...
		// 次数清零单击事件绑定
		$(".fn-liquidation").click(Events.liquidationClkAct),
		// 出借单击事件绑定
		$(".fn-invest").click(Events.investClkAct),
		// 出借明细按钮的单击事件绑定
		$(".fn-transferRecord").click(Events.transferRecordClkAct);
	}
});
