...
.table-striped .checkbox {
	...
	overflow: hidden
				...
				<div class="row">
					<div class="col-md-12">
						<div class="search-classic">
							<%-- 功能栏 --%>
							<div class="well">
								<c:set var="jspPrevDsb" value="${borrowRepaymentPlanForm.paginator.firstPage ? ' disabled' : ''}"></c:set>
								<c:set var="jspNextDsb" value="${borrowRepaymentPlanForm.paginator.lastPage ? ' disabled' : ''}"></c:set>
											...
											<c:forEach items="${recordList }" var="record" begin="0"
												...
												<tr>
													<td class="center"><c:out
															value="${status.index+((borrowRepaymentPlanForm.paginatorPage - 1) * borrowRepaymentPlanForm.paginator.limit) + 1 }"></c:out></td>
							...
							<hyjf:paginator id="equiPaginator" hidden="paginator-page"
								action="init" paginator="${borrowRepaymentPlanForm.paginator}"></hyjf:paginator>
							<br /> <br />
						</div>
					</div>
				</div>
		...
		<tiles:putAttribute name="searchPanels" type="string">
				...
				<select name="instCodeSrch" id="instCodeSrch" class="form-control underline form-select2">
					...
					<c:forEach items="${hjhInstConfigList }" var="inst" begin="0" step="1">
						<option value="${inst.instCode }"
							<c:if test="${inst.instCode eq borrowRepaymentPlanForm.instCodeSrch}">selected="selected"</c:if>>
			...
			<div class="form-group">
				<label>还款状态</label> <select name="status"
					...
					<option value="0"
						<c:if test="${borrowRepaymentPlanForm.status==0}">selected="selected"</c:if>>未还款</option>
					<option value="1"
						<c:if test="${borrowRepaymentPlanForm.status==1}">selected="selected"</c:if>>已还款</option>
			...
			<div class="form-group">
				...
				<div class="input-group input-daterange datepicker">
					<span class="input-icon"> <input type="text"
						...
						value="${borrowRepaymentPlanForm.repayLastTimeStartSrch}" /> <i
						class="ti-calendar"></i>
					...
					<input type="text" name="repayLastTimeEndSrch"
						...
						value="${borrowRepaymentPlanForm.repayLastTimeEndSrch}" />
				</div>
			</div>
		</tiles:putAttribute>
		...
		<tiles:putAttribute name="pageJavaScript" type="string">
			...
			<script type='text/javascript'
				src="${webRoot}/jsp/manager/borrow/borrowrepayment/plan/repayplan.js"></script>
		</tiles:putAttribute>
