...
Events = {
	...
	exportClkAct: function(event) {
		...
		if(startTime == "" || endTime == ""){
			Page.confirm("","请选择导出数据的应还款日期起止时间（起止时间需小于等于31天）","error",{showCancelButton: false}, function(){});
			return false;
		}
...
$.extend(Page, {
	...
	doLayout : function() {
	 ...
	 // 日历选择器
		$('#recoverYesTimeStartSrch').datepicker({
			autoclose: true,
			todayHighlight: true
		});
		$('#recoverYesTimeEndSrch').datepicker({
			autoclose: true,
			todayHighlight: true
		});
		var day30 = 30 * 24 * 60 * 60 * 1000;
		$("#recoverYesTimeStartSrch").on("changeDate", function(ev) {
	        var selectedDate = new Date(ev.date.valueOf());
	        var endDate = $('#recoverYesTimeStartSrch').datepicker("getDate");
	        var finalEndDate = new Date(+selectedDate + day30);
	        if(endDate==null){
	        	 $('#recoverYesTimeStartSrch').datepicker("setDate", finalEndDate);
	        }else if (+selectedDate > +endDate) {
	        	 $('#recoverYesTimeStartSrch').datepicker("setDate", finalEndDate);
	        }else if(+endDate - selectedDate > day30){
	        	$('#recoverYesTimeStartSrch').datepicker("setDate", finalEndDate);
	    ...
	    $("#recoverYesTimeEndSrch").on("changeDate", function(ev) {
	        var selectedDate = new Date(ev.date.valueOf());
	        var startDate = $('#recoverYesTimeEndSrch').datepicker("getDate");
	        var finalEndDate = new Date(+selectedDate -day30);
	        if(startDate==null){
	        	$('#recoverYesTimeEndSrch').datepicker("setDate", finalEndDate);
	        }else if (+startDate > + selectedDate) {
	            $('#recoverYesTimeEndSrch').datepicker("setDate", finalEndDate);
	        }else if(+selectedDate - startDate > day30){
	        	$('#recoverYesTimeEndSrch').datepicker("setDate", finalEndDate);
