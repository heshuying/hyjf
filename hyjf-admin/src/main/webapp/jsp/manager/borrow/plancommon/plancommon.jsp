...
<tiles:insertTemplate template="/jsp/layout/listLayout.jsp" flush="true">
						...
						<div class="col-md-12">
							<div class="panel">
								<div class="form-group">
									...
									<c:if test="${!empty planForm.debtPlanNid }">
										<div>
											<select id="" name="" class="form-select2" style="width:30%" disabled="disabled" data-placeholder="请选择计划类型...">
												...
												<c:forEach items="${debtPlanConfigList }" var="debtPlanConfig" begin="0" step="1" varStatus="status">
													...
													data-debtPlanTypeName="${debtPlanConfig.debtPlanTypeName }" data-debtPlanType="${debtPlanConfig.debtPlanType}" data-debtInvestmentIncrement="${debtPlanConfig.debtInvestmentIncrement}" data-debtMaxInvestment ="${debtPlanConfig.debtMaxInvestment}"
														 data-debtLockPeriod="${debtPlanConfig.debtLockPeriod}" data-debtQuitPeriod="${debtPlanConfig.debtQuitPeriod }" data-couponConfig="${debtPlanConfig.couponConfig}"
											...
											<input type="hidden" id="debtPlanType" name="debtPlanType" class="form-control input-sm" value='<c:out value="${planForm.debtPlanType}"></c:out>' readonly="readonly"/>
											<hyjf:validmessage key="debtPlanType" label="计划类型"></hyjf:validmessage>
										</div>
									</c:if>
									<c:if test="${empty planForm.debtPlanNid }">
										<div>
											<select id="debtPlanType" name="debtPlanType" class="form-select2" datatype="*" nullmsg="未指定计划类型！" style="width:30%" data-placeholder="请选择计划类型...">
												...
												<c:forEach items="${debtPlanConfigList }" var="debtPlanConfig" begin="0" step="1" varStatus="status">
													<option value="${debtPlanConfig.debtPlanType }" data-debtPlanPrefix="${debtPlanConfig.debtPlanPrefix }" data-debtMinInvestment="${debtPlanConfig.debtMinInvestment}"
													data-debtPlanTypeName="${debtPlanConfig.debtPlanTypeName }" data-debtPlanType="${debtPlanConfig.debtPlanType}" data-debtInvestmentIncrement="${debtPlanConfig.debtInvestmentIncrement}" data-debtMaxInvestment ="${debtPlanConfig.debtMaxInvestment}"
														 data-debtLockPeriod="${debtPlanConfig.debtLockPeriod}" data-debtQuitStyle="${debtPlanConfig.debtQuitStyle}" data-debtQuitPeriod="${debtPlanConfig.debtQuitPeriod }" data-couponConfig="${debtPlanConfig.couponConfig}"
														<c:if test="${debtPlanConfig.debtPlanType eq planForm.debtPlanType}">selected="selected"</c:if>>
														<c:out value="${debtPlanConfig.debtPlanTypeName }"></c:out></option>
												</c:forEach>
											</select>
								...
								<c:if test="${empty planForm.debtPlanNid }">
									<div class="form-group">
										<label class="control-label">
											计划预编号 <span class="symbol required"></span>
										</label>
										<div class="input-group" style="width:30%">
											<input type="text" id="debtPlanPreNid" name="debtPlanPreNid" class="form-control input-sm" value='<c:out value="${planForm.debtPlanPreNid}"></c:out>'
													maxlength="20" datatype="*1-20" nullmsg="未填写计划预编号" ajaxurl="isExistsPlanPreNidRecord" errormsg="计划预编号长度不能超过20位"/>
								...
								<c:if test="${!empty planForm.debtPlanNid }">
									<div class="form-group">
										...
										<input type="hidden" id="debtPlanPreNidHid" name="debtPlanPreNidHid" class="form-control input-sm" value='<c:out value="${planForm.debtPlanPreNid}"></c:out>' readonly="readonly"/>
								...
								<div class="form-group">
									...
									<div class="input-group" style="width:30%">
										<input type="text" name="debtPlanName" id="debtPlanName" class="form-control input-sm" value='<c:out value="${planForm.debtPlanName}"></c:out>' readonly="readonly"
								...
								<div class="form-group">
									<label class="control-label">
										计划金额 <span class="symbol required"></span>
									</label>
									<div class="input-group" style="width:30%">
										<input type="text" id="debtPlanMoney" name="debtPlanMoney" class="form-control input-sm" value='<c:out value="${planForm.debtPlanMoney}"></c:out>' <c:if test="${ ( isEdit eq '1' ) }">readonly="readonly"</c:if>
												maxlength="10" datatype="n1-10,range" data-minvalue="1" data-maxvalue="9999999999" nullmsg="未填写计划金额" errormsg="计划金额至少为1元，最大不能超过9999999999元，且必须为数字" 
												 />
								...
								<div class="form-group">
									...
									<div class="input-group" style="width:30%">
										...
										<c:if test="${ ( isEdit ne '1' ) }">
											<input type="text" id="debtQuitPeriod" datatype="n1-2" nullmsg="请填写退出所需天数" errormsg="退出所需天数必须为1-2位数字！" maxlength="2" name="debtQuitPeriod" class="form-control input-sm" value='<c:out value="${planForm.debtQuitPeriod}"></c:out>' />
										</c:if>
								...
								<div class="form-group">
									...
									<div class="clip-radio radio-primary">
										<input type="radio" value="yes" name="isAudits" datatype="*" <c:if test="${ ( isEdit eq '1' ) }">disabled="disabled"</c:if> <c:if test="${ planForm.isAudits eq 'yes' }">checked</c:if> id="isAudits_yes" >
										<label for="isAudits_yes">立即提交审核</label>
										<input type="radio" value="no" name="isAudits" datatype="*" <c:if test="${ ( isEdit eq '1' ) }">disabled="disabled"</c:if> <c:if test="${ planForm.isAudits eq 'no' }">checked</c:if> id="isAudits_no">
						...
						<div class="col-md-12">
							...
							<div class="form-group">
								<textarea name="planConcept" datatype="*" nullmsg="未填写计划概念" class="tinymce" cols="10" rows="12">
									<c:set value="<p style='text-align: justify;'>汇添金出借计划是汇盈金服平台为出借人提供的本息自动循环出借及到期自动转让退出的投标工具，在出借人认可的标的范围内，系统对符合要求的标的进行自动投标。汇添金出借计划为出借人实现分散投标，更好的满足出借人多样化的出借需求。</p>" var="str1"></c:set>
									<c:if test="${planForm.planConcept !=null && planForm.planConcept !='' }"><c:out value="${planForm.planConcept }"></c:out></c:if>
									<c:if test="${planForm.planConcept == null || planForm.planConcept == ''}"><c:out value="${str1 }"></c:out></c:if>
								</textarea>
							</div>
						...
						<div class="col-md-12">
							...
							<div class="form-group">
								<textarea name="safeguardMeasures" datatype="*" nullmsg="未填写风控保障措施" class="tinymce" cols="10" rows="12">
										<c:set value="<p>汇盈金服平台风控团队具备丰富的法律、金融服务机构风险管理控制等相关行业经验，兼备贷前、贷中、贷后管理的综合能力，本着安全大于一切的理念，对平台的每个标的进行严格详细地审核，为出借方建立资金安全壁垒。</p>" var="str1"></c:set>
										<c:if test ="${planForm.safeguardMeasures !=null && planForm.safeguardMeasures !='' }"><c:out value="${planForm.safeguardMeasures }"></c:out></c:if>
										<c:if test ="${planForm.safeguardMeasures ==null || planForm.safeguardMeasures =='' }"><c:out value="${str1 }"></c:out></c:if>
									</textarea>
							</div>
						...
						<div class="col-md-12">
							...
							<div class="form-group">
								<textarea name="marginMeasures" datatype="*" nullmsg="风险保证金措施" class="tinymce" cols="10" rows="12">
										<c:set value="<p>在汇盈金服平台发布的每一笔融资项目都由合作机构按照该笔业务授信额度的2%-5%提交保证金，该笔风险保证金存放于中国建设银行为汇盈金服专门开设的独立账户中。若合作机构无法回购出借方债权，风险保证金将用于对出借方进行垫付.</p>" var="str1"></c:set>
										<c:if test="${planForm.marginMeasures != null && planForm.marginMeasures != '' }"><c:out value="${planForm.marginMeasures }"></c:out></c:if>
										<c:if test="${planForm.marginMeasures == null || planForm.marginMeasures == '' }"><c:out value="${str1 }"></c:out></c:if>
									</textarea>
							</div>	
					...
					<div class="row">
						<div class="col-md-12">
							<div class="panel">
								...
								<div class="form-group">
									...
									<div class="input-group" style="width:50%">
										<div class="checkbox clip-check check-primary checkbox-inline">
											<input type="checkbox" id="checkAllUse" value="all"  <c:if test="${ fn:contains(planForm.couponConfig, '1') and fn:contains(planForm.couponConfig, '2') and fn:contains(planForm.couponConfig, '3') }">checked</c:if> />
											<label for="checkAllUse">全部</label>
										</div>
										<div class="checkbox clip-check check-info checkbox-inline">
											<input type="checkbox" name="couponConfig" id="couponConfigTYJ" value="1" class="checkAllUsebox"  <c:if test="${fn:contains(planForm.couponConfig, '1')}">checked</c:if> />
											<label for="couponConfigTYJ">体验金</label>
										</div>
										<div class="checkbox clip-check check-info checkbox-inline">
											<input type="checkbox" name="couponConfig" id="couponConfigJXQ" value="2" class="checkAllUsebox" <c:if test="${fn:contains(planForm.couponConfig, '2')}">checked</c:if> />
											<label for="couponConfigJXQ">加息券</label>
										</div>
										<div class="checkbox clip-check check-info checkbox-inline">
											<input type="checkbox" name="couponConfig" id="couponConfigDJQ" value="3" class="checkAllUsebox" <c:if test="${fn:contains(planForm.couponConfig, '3')}">checked</c:if> />
										...
										<c:if test="${ ( isEdit ne '1' ) }">
											<span class="input-icon input-icon-right">
												<input type="text" name="buyBeginTime" id="buyBeginTime" class="form-control input-sm"
														...
														onclick="WdatePicker({skin:'twoer',minDate:'%y-%M-{%d}', dateFmt:'yyyy-MM-dd HH:mm', errDealMode: 1})"/>
								...
								<div class="form-group">
									<label class="control-label">
										最高授权服务金额<span class="symbol required"></span>
									</label>
									<div class="input-group" style="width:30%">
										<c:if test="${ ( isEdit eq '1' ) }">
										...
										<c:if test="${ ( isEdit ne '1' ) }">
											<input type="text" id="debtMaxInvestment" name="debtMaxInvestment" class="form-control input-sm" value='<c:out value="${planForm.debtMaxInvestment}"></c:out>' maxlength="10" datatype="/^[1-9]\d*000$/|/^[1-9]\d*000.00$/" nullmsg="未填写最高授权服务金额" errormsg="最高授权服务金额必须为数字,并且必须是1000的倍数" />
										</c:if>
										<span class="input-group-addon">元</span>
									</div>
					...
					<div class="row">
						<div class="col-md-12">
							<div class="search-classic">
								...
								<br/>
								<div class="form-group">
									<hyjf:validmessage key="borrow" label="关联资产"></hyjf:validmessage>
								</div>
								<%-- 列表一览 --%>
													...
													<tr>
														...
														<td align="center">
															<div align="left" class="checkbox clip-check check-primary checkbox-inline">
																<input type="checkbox" class="listCheck" <c:if test="${ ( isEdit eq '1' ) }">disabled="disabled"</c:if> <c:if test="${record.isSelected eq '1' }">checked='checked'</c:if> id="row${status.index }" value="${record.borrowNid }">
								...
								<%-- 分页栏 --%>
								<shiro:hasPermission name="planmanager:SEARCH">
									<%-- <hyjf:paginatorpost id="equiPaginator" hidden="paginator-page" action="searchAction" paginator="${planForm.paginator}"></hyjf:paginatorpost> --%>
								</shiro:hasPermission>
								<br/><br/>
							</div>
						</div>
	...
	<tiles:putAttribute name="pageJavaScript" type="string">
		<script type='text/javascript' src="${webRoot}/jsp/manager/borrow/plancommon/plancommon.js"></script>
	</tiles:putAttribute>
