...
$.extend(Page, {
	...
	doLayout: function() {
		...
		$('#reAuthPassStartSrch').datepicker({
			autoclose: true,
			todayHighlight: true,
			endDate:new Date()
		});
		$('#reAuthPassEndSrch').datepicker({
			autoclose: true,
			todayHighlight: true,
			endDate:new Date()
		});
		...
		$("#reAuthPassStartSrch").on("changeDate", function(ev) {
			var now = new Date();
			var selectedDate = new Date(ev.date.valueOf());
			var endDate = $('#reAuthPassEndSrch').datepicker("getDate");
			var finalEndDate = +selectedDate + day30 >= +now ? now : new Date(+selectedDate + day30);
			$('#reAuthPassEndSrch').datepicker("setStartDate", selectedDate);
			$('#reAuthPassEndSrch').datepicker("setEndDate", finalEndDate);
			//如果end值范围超过30天，设置end最大结束时间
			if (endDate != null && (+endDate - selectedDate >= day30)) {
				$('#reAuthPassEndSrch').datepicker("setDate", finalEndDate);
			}
		});
