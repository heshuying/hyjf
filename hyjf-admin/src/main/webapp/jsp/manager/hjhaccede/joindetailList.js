...
Events = {
    ...
    pdfSignClkAct : function(event) {
        ...
        Page.primaryKey2.val($(this).data("planorderid"));
	...
	doLayout: function() {
	 ...
	 // add by nxl 计息时间日历选择 start
	    ...
	    $('#end-time').datepicker({
	      ...
	      // add by nxl 计息时间日历选择 end

		//预计开始退出时间
        $('#endDateStartSrch').datepicker({
            autoclose: true,
            todayHighlight: true
        });
        $('#endDateEndSrch').datepicker({
            autoclose: true,
            todayHighlight: true
        });

        $("#endDateStartSrch").on("changeDate", function(ev) {
            var selectedDate = new Date(ev.date.valueOf());
            var endDate = $('#endDateEndSrch').datepicker("getDate");
            var finalEndDate = new Date(+selectedDate + day30);
            if(endDate==null){
                $('#endDateEndSrch').datepicker("setDate", finalEndDate);
        ...
        $("#endDateEndSrch").on("changeDate", function(ev) {
            var selectedDate = new Date(ev.date.valueOf());
            var startDate = $('#endDateStartSrch').datepicker("getDate");
            var finalEndDate = new Date(+selectedDate -day30);
            if(startDate==null){
                $('#endDateStartSrch').datepicker("setDate", finalEndDate);
		...
		//实际退出时间
        $('#acctualPaymentTimeStartSrch').datepicker({
            autoclose: true,
            todayHighlight: true,
            endDate:new Date()
        });
        $('#acctualPaymentTimeEndSrch').datepicker({
            autoclose: true,
            todayHighlight: true,
            endDate:new Date()
        });

        $("#acctualPaymentTimeStartSrch").on("changeDate", function(ev) {
            var now = new Date();
            var selectedDate = new Date(ev.date.valueOf());
            var endDate = $('#acctualPaymentTimeEndSrch').datepicker("getDate");
            var finalEndDate = +selectedDate + day30 >= +now ? now : new Date(+selectedDate + day30);
            $('#acctualPaymentTimeEndSrch').datepicker("setStartDate", selectedDate);
            $('#acctualPaymentTimeEndSrch').datepicker("setEndDate", finalEndDate);
            //如果end值范围超过30天，设置end最大结束时间
            if (endDate != null && (+endDate - selectedDate >= day30)) {
                $('#acctualPaymentTimeEndSrch').datepicker("setDate", finalEndDate);
            }
        });
        if($("#acctualPaymentTimeStartSrch").val() != ''){
            $('#acctualPaymentTimeStartSrch').datepicker("setDate", $("#acctualPaymentTimeStartSrch").val());
        }
