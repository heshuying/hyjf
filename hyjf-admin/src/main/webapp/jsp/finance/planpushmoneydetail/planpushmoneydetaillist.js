...
Action = {
	...
	// 查询的Action
	searchAction : "planPushMoneyDetailSearch",
},
/* 事件动作处理 */
Events = {
	// 发提成按钮的单击事件
	confirmPushMoneyClkAct : function(event) {
		if(event) {
			...
			Page.primaryKey1.val($(this).data("accedeorderid"))
		}
		Page.confirm("", "确定要执行本次发提成操作吗？", function(isConfirm) {
			if (isConfirm) {
				var param = {};
				param.ids = Page.primaryKey.val();
				param.orderId = Page.primaryKey1.val()
	...
	exportClkAct : function(selection, cds) {
		...
		var esTime = $("#return-start-date-time").val();
		var eeTime = $("#return-end-date-time").val();
		var flag1 = false;
		var flag2 = false;
...
$.extend(Page, {
	...
	doLayout: function() {
		...
		$('#return-start-date-time').datepicker({
			autoclose: true,
			todayHighlight: true,
			endDate:new Date()
		});
		$('#return-end-date-time').datepicker({
			autoclose: true,
			todayHighlight: true,
			endDate:new Date()
		});
	    ...
	    $("#return-start-date-time").on("changeDate", function(ev) {
	        var now = new Date();
	        var selectedDate = new Date(ev.date.valueOf());
	        var endDate = $('#return-end-date-time').datepicker("getDate");
	        var finalEndDate = +selectedDate + day30 >= +now ? now : new Date(+selectedDate + day30);
	        $('#return-end-date-time').datepicker("setStartDate", selectedDate);
	        $('#return-end-date-time').datepicker("setEndDate", finalEndDate);
	        //如果end值范围超过30天，设置end最大结束时间
	        if (endDate != null && (+endDate - selectedDate >= day30)) {
	            $('#return-end-date-time').datepicker("setDate", finalEndDate);
	        }
	    });
	    if($("#return-start-date-time").val() != ''){
	    	 $('#return-start-date-time').datepicker("setDate", $("#return-start-date-time").val());
	    }
