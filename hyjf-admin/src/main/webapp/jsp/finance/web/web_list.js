...
Action = {	
	// 导出execlAction
	exportExcelAction : "exportWeblistExcel",
    // 具有组织机构查看权限的增强导出
	...
	// 余额查询action
	yueSearchAction:"yueSearchAction"
},
/* 事件动作处理 */
Events = {
	...
	// 余额查询
	yueSearchClkAct:function(event) {
		$.colorbox({
			overlayClose : false,
			href : "#urlDialogPanel",
			title : "<i class='fa fa-pencil'></i> 余额查询",
			width : 650,
			height : 520,
			inline : true,
			fixed : true,
			...
			onOpen : function() {
				setTimeout(function() {
					Page.submit(Action.yueSearchAction, null, null, "dialogIfm");
				}, 0)
			},
			// Close事件回调
			onClosed : function() {
...
$.extend(Page, {
	...
	initEvents : function() {
		...
		// 余额查询
		$(".fn-yue-Search").click(Events.yueSearchClkAct);
	}
});
