...
//							Page.confirm("",data.result,"error",{showCancelButton: false}, function(){Events.refreshClkAct()});
	...
	exportClkAct : function(selection, cds) {
		...
		if((startTime != "" && endTime != "") || (startTimeSend != "" && endTimeSend != "")){
...
$.extend(Page, {
	...
	doLayout: function() {
		...
		var day30 = 30 * 24 * 60 * 60 * 1000;
	    $("#start-date-time-send").on("changeDate", function(ev) {
	        var now = new Date();
	        var selectedDate = new Date(ev.date.valueOf());
	        var endDate = $('#end-date-time-send').datepicker("getDate");
	        var finalEndDate = +selectedDate + day30 >= +now ? now : new Date(+selectedDate + day30);
	        $('#end-date-time-send').datepicker("setStartDate", selectedDate);
	        $('#end-date-time-send').datepicker("setEndDate", finalEndDate);
	        //如果end值范围超过30天，设置end最大结束时间
	        if (endDate != null && (+endDate - selectedDate >= day30)) {
	            $('#end-date-time-send').datepicker("setDate", finalEndDate);
	        }
	    });
	    if($("#start-date-time-send").val() != ''){
	    	 $('#start-date-time-send').datepicker("setDate", $("#start-date-time-send").val());
	    }
