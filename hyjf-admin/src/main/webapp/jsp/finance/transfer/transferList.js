...
Action = {
	...
	// 发送邮件
	transferSendMailAction : "transferSendMail",
	// 导出的Action
...
Events = {
	...
	addClkAct : function(event) {
		$.colorbox({
			overlayClose : false,
			href : "#urlDialogPanel",
			title : "<i class='fa fa-pencil'></i> 发起用户转账",
			width : 650,
			height : 420,
			inline : true,
			fixed : true,
	...
	sendClkAct:function(event){
		var id = $(this).data("transferid");
		$.ajax({
			url : Action.transferSendMailAction,
			type : "POST",
			async : true,
			data : {
				id : id
...
$.extend(Page, {
	...
	initEvents : function() {
		...
		// 修改按钮的单击事件绑定
		$(".fn-Send").click(Events.sendClkAct),
		// 查找按钮的单击事件绑定
	...
	initialize : function() {
		...
		Page.clip.on("copy", function(e){
		    e.clipboardData.setData("text/plain", $(e.target).data("transferurl"));
		    Page.notice("复制成功");
		});
	}
});
