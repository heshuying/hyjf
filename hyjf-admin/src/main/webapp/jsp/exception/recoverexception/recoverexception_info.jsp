	...
	<tiles:insertTemplate template="/jsp/layout/listLayout.jsp" flush="true">
				...
				<div class="row">
					<div class="col-md-12">
						<div class="search-classic">
							...
							<%-- 分页栏 --%>
							<shiro:hasPermission name="recoverexception:SEARCH">
								<hyjf:paginator id="equiPaginator" hidden="paginator-page" action="searchInfoAction" paginator="${form.paginator}"></hyjf:paginator>
							</shiro:hasPermission>
							<br/><br/>
						</div>
					</div>
		...
		<tiles:putAttribute name="searchPanels" type="string">
			<shiro:hasPermission name="recoverexception:SEARCH">
				...
				<div class="form-group">
					<label>出借订单号</label>
					<input type="text" name="orderNumSrch" id="orderNumSrch" class="form-control input-sm underline" value="${form.orderNumSrch}" />
				</div>
				<div class="form-group">
					<label>放款状态</label>
					<select name="isRecoverSrch" class="form-control underline form-select2">
						...
						<c:forEach items="${recoverList }" var="recover" begin="0" step="1" varStatus="status">
							<option value="${recover.nameCd }"
								<c:if test="${recover.nameCd eq form.isRecoverSrch}">selected="selected"</c:if>>
				...
				<div class="form-group">
					...
					<div class="input-group input-daterange datepicker">
						<span class="input-icon">
							<input type="text" name="timeRecoverStartSrch" id="recover-start-date-time" class="form-control underline" value="${form.timeRecoverStartSrch}" />
							<i class="ti-calendar"></i> </span>
						<span class="input-group-addon no-border bg-light-orange">~</span>
						<input type="text" name="timeRecoverEndSrch" id="recover-end-date-time" class="form-control underline" value="${form.timeRecoverStartSrch}" />
					</div>
				</div>
				<div class="form-group">
					<label>放款时间</label>
		...
		<tiles:putAttribute name="pageJavaScript" type="string">
			<script type='text/javascript' src="${webRoot}/jsp/exception/recoverexception/recoverexception_info.js"></script>
		</tiles:putAttribute>
