...
<tiles:insertTemplate template="/jsp/layout/mainLayout.jsp" flush="true">
	...
	<tiles:putAttribute name="mainContentinner" type="string">
		<c:set var="jspEditType" value="${empty msgPushMessagesForm.id ? '添加' : '修改'}"></c:set>
		<div class="container-fluid container-fullw bg-white">
			...
			<form id="mainForm" action="${empty msgPushMessagesForm.id ? 'insertAction' : 'updateAction'}" method="post" role="form" class="form-horizontal">
					...
					<div class="tab-pane fade in active" id="tabs_1">
						<div class="row">
							...
							<div class="col-xs-12 col-md-12">
								<input type="hidden" name="id" id="id" value="${msgPushMessagesForm.id }" /> <input type="hidden" name="pageToken" value="${sessionScope.RESUBMIT_TOKEN}" /> <input type="hidden" id="success" value="${success}" />
								<div class="form-group">
									...
									<div class="col-xs-8">
										<select id="tagId" name="tagId" class="form-select2" style="width: 70%" datatype="*" data-placeholder="请选择项消息标签...">
											<option value=""></option>
											<c:forEach items="${messagesPushTags }" var="tags" begin="0" step="1" varStatus="status">
												<option value="${tags.id }" tid="${tags.tagCode }" <c:if test="${tags.id eq msgPushMessagesForm.tagId}">selected="selected"</c:if>>
													<c:out value="${tags.tagName }"></c:out></option>
											</c:forEach>
										</select>
										<hyjf:validmessage key="tagId" label="消息标签"></hyjf:validmessage>
										<input type="hidden" id="tagCode" name="tagCode" value="${msgPushMessagesForm.tagCode}" />
									</div>
								</div>
								<div class="form-group">
									...
									<div class="col-xs-8">
										<input type="text" placeholder="消息标题" id="msgTitle" name="msgTitle" value="${msgPushMessagesForm.msgTitle}" class="form-control" maxlength="20" datatype="*1-20" errormsg="请输入消息标题！">
									...
									<div class="col-xs-8">
										<div class="fileinput fileinput-new picClass" data-provides="fileinput">
											...
											<div class="fileinput-new thumbnail">
												<img width="160" height="120" src="${fileDomainUrl}${msgPushMessagesForm.msgImageUrl}" alt="">
											</div>
											<!-- 显示图片 -->
											<div class="fileinput-preview fileinput-exists thumbnail"></div>
											<!-- 图片路径 -->
											<div class="purMargin">
												<input type="text" readonly="readonly" name="msgImageUrl" id="msgImageUrl" value="${msgPushMessagesForm.msgImageUrl}" placeholder="上传图片路径" />
								...
								<div class="form-group">
									...
									<div class="col-xs-8">
										<textarea name="msgContent" id="msgContent" placeholder="文章内容" maxlength="4000" class="form-control tinymce" cols="10" rows="5" datatype="*" errormsg="通知内容必填！" errormsg="未填写通知内容">${msgPushMessagesForm.msgContent }</textarea>
										<hyjf:validmessage key="msgContent" label="文章内容"></hyjf:validmessage>
									</div>
								</div>
								<div class="form-group margin-bottom-0">
									<div class="col-xs-offset-2 col-xs-8">
										<c:if test="${msgPushMessagesForm.updateOrReSend != 1}">
							 ...
							 <div class="form-group">
								<label class="col-xs-2 control-label" for="projectType"><span class="symbol required"></span>推送终端：</label>
									<div class="col-xs-8 checkbox clip-check check-primary inline" style="padding-left: 15px;">
										<c:set var="tempStrs" value="${fn:split(msgPushMessagesForm.msgTerminal, ',')}" />
								...
								<div class="form-group">
									...
									<div class="col-xs-8">
										<select id="msgAction" name="msgAction" class="form-select2" style="width: 70%" datatype="*" data-placeholder="请选择后续动作...">
											<c:forEach items="${messagesActions }" var="actions" begin="0" step="1" varStatus="status">
												<option value="${actions.nameCd }" <c:if test="${actions.nameCd eq msgPushMessagesForm.msgAction}">selected="selected"</c:if>>
													<c:out value="${actions.name }"></c:out></option>
											</c:forEach>
										</select>
										<hyjf:validmessage key="msgAction" label="后续动作"></hyjf:validmessage>
										<div id="actionPanel0" ${(msgPushMessagesForm.msgAction eq '0') ? '' : 'style="display:none;"'}>
											<!-- 打开APP -->
										</div>
										<c:choose>
										<c:when test="${msgPushMessagesForm.id == null}">
											<div id="actionPanel1" ${(msgPushMessagesForm.msgAction eq '1') ? '' : 'style="display:none;"'}>
												<!-- 打开H5页面 -->
												<input type="text" placeholder="请输入http地址（全路径）" id="messagesActionUrl1" name="messagesActionUrl1" value="${msgPushMessagesForm.messagesActionUrl1}" class="form-control" datatype="*" maxlength="100" ajaxurl="checkUrlAction">
												<hyjf:validmessage key="messagesActionUrl1" label="http地址"></hyjf:validmessage>
											</div>
										</c:when>
										<c:otherwise>
											<div id="actionPanel1" ${(msgPushMessagesForm.msgAction eq '1') ? '' : 'style="display:none;"'}>
											...
											<div id="actionPanel3" ${(msgPushMessagesForm.msgAction eq '3') ? '' : 'style="display:none;"'}>
												<!-- 打开H5页面 -->
												<input type="text" placeholder="请输入http地址（全路径）" id="messagesActionUrl3" name="messagesActionUrl3" value="${msgPushMessagesForm.messagesActionUrl3}" class="form-control" datatype="*" maxlength="100" ajaxurl="checkUrlAction">
												<hyjf:validmessage key="messagesActionUrl3" label="http地址"></hyjf:validmessage>
											</div>
										</c:otherwise>
										</c:choose>
										<div id="actionPanel2" ${(msgPushMessagesForm.msgAction eq '2') ? '' : 'style="display:none;"'}>
											<!-- 指定原生页面 -->
											<span class="symbol"></span><select id="messagesActionUrl2" name="messagesActionUrl2" class="form-select2" style="width: 70%" datatype="*" data-placeholder="请选择项原生页面...">
												<option value=""></option>
												<option value="hyjf://jumpInvest" <c:if test="${'hyjf://jumpInvest' eq msgPushNoticesForm.messagesActionUrl2}">selected="selected"</c:if>>财富汇</option>
												<option value="hyjf://jumpXSH" <c:if test="${'hyjf://jumpXSH' eq msgPushNoticesForm.messagesActionUrl2}">selected="selected"</c:if>>新手汇</option>
												<option value="hyjf://jumpZXH" <c:if test="${'hyjf://jumpZXH' eq msgPushNoticesForm.messagesActionUrl2}">selected="selected"</c:if>>尊享汇</option>
												<option value="hyjf://jumpMine" <c:if test="${'hyjf://jumpMine' eq msgPushNoticesForm.messagesActionUrl2}">selected="selected"</c:if>>我的账户</option>
												<option value="hyjf://jumpCouponsList" <c:if test="${'hyjf://jumpCouponsList' eq msgPushNoticesForm.messagesActionUrl2}">selected="selected"</c:if>>优惠券列表</option>
												<option value="hyjf://jumpTransactionDetail" <c:if test="${'hyjf://jumpTransactionDetail' eq msgPushNoticesForm.messagesActionUrl2}">selected="selected"</c:if>>交易记录</option>
												<option value="hyjf://jumpTransfer" <c:if test="${'hyjf://jumpTransfer' eq msgPushNoticesForm.messagesActionUrl2}">selected="selected"</c:if>>债券转让-已承接</option>
												<option value="hyjf://jumpTransferRecord" <c:if test="${'hyjf://jumpTransferRecord' eq msgPushNoticesForm.messagesActionUrl2}">selected="selected"</c:if>>债权转让-转让记录</option>
											</select>
											<hyjf:validmessage key="messagesActionUrl2" label="原生页面"></hyjf:validmessage>
										</div>
									</div>
								</div>
								<div class="form-group">
									...
									<div class="col-xs-8">
										<select id="msgSendType" name="msgSendType" class="form-select2" style="width: 70%" datatype="*" data-placeholder="请选择发送时间...">
											<option value="0" <c:if test="${msgPushMessagesForm.msgSendType == 0}">selected="selected"</c:if>>立即发送</option>
											<option value="1" <c:if test="${msgPushMessagesForm.msgSendType == 1}">selected="selected"</c:if>>定时发送</option>
										</select>
										<hyjf:validmessage key="msgSendType" label="发送时间"></hyjf:validmessage>
										<div id="msgSendTypePanel0" ${(msgPushMessagesForm.msgSendType ==1) ? '' : 'style="display:none;"'}>
											<!-- 立即发送 -->
										</div>
										<div id="msgSendTypePanel1" ${(msgPushMessagesForm.msgSendType==0) ? '' : 'style="display:none;"'}>
											<!-- 定时发送 -->
											<span class="symbol"></span><input type="text" name="messagesPreSendTimeStr" id="messagesPreSendTimeStr" class="form-control underline" value="${msgPushMessagesForm.messagesPreSendTimeStr}" maxlength="19" datatype="*1-19" nullmsg="请填写定时发送时间" onclick="WdatePicker({skin:'twoer', dateFmt:'yyyy-MM-dd HH:mm:ss', errDealMode: 1})" />
											<hyjf:validmessage key="messagesPreSendTimeStr" label="定时时间"></hyjf:validmessage>
										</div>
									</div>
								</div>
								<div class="form-group">
									...
									<div class="col-xs-8">
										<textarea name="msgDestination" id="msgDestination" placeholder="请输入用户手机号，多个用户请用英文半角逗号 “,” 隔开" maxlength="50000" cols="50" rows="5" datatype="*" errormsg="用户手机号必填！" ajaxurl="checkMobilesAction">${msgPushMessagesForm.msgDestination }</textarea>
										<hyjf:validmessage key="msgDestination" label="用户"></hyjf:validmessage>
									</div>
								</div>
								<div class="form-group margin-bottom-0">
									<div class="col-xs-offset-2 col-xs-8">
...
.purMargin input {
	...
	<tiles:putAttribute name="pageJavaScript" type="string">
		...
		<script type='text/javascript' src="${webRoot}/jsp/msgpush/messages/messagesInfo.js"></script>
	</tiles:putAttribute>
