...
<shiro:hasPermission name="msgpushmessages:VIEW">
	<tiles:insertTemplate template="/jsp/layout/listLayout.jsp" flush="true">
				...
				<div class="row">
					<div class="col-md-12">
						<div class="search-classic">
							<%-- 功能栏 --%>
							<div class="well">
								<c:set var="jspPrevDsb" value="${msgPushMessagesForm.paginator.firstPage ? ' disabled' : ''}"></c:set>
								<c:set var="jspNextDsb" value="${msgPushMessagesForm.paginator.lastPage ? ' disabled' : ''}"></c:set>
								...
								<shiro:hasPermission name="msgpushmessages:ADD">
							...
							<table id="equiList" class="table table-striped table-bordered table-hover">
								...
								<tbody id="roleTbody">
									<c:choose>
										<c:when test="${empty msgPushMessagesForm.recordList}">
											<tr>
												<td colspan="11">暂时没有数据记录</td>
											</tr>
										</c:when>
										<c:otherwise>
											<c:forEach items="${msgPushMessagesForm.recordList }" var="record" begin="0" step="1" varStatus="status">
												<tr>
													<td class="center"><c:out value="${status.index+((msgPushMessagesForm.paginatorPage - 1) * msgPushMessagesForm.paginator.limit) + 1 }"></c:out></td>
													<td class="center">
													<c:forEach items="${messagesPushTags }" var="tag" begin="0" step="1">
													...
													<td class="center"><c:forEach items="${messagesSendStatus }" var="status" begin="0" step="1">
													...
													<td class="center">
														<div class="visible-md visible-lg hidden-sm hidden-xs">
															<shiro:hasPermission name="msgpushmessages:RESEND">
															...
															<shiro:hasPermission name="msgpushmessages:MODIFY">
															...
															<shiro:hasPermission name="msgpushmessages:INFO">
																<c:if test="${record.msgSendStatus eq 1}">
															...
															<shiro:hasPermission name="msgpushmessages:DELETE">
							...
							<%-- 分页栏 --%>
							<hyjf:paginator id="equiPaginator" hidden="paginator-page" action="init" paginator="${msgPushMessagesForm.paginator}"></hyjf:paginator>
							<br /> <br />
						</div>
					</div>
				</div>
		...
		<tiles:putAttribute name="searchPanels" type="string">
			...
			<input type="hidden" name="paginatorPage" id="paginator-page" value="${msgPushMessagesForm.paginatorPage}" />
			<div class="form-group">
				<label>标签类型</label> <select name="messagesTagIdSrch" class="form-control underline form-select2">
					<option value=""></option>
					<c:forEach items="${messagesPushTags }" var="tags" begin="0" step="1">
						<option value="${tags.id }" <c:if test="${tags.id eq msgPushMessagesForm.messagesTagIdSrch}">selected="selected"</c:if>>
			...
			<div class="form-group">
				<label>标题</label> <input type="text" name="messagesTitleSrch" class="form-control input-sm underline" value="${msgPushMessagesForm.messagesTitleSrch}" maxlength="20" />
			</div>
			<div class="form-group">
				<label>消息编码</label> <input type="text" name="messagesCodeSrch" class="form-control input-sm underline" value="${msgPushMessagesForm.messagesCodeSrch}" maxlength="20" />
			</div>
			<div class="form-group">
				<label>作者</label> <input type="text" name="messagesCreateUserNameSrch" class="form-control input-sm underline" value="${msgPushMessagesForm.messagesCreateUserNameSrch}" maxlength="20" />
			</div>
			<div class="form-group">
				<label>推送平台</label> <select name="messagesTerminalSrch" class="form-control underline form-select2">
					<option value="">全部</option>
					<c:forEach items="${plats }" var="plat" begin="0" step="1">
						<option value="${plat.nameCd }" <c:if test="${plat.nameCd eq msgPushMessagesForm.messagesTerminalSrch}">selected="selected"</c:if>>
							<c:out value="${plat.name }"></c:out>
						</option>
					</c:forEach>
				</select>
			...
			<div class="form-group">
				<label>状态</label> <select name="messagesStatusSrch" class="form-control underline form-select2">
					<option value=""></option>
					<c:forEach items="${messagesSendStatus }" var="status" begin="0" step="1">
						<option value="${status.nameCd }" <c:if test="${status.nameCd eq msgPushMessagesForm.messagesSendStatusSrch}">selected="selected"</c:if>>
							<c:out value="${status.name }"></c:out>
						</option>
					</c:forEach>
				</select>
			...
			<div class="form-group">
				...
				<div class="input-group ">
					<span class="input-icon"> <input type="text" name="startSendTimeSrch" id="startSendTimeSrch" class="form-control underline" value="${msgPushMessagesForm.startSendTimeSrch}" onclick="WdatePicker({skin:'twoer', dateFmt:'yyyy-MM-dd HH:mm:ss', errDealMode: 1})" /> <i class="ti-calendar"></i>
					</span> <span class="input-group-addon no-border bg-light-orange">~</span> <input type="text" name="endSendTimeSrch" id="endSendTimeSrch" class="form-control underline" value="${msgPushMessagesForm.endSendTimeSrch}" onclick="WdatePicker({skin:'twoer', dateFmt:'yyyy-MM-dd HH:mm:ss', errDealMode: 1})" />
				</div>
			</div>
		</tiles:putAttribute>
...
.popover img {
	max-width: 460px;
		...
		<tiles:putAttribute name="pageJavaScript" type="string">
			...
			<script type='text/javascript' src="${webRoot}/jsp/msgpush/messages/messages_list.js"></script>
		</tiles:putAttribute>
