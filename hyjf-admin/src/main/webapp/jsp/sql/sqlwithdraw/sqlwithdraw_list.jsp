...
<%-- 画面功能路径 (ignore) --%>
<c:set value="${fn:split('汇盈金服,数据,提现详情', ',')}" var="functionPaths" scope="request"></c:set>

<%-- <shiro:hasPermission name="employeeinfo:VIEW">		--%>
	<tiles:insertTemplate template="/jsp/layout/listLayout.jsp" flush="true">
		<%-- 画面的标题 --%>
		<tiles:putAttribute name="pageTitle" value="提现详情" />
		<%-- 画面的CSS (ignore) --%>
		...
		<tiles:putAttribute name="pageFunCaption" type="string">
			<h1 class="mainTitle">提现详情</h1>
			<span class="mainDescription">提现详情信息。</span>
		</tiles:putAttribute>
		<%-- 画面主面板 --%>
								...
								<tbody id="roleTbody">
											<c:forEach items="${lists}" var="record" begin="0" step="1" varStatus="status">
												<tr>
													...
													<td><c:out value="${record.total }"></c:out></td>
		...
		<tiles:putAttribute name="pageJavaScript" type="string">
			...
			<script type='text/javascript' src="${webRoot}/jsp/sql/sqlwithdraw/sqlwithdraw_list.js"></script>
