	...
	<tiles:insertTemplate template="/jsp/layout/listLayout.jsp" flush="true">
					...
					<div class="col-md-12">
						<div class="search-classic">
							<shiro:hasPermission name="investdetail:SEARCH">
								<%-- 功能栏 --%>
								<div class="well">
									<c:set var="jspPrevDsb" value="${investForm.paginator.firstPage ? ' disabled' : ''}"></c:set>
									<c:set var="jspNextDsb" value="${investForm.paginator.lastPage ? ' disabled' : ''}"></c:set>
									...
									<c:choose>
										...
										<c:otherwise>
											<c:forEach items="${debtInvestList }" var="record" begin="0" step="1" varStatus="status">
												<tr>
													<td class="center">${(investForm.paginatorPage - 1 ) * investForm.paginator.limit + status.index + 1 }</td>
							...
							<%-- 分页栏 --%>
							<shiro:hasPermission name="investdetail:SEARCH">
								<hyjf:paginator id="equiPaginator" hidden="paginator-page" action="searchAction" paginator="${investForm.paginator}"></hyjf:paginator>
							</shiro:hasPermission>
							<br/><br/>
						</div>
					</div>
		...
		<tiles:putAttribute name="searchPanels" type="string">
			<shiro:hasPermission name="loandetail:SEARCH">
				<input type="hidden" name="paginatorPage" id="paginator-page" value="${investForm.paginatorPage}" />
				
					<!-- 检索条件 -->
					<div class="form-group">
						<label>智投编号</label>
						<input type="text" name="planNidSrch" id="planNidSrch" class="form-control input-sm underline" value="${investForm.planNidSrch}" />
					</div>
					<div class="form-group">
					<label>智投订单号:</label>
					<input type="text" name="planOrderId" id="planOrderId" class="form-control input-sm underline" value="${investForm.planOrderId}" />
					</div>
					<div class="form-group">
					<label>出借/承接订单号:</label>
					<input type="text" name="orderId" id="orderId" class="form-control input-sm underline" value="${investForm.orderId}" />
					</div>
					<div class="form-group">
					<label>用户名</label>
					<input type="text" name="userName" id="userName" class="form-control input-sm underline" value="${investForm.userName}" />
					</div>
					<div class="form-group">
						<label>项目编号</label>
						<input type="text" name="borrowNidSrch" id="borrowNidSrch" class="form-control input-sm underline" value="${investForm.borrowNidSrch}" />
					</div>
					<div class="form-group">
					<label>项目类型</label>
					<select name="projectTypeSrch" class="form-control underline form-select2">
						...
						<c:forEach items="${borrowProjectTypeList }" var="borrowProjectType" begin="0" step="1" varStatus="status">
							<option value="${borrowProjectType.borrowCd }"
								<c:if test="${borrowProjectType.borrowCd eq investForm.projectTypeSrch}">selected="selected"</c:if>>
					...
					<div class="input-group input-daterange datepicker">
						<span class="input-icon">
							<input type="text" name="investTimeStart" id="investTimeStart" class="form-control underline" value="${investForm.investTimeStart}" />
							<i class="ti-calendar"></i>
						</span>
						<span class="input-group-addon no-border bg-light-orange">~</span>
						<span class="input-icon">
							<input type="text" name="investTimeEnd" id="investTimeEnd" class="form-control underline" value="${investForm.investTimeEnd}" />
						</span>
					</div>
					</div>
			</shiro:hasPermission>
		...
		<tiles:putAttribute name="pageJavaScript" type="string">
			<script type='text/javascript' src="${webRoot}/jsp/htj/investdetail/investDetailList.js"></script>
		</tiles:putAttribute>
