	...
	<tiles:putAttribute name="mainContentinner" type="string">
		...
		<div class="panel panel-white" style="margin:0">
			<div class="panel-heading">
				<h5 class="panel-title">${jspEditType}菜单</h5>
			</div>
			<div class="panel-body">
				...
				<form id="mainForm" action="${empty adminForm.id ? 'insertAction' : 'updateAction'}"
							...
							<c:choose>
								<c:when test="${empty adminForm.id}">
									<input type="text" placeholder="用户名" id="username" name="username" value="${adminForm.username}"  class="form-control"
										datatype="s2-20" errormsg="用户名 只能是数字、字母和汉字，长度2~20个字符！" >
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<input type="text" placeholder="姓名" id="truename" name="truename" value="${adminForm.truename}"  class="form-control"
								datatype="/^[\u4E00-\u9FA5\uf900-\ufa2d]{2,10}$/" errormsg="姓名 只能是汉字，长度2~10个字符！" >
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<input type="email" placeholder="邮箱" id="email" name="email" value="${adminForm.email}"  class="form-control"
								ignore="ignore" datatype="* , e" errormsg="邮箱地址格式不对！" >
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<input type="text" placeholder="手机号码" id="mobile" name="mobile" value="${adminForm.mobile}"  class="form-control" 
								datatype="n11-11" errormsg="手机号码 只能是11位数字!" >
							<hyjf:validmessage key="mobile" label="手机号码"></hyjf:validmessage>
						</div>
					</div>
					<div class="form-group">
						<label class="col-sm-2 control-label" for="groupId"> <span class="symbol required"></span>角色 </label>

						<div class="col-sm-10 admin-select">
							<select id="groupId" name="groupId" class="form-control " datatype="*" errormsg="请选择角色！" >
								<option value="" ></option>
								<c:if test="${!empty adminForm.adminRoleList}">
									<c:forEach items="${adminForm.adminRoleList }" var="role" begin="0" step="1" varStatus="status">
										<c:choose>
											<c:when test="${adminForm.groupId == role.id }">
												<option value="${role.id }" data-class="fa" selected="selected">${role.groupName }</option>
											</c:when>
											<c:otherwise>
												<option value="${role.id }" data-class="fa">${role.groupName }</option>
											</c:otherwise>
										</c:choose>
									</c:forEach>
								</c:if>
					...
					<div class="form-group">
						<label class="col-sm-2 control-label" > <span class="symbol required"></span>菜单状态 </label>
						<div class="col-sm-10 clip-radio radio-primary">
							...
							<hyjf:validmessage key="state" label="菜单状态"></hyjf:validmessage>
						</div>
					</div>
