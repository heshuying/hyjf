...
Events = {
	...
	resetPwdClkAct : function(event) {
		if(event) {
			Page.primaryKey.val(JSON.stringify([$(this).data("id")]))
		}
		Page.confirm("", "确定要执行本次重置密码操作吗？", function(isConfirm) {
			//alert(Action.resetPwdAction);
			isConfirm && Page.submit(Action.resetPwdAction);
		})
	},
	// 刷新按钮的单击动作事件
...
$.extend(Page, {
	...
	initEvents : function() {
		...
		// 边界面板查询按钮的单击事件绑定
		$(".fn-ResetPwd").click(Events.resetPwdClkAct);
	}
});
