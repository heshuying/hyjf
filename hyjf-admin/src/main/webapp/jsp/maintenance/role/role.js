...
Events = {
	...
	authClkAct: function(event) {
		...
		$.colorbox({
			overlayClose: false,
			href: "#urlDialogPanel",
			title: "<i class=\"fa fa-plus\"></i> 权限设置",
			width: "50%", height: 500,
			inline: true,  fixed: true, returnFocus: false, open: true,
			// Open事件回调
			onOpen: function() {
				setTimeout(function() {
					Page.form.attr("target", "dialogIfm").attr("action", Action.authAction).submit();
				}, 0)
			},
			// Close事件回调
			onClosed: function() {
	...
	modifyClkAct: function(event) {
		...
		$.colorbox({
			overlayClose: false,
			href: "#urlDialogPanel",
			title: "<i class=\"fa fa-plus\"></i> 修改角色",
			width: "50%", height: 430,
			inline: true,  fixed: true, returnFocus: false, open: true,
			// Open事件回调
			onOpen: function() {
...
$.extend(Page, {
	...
	initEvents : function() {
		...
		// 授权按钮的单击事件绑定
		$(".fn-Authorize").click(Events.authClkAct),
		// 修改按钮的单击事件绑定
