...
<shiro:hasPermission name="certlog:VIEW" >
	<tiles:insertTemplate template="/jsp/layout/listLayout.jsp" flush="true">
		<%-- 画面的标题 --%>
		<tiles:putAttribute name="pageTitle" value="应急中心发送日志" />
				...
				<div class="row">
					<div class="col-md-12">
						<div class="search-classic">
							<shiro:hasPermission name="certlog:SEARCH">
								<%-- 功能栏 --%>
								<div class="well">
										...
										<c:otherwise>
 											<c:forEach items="${form.recordList}" var="record" begin="0" step="1" varStatus="status">
												<tr>
													...
													<td class="center">
														<c:if test="${record.queryResult==0}">
															<c:out value="初始"></c:out>
														</c:if>
														<c:if test="${record.queryResult==1}">
															<c:out value="成功"></c:out>
														</c:if>
														<c:if test="${record.queryResult==2}">
															<c:out value="等待处理"></c:out>
														</c:if>
														<c:if test="${record.queryResult==8}">
															<c:out value="批次号无效"></c:out>
														</c:if>
														<c:if test="${record.queryResult==9}">
															<c:out value="失败"></c:out>
														</c:if>
														<c:if test="${record.queryResult==99}">
															<c:out value="无响应"></c:out>
														</c:if>
													</td>
		...
		<tiles:putAttribute name="searchPanels" type="string">
			...
			<div class="form-group">
				<label>对账状态</label>
				<select name="queryStatus" class="form-control underline form-select2">
					<option value="-1"></option>
					<option value="0" <c:if test="${'0' eq form.queryStatus}">selected="selected"</c:if>>初始</option>
					<option value="1" <c:if test="${'1' eq form.queryStatus}">selected="selected"</c:if>>成功</option>
					<option value="2" <c:if test="${'2' eq form.queryStatus}">selected="selected"</c:if>>等待处理</option>
					<option value="8" <c:if test="${'8' eq form.queryStatus}">selected="selected"</c:if>>批次号无效</option>
					<option value="9" <c:if test="${'9' eq form.queryStatus}">selected="selected"</c:if>>失败</option>
					<option value="99" <c:if test="${'99' eq form.queryStatus}">selected="selected"</c:if>>无响应</option>
				</select>
			</div>
		...
		<tiles:putAttribute name="pageJavaScript" type="string">
			<script type='text/javascript' src="${webRoot}/jsp/datacenter/certreportlog/certreportlog.js"></script>
		</tiles:putAttribute>
