...
Events = {
	...
	addClkAct : function() {
		$.colorbox({
			overlayClose: false,
			href: "#urlDialogPanel",
			title: "<i class=\"fa fa-plus\"></i> 添加广告",
	        ...
	        onOpen: function() {
	        	setTimeout(function() {
	        		Page.primaryKey.val(""),
					Page.platformType.val($("#platformType").val()),
	...
	modifyClkAct: function(event) {
		...
		$.colorbox({
			overlayClose: false,
			href: "#urlDialogPanel",
			title: "<i class=\"fa fa-plus\"></i> 修改广告",
			width: 650, height: 430,
			inline: true,  fixed: true, returnFocus: false, open: true,
			// Open事件回调
			onOpen: function() {
    ...
    setClickVal:function (event) {
        if(event) {
            $(".nav-tabs li").removeAttr("class");
            $(this).parent().attr("class","active");
            Page.platformType.val($(this).data("type"));
	...
	deleteClkAct : function(event) {
		if(event) {
			...
			Page.platformType.val(JSON.stringify($(this).data("type")));
		}
...
$.extend(Page, {
	...
	initEvents : function() {
        ...
        $(".nav-tabs li a").click(Events.setClickVal );
	}
});
