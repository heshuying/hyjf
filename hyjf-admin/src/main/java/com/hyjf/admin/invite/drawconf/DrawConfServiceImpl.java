...
@Service
public class DrawConfServiceImpl extends BaseServiceImpl implements DrawConfService {
     ...
     */
	@Override
    public List<PrizeGetCustomize> getRecordList(DrawConfBean bean, int limitStart, int limitEnd) {
    ...
    public int insertRecord(InvitePrizeConf prize){
	    ...
	    prize.setPrizeKind(Integer.parseInt(CustomConstants.CONF_PRIZE_KIND_DRAW));
	    if(prize.getPrizeQuantity() == null){
	        prize.setPrizeReminderQuantity(null);
	    }else{
     ...
     */
    @Override
    public int updatePrizeConfig(DrawConfBean prize){
        AdminSystem adminSystem = (AdminSystem) SessionUtils.getSession(CustomConstants.LOGIN_USER_INFO);
