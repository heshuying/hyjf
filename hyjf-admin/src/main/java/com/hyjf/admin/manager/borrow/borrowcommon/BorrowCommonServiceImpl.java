...
public class BorrowCommonServiceImpl extends BaseServiceImpl implements BorrowCommonService{
	 ...
	 */
	@Override
	public synchronized void insertRecord(BorrowCommonBean borrowBean) throws Exception {
		...
		if (borrowCommonNameAccountList != null && borrowCommonNameAccountList.size() > 0) {
			...
			for (int i = 0; i < borrowCommonNameAccountList.size(); i++) {
				...
				if (StringUtils.isNotEmpty(account)) {
					...
					// 借款表插入
					this.insertBorrowCommonData(borrowBean, borrow, borrowPreNid, borrowPreNidNew, borrowNid, account);
					// 删写redis的定时发标时间
                    ...
                    borrowLog.setType(BorrowCommonDefine.BORROW_LOG_ADD);
                    borrowLog.setCreateTime(GetDate.getMyTimeInMillis());
                    AdminSystem userAdminSystem = ShiroUtil.getLoginUserInfo();
                    borrowLog.setCreateUserId(StringUtil.isBlank(userAdminSystem.getId())?0:Integer.valueOf(userAdminSystem.getId()));
                    borrowLog.setCreateUserName(userAdminSystem.getUsername());
                    borrowLog.setDelFlg(0);
...
//		borrow.setApplicant(borrowBean.getApplicant().trim());
		...
		borrow.setCreateUserName(adminSystem.getUsername());
		...
		// 产品类型
		borrow.setAssetType(Integer.parseInt(borrowBean.getProjectType()));
		// 根据资产机构编号,产品类型检索产品名称
		...
		// 项目类型
		borrow.setProjectType(Integer.valueOf(borrowBean.getProjectType()));
		...
		// 根据项目编号查询项目类型配置
		BorrowProjectType borrowProjectType = this.getBrrowProjectTpyeByProjectType(borrowBean.getProjectType());
		if(borrowProjectType.getIncreaseInterestFlag()==1&&!StringUtils.isEmpty(borrowBean.getBorrowExtraYield())&&new BigDecimal(borrowBean.getBorrowExtraYield()).compareTo(BigDecimal.ZERO)>0){
			...
			// 产品加息率
			borrow.setBorrowExtraYield(new BigDecimal(borrowBean.getBorrowExtraYield()));
		}else{
			// 是否加息
			...
			if(!StringUtils.isEmpty(borrowBean.getBorrowExtraYield())){
			    ...
			    borrow.setBorrowExtraYield(new BigDecimal(0));
			}
		}
	...
	public void modifyRecord(BorrowCommonBean borrowBean) throws Exception {
		...
		if (StringUtils.isNotEmpty(borrowNid)) {
			...
			if (borrowList != null && borrowList.size() == 1) {
				...
				borrowCra.andBorrowPreNidNewEqualTo(record.getBorrowPreNidNew());
				...
				if (borrowAllList != null && borrowAllList.size() > 0) {
					for (BorrowWithBLOBs borrow : borrowAllList) {
						...
						if (borrowBean.getVerifyStatus() != null && StringUtils.isNotEmpty(borrowBean.getVerifyStatus())) {
							...
							if (Integer.valueOf(borrowBean.getVerifyStatus()) == 4) {
								if (borrow.getIsEngineUsed().equals(1)) {
									// 成功后到关联计划队列
									this.sendToMQ(borrowBean, RabbitMQConstants.ROUTINGKEY_BORROW_ISSUE);
						...
						borrowLog.setBorrowStatus(statusNameString);
						borrowLog.setBorrowStatusCd(StringUtil.isBlank(borrowBean.getStatus())?0:Integer.valueOf(borrowBean.getStatus()));
						borrowLog.setType(BorrowCommonDefine.BORROW_LOG_UPDATE);
						borrowLog.setCreateTime(GetDate.getMyTimeInMillis());
						AdminSystem userAdminSystem = ShiroUtil.getLoginUserInfo();
						borrowLog.setCreateUserId(StringUtil.isBlank(userAdminSystem.getId())?0:Integer.valueOf(userAdminSystem.getId()));
						borrowLog.setCreateUserName(userAdminSystem.getUsername());
	 ...
	 */
	@Override
	public void updateBorrowCommonData(BorrowCommonBean borrowBean, BorrowWithBLOBs borrow, String borrowMainNid) throws Exception {
...
//		}
		...
		if (status == 0 && borrowMainNid.equals(borrowNid)) {
            ...
            // 根据项目编号查询项目类型配置
            BorrowProjectType borrowProjectType = this.getBrrowProjectTpyeByProjectType(borrowBean.getProjectType());
            if (borrowProjectType.getIncreaseInterestFlag() == 1
                    && !StringUtils.isEmpty(borrowBean.getBorrowExtraYield())
                    && new BigDecimal(borrowBean.getBorrowExtraYield()).compareTo(BigDecimal.ZERO) > 0) {
                // 是否加息
                borrow.setIncreaseInterestFlag(1);
                borrow.setBorrowExtraYield(new BigDecimal(borrowBean.getBorrowExtraYield()));
            } else {
		...
		if ("BORROW_FIRST".equals(borrowBean.getMoveFlag()) && borrowMainNid.equals(borrowNid)) {
			...
			// 初审人员ID
			borrow.setVerifyUserid(adminSystem.getId());
			// 初审人员用户名
			borrow.setVerifyUserName(adminSystem.getUsername());
			// 初审备注
	...
	public int insertBorrowUsers(String borrowNid, BorrowCommonBean borrowBean, BorrowWithBLOBs borrow) {
		...
		if (StringUtils.equals("1", borrowBean.getCompanyOrPersonal())) {
			...
			//20180705 add by NXL 添加企业组织机构代码和企业注册地
	...
	public void getBorrowCommonFiled(BorrowCommonBean borrowBean, BorrowWithBLOBs borrowWithBLOBs) {
		...
		// 产品加息标识
		borrowBean.setIncreaseInterestFlag(this.getValue(String.valueOf(borrowWithBLOBs.getIncreaseInterestFlag())));
		// 最低投标金额
		...
		if (StringUtils.isNotEmpty(borrowWithBLOBs.getFiles())) {
			...
			if (borrowCommonFileList != null && borrowCommonFileList.size() > 0) {
				...
				Collections.sort(borrowCommonImageList, new Comparator<BorrowCommonImage>() {
					@Override
					public int compare(BorrowCommonImage o1, BorrowCommonImage o2) {
						if (o1 != null && o2 != null) {
		...
		// 标的投资风险测评类型
		borrowBean.setInvestLevel(this.getValue(borrowWithBLOBs.getInvestLevel()));
		// 借款人信息数据获取
	...
	public void getBorrowCarinfo(BorrowCommonBean borrowBean) {
		...
		List<BorrowCommonCar> borrowCarList = new ArrayList<BorrowCommonCar>();
		if (borrowCarinfoList != null && borrowCarinfoList.size() > 0) {
			for (BorrowCarinfo record : borrowCarinfoList) {
				BorrowCommonCar borrowCar = new BorrowCommonCar();
				// 车辆信息 品牌
		...
		if (borrowHousesList != null && borrowHousesList.size() > 0) {
			borrowBean.setBorrowHousesList(borrowHousesList);
		}
		...
		List<BorrowCommonCompanyAuthen> borrowCommonCompanyAuthenList = new ArrayList<BorrowCommonCompanyAuthen>();
		if (borrowCompanyAuthenList != null && borrowCompanyAuthenList.size() > 0) {
			borrowBean.setBorrowCompanyAuthenList(borrowCompanyAuthenList);
			for (BorrowCompanyAuthen borrowCompanyAuthen : borrowCompanyAuthenList) {
				BorrowCommonCompanyAuthen borrowCommonCompanyAuthen = new BorrowCommonCompanyAuthen();
	...
	public void getBorrowUsers(BorrowCommonBean borrowBean) {
		...
		cra.andBorrowNidEqualTo(borrowBean.getBorrowNid());
		List<BorrowUsers> borrowUsersList = this.borrowUsersMapper.selectByExampleWithBLOBs(example);

		if (borrowUsersList != null && borrowUsersList.size() > 0) {
			for (BorrowUsers record : borrowUsersList) {
				// 用户信息 用户名称
	...
	public int isExistsUser(String userId) {
		if (StringUtils.isNotEmpty(userId)) {
            ...
            // 判断是否服务费授权 和还款授权
            HjhUserAuth auth = autoPlusService.getHjhUserAuthByUserId(users.getUserId());
            
            return CommonUtils.checkAuthStatus(auth==null?null:auth.getAutoRepayStatus(), users.getPaymentAuthStatus());
		}
		return 1;
	}
	...
	public String isExistsUser(HttpServletRequest request) {
		...
		if (usersFlag == 1) {
		    ...
		    // 服务费授权
            message = ValidatorFieldCheckUtil.getErrorMessage("auth.payment");
            ret.put(AdminDefine.JSON_VALID_INFO_KEY, message);
            return ret.toString();
        } else if (usersFlag == 6) {
            // 还款授权
            message = ValidatorFieldCheckUtil.getErrorMessage("auth.repay");
            ret.put(AdminDefine.JSON_VALID_INFO_KEY, message);
            return ret.toString();
        } else if (usersFlag == 7) {
            // 还款授权和服务费授权
            message = ValidatorFieldCheckUtil.getErrorMessage("auth.repay.payment");
            ret.put(AdminDefine.JSON_VALID_INFO_KEY, message);
            return ret.toString();
        }
	...
	public String isRepayOrgUser(HttpServletRequest request) {
		...
		cri.andBankOpenAccountEqualTo(1);// 江西银行已开户
		...
		// 判断是否服务费授权 和还款授权
        Integer authState = CommonUtils.checkPaymentAuthStatus(ulist.get(0).getPaymentAuthStatus());
		
        if (authState == 0) {
            // 服务费授权
            ret.put(AdminDefine.JSON_VALID_INFO_KEY, "未开通服务费授权！！");
            return ret.toString();
        } 
		ret.put(AdminDefine.JSON_VALID_STATUS_KEY, AdminDefine.JSON_VALID_STATUS_OK);
		return ret.toJSONString();
     ...
     */
	@Override
	public String getBorrowServiceScale(String projectType, String borrowStyle, String instCode, Integer borrowPeriod) {
		String serviceRate = "";
	...
	public String getUploadImage(BorrowCommonBean borrowBean, String files, String borrowNid) throws Exception {
		...
		if (borrowCommonImageList != null && borrowCommonImageList.size() > 0) {
			...
			// 正式保存的路径
			String fileUploadRealPath = UploadFileUtils.getDoPath(PropUtils.getSystem("file.upload.real.path")) + UploadFileUtils.getDoPath(borrowNid);
			...
			for (BorrowCommonImage borrowCommonImage : borrowCommonImageList) {
	...
	public void validatorFieldCheck(ModelAndView mav, BorrowCommonBean borrowBean, boolean isExistsRecord, String HztOrHxf) {
		...
		// 判断担保机构是否服务费授权 和还款授权 start
		if(borrowBean.getRepayOrgUserId()!=null){
		    Users dfUser = autoPlusService.getUsers(borrowBean.getRepayOrgUserId());
	        HjhUserAuth auth = autoPlusService.getHjhUserAuthByUserId(dfUser.getUserId());
	        Integer authState = CommonUtils.checkAuthStatus(auth==null?null:auth.getAutoRepayStatus(), dfUser.getPaymentAuthStatus());
	        if (authState == 5) {
	            // 服务费授权
	            ValidatorFieldCheckUtil.validateSpecialError(mav, "repayOrgName", "auth.payment");
	        } else if (authState == 6) {
	            // 还款授权
	            ValidatorFieldCheckUtil.validateSpecialError(mav, "repayOrgName", "auth.repay");
	        } else if (authState == 7) {
	            // 还款授权和服务费授权
	            ValidatorFieldCheckUtil.validateSpecialError(mav, "repayOrgName", "auth.repay.payment");
	        }
		}
		
        // 判断担保机构是否服务费授权 和还款授权 end
		...
		if(borrowBean.getEntrustedFlg().equals("1")){
			...
			} else if (entrustedUsersFlg == 7) {
			    // 未服务费授权
                ValidatorFieldCheckUtil.validateSpecialError(mav, "entrustedUsername", "auth.payment");
            }
		}
		...
		if ("BORROW_FIRST".equals(borrowBean.getMoveFlag())) {
			...
			/*-------------upd by liushouyi HJH3 Start-----------------------*/
			//使用引擎时是否有关联计划
			if ("1".equals(borrowBean.getIsEngineUsed())) {
				BorrowWithBLOBs borrow = this.getBorrowByNid(borrowBean.getBorrowNid());
				HjhLabel label = autoIssueService.getLabelId(borrow,null);
				if(label == null || label.getId() == null){
					ValidatorFieldCheckUtil.validateSpecialError(mav, "isEngineUsedErr", "engine.not.exists");
				}
			}
			/*-------------upd by liushouyi HJH3 Start-----------------------*/
		}
		...
		if (CustomConstants.HZT.equals(HztOrHxf)) {
			// 汇盈资产手动录标 upd by liushouyi HJH3
			if ("10000000".equals(borrowBean.getInstCode())) {
				// 放款服务费(放款时收)
				...
				BorrowFinmanNewCharge borrowFinmanNewCharge = this.getBorrowFinmanNewCharge(borrowBean);
				if (null != borrowFinmanNewCharge) {
					borrowBean.setBorrowServiceScale(borrowFinmanNewCharge.getChargeRate());
					borrowBean.setBorrowManagerScale(borrowFinmanNewCharge.getManChargeRate());
				} else {
					ValidatorFieldCheckUtil.validateSpecialError(mav, "borrowServiceScale", "notget.borrowservicescale");
					ValidatorFieldCheckUtil.validateSpecialError(mav, "borrowManagerScale", "notget.borrowmanagerscale");
				}
	...
	public void setPageListInfo(ModelAndView modelAndView, BorrowCommonBean form, boolean isExistsRecord) {
		...
		// 项目名称 & 借款金额
		if (!isExistsRecord && "yes".equals(isChaibiao)) {
			borrowCommonNameAccountList = JSONArray.parseArray(form.getBorrowNameJson(), BorrowCommonNameAccount.class);
		}
		form.setBorrowCommonNameAccountList(borrowCommonNameAccountList);
		// 车辆信息
		if (StringUtils.equals("2", form.getTypeCar())) {
			List<BorrowCommonCar> borrowCarinfoList = JSONArray.parseArray(form.getBorrowCarJson(), BorrowCommonCar.class);
			form.setBorrowCarinfoList(borrowCarinfoList);
		}
		// 房产信息
		if (StringUtils.equals("1", form.getTypeHouse())) {
			List<BorrowHouses> borrowHousesList = JSONArray.parseArray(form.getBorrowHousesJson(), BorrowHouses.class);
			form.setBorrowHousesList(borrowHousesList);
		}
		// 认证信息
		List<BorrowCommonCompanyAuthen> borrowCommonCompanyAuthenList = JSONArray.parseArray(form.getBorrowAuthenJson(), BorrowCommonCompanyAuthen.class);
		form.setBorrowCommonCompanyAuthenList(borrowCommonCompanyAuthenList);
		// 项目资料
		List<BorrowCommonImage> borrowCommonImageList = JSONArray.parseArray(form.getBorrowImageJson(), BorrowCommonImage.class);
		String fileDomainUrl = UploadFileUtils.getDoPath(PropUtils.getSystem("file.domain.url"));
		for (BorrowCommonImage borrowCommonImage : borrowCommonImageList) {
	...
	public String getBorrowServiceScale(HttpServletRequest request) {
        ...
        if (!GenericValidator.isLong(borrowPeriod) || !NumberUtils.isNumber(borrowPeriod) || Integer.valueOf(borrowPeriod) < 0 || StringUtils.isEmpty(borrowStyle) || StringUtils.isEmpty(projectType)
                || StringUtils.isEmpty(instCode)) {
...
//							resultMap.put("comRegTime", this.getValue(hssfRow.getCell(1)));
							if(GetDate.isYYYYMMDD(this.getValue(hssfRow.getCell(1)))){
								resultMap.put("comRegTime", this.getValue(hssfRow.getCell(1)));
							}
	 ...
	 * @return
	 */
	private String getAssetTypeName(String instCode, String productType) {
		...
		cra.andAssetTypeEqualTo(Integer.parseInt(productType));
	...
	public int isEntrustedExistsUser(String userName) {
		if (StringUtils.isNotEmpty(userName)) {
	        ...
	        // 检查是否服务费授权
	        Integer isPaymentAuth = CommonUtils.checkPaymentAuthStatus(users.getPaymentAuthStatus());
	 ...
	 * @return
	 */
    public String getLateInterestRate(String projectType, String borrowStyle, String instCode,Integer borrowPeriod) {
	 ...
	 * @return
	 */
    public String getLateFreeDays(String projectType, String borrowStyle, String instCode,Integer borrowPeriod) {
        String lateFreeDays = "";
        Map<String,Object> params = new HashMap<String,Object>();
        params.put("borrowStyle",borrowStyle);
        params.put("borrowPeriod",borrowPeriod);
    ...
    private BorrowFinmanNewCharge getBorrowFinmanNewCharge(BorrowCommonBean borrowBean){
    	// 获取管理费率，服务费率，自动发标费率
		Borrow borrow = this.getBorrowByNid(borrowBean.getBorrowNid());
    ...
    public void isAutoRecord(String borrowPreNid) {
		...
		if (list.isEmpty() || null == list) {
			_log.error("判断是否自动备案发送MQ拉取标的信息失败！");
			return;
		}
		...
		if (null != hjhAssetBorrowType && null != hjhAssetBorrowType.getAutoRecord() && hjhAssetBorrowType.getAutoRecord() == 1) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				_log.info("标的编号：" + list.get(i).getBorrowNid()+ " 已发送到自动备案消息队列！");
			}
		}
