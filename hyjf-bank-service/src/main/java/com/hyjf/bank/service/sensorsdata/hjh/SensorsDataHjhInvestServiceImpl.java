...
/**
 * 神策数据统计:汇计划相关Service实现类
 *
 * @author liuyang
 * @version SensorsDataHjhInvestServiceImpl, v0.1 2018/7/21 18:28
 */
@Service
public class SensorsDataHjhInvestServiceImpl extends BaseServiceImpl implements SensorsDataHjhInvestService {

    Logger _log = LoggerFactory.getLogger(SensorsDataHjhInvestServiceImpl.class);
    ...
    public void sendSensorsData(SensorsDataBean sensorsDataBean) throws IOException, InvalidArgumentException {
        ...
        // 根据加入订单号查询计划加入订单
        HjhAccede hjhAccede = this.selectHjhAccedeByOrderId(orderId);

        if (hjhAccede == null) {
            _log.info("根据加入订单号查询计划加入订单不存在,计划加入订单号:[" + orderId + "].");
            return;
        }
        ...
        if (hjhPlan == null) {
            _log.info("根据计划编号查询计划信息不存在,计划加入订单号:[" + orderId + "],计划编号:[" + planNid + "].");
            return;
        }
        ...
        if (couponRealTender != null) {
            ...
            _log.info("此笔加入使用了优惠券:优惠券出借ID:[" + couponTenderId + "],计划加入订单号:[" + orderId + "].");
            ...
            try {
                ...
                e.printStackTrace();
                _log.info("计算优惠券的利息错误,订单号:" + orderId + "].");
            }
     ...
     * @return
     */
    private HjhAccede selectHjhAccedeByOrderId(String orderId) {
