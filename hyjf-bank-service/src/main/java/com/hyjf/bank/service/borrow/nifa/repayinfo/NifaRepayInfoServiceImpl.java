...
/**
 * @author PC-LIUSHOUYI
 * @version NifaRepayInfoServiceImpl, v0.1 2018/7/11 13:57
 */
@Service
public class NifaRepayInfoServiceImpl extends BaseServiceImpl implements NifaRepayInfoService {

    Logger _log = LoggerFactory.getLogger(NifaRepayInfoServiceImpl.class);
    ...
    public boolean insertNifaRepayInfo(String borrowNid, Integer repayPeriod) {
        ...
        if (null == borrow) {
            _log.error(thisMessName + "未查询到相应的借款详情数据！！borrowNid:" + borrowNid + " repayPeriod:" + repayPeriod);
            return false;
        }
        ...
        if (null == borrowRepay) {
            _log.error(thisMessName + "未查询到相应的还款信息！！borrowNid:" + borrowNid + " repayPeriod:" + repayPeriod);
            return false;
        }

        NifaRepayInfo nifaRepayInfo = new NifaRepayInfo();
        // 统一社会信用代码
        nifaRepayInfo.setPlatformNo(comSocialCreditCode);
        // 项目编号
        ...
        if (Validator.isNumber(borrowRepay.getRepayActionTime())) {
            try {
                nifaRepayInfo.setPaymentDate(GetDate.getDateMyTimeInMillis(Integer.parseInt(borrowRepay.getRepayActionTime())));
        ...
        if (isMonth) {
            ...
            if (null == borrowRepayPlanList) {
                _log.error(thisMessName + "未获取到放款记录详情，borrowNid:" + borrowNid + " repayPeriod:" + repayPeriod);
                return false;
            }
            // 判断是否提前、逾期还款1：提前3：逾期
        ...
        List<NifaRepayInfo> nifaRepayInfoList = this.nifaRepayInfoMapper.selectByExample(example);
    ...
    public boolean insertNifaContractStatus(String borrowNid, Integer repayPeriod) {
        ...
        for (BorrowTender borrowTender : borrowTenderList) {
            ...
            if (null == borrowRecover) {
                _log.error(thisMessName + "放款数据获取失败，borrowNid:" + borrowRepay.getBorrowNid() + "订单号：" + borrowTender.getNid());
                continue;
            }
            ...
            if (null == nifaContractStatusOld) {
                // 插入数据
                if (!insertNifaContractStatusRecorde(thisMessName, comSocialCreditCode, borrowNid, contractStatus, borrowTender, borrowRepay)) {
    ...
    public boolean insertNifaReceivedPayments(String borrowNid, Integer repayPeriod) {
        ...
        Borrow borrow = this.getBorrowByBorrowNid(borrowNid);
        if (null == borrow) {
            _log.error(thisMessName + "未获取到借款详细信息，borrowNid:" + borrowNid + " repayPeriod:" + repayPeriod);
            return false;
        }
        ...
        BorrowRepay borrowRepay = this.selectBorrowRepay(borrowNid);
        if (null == borrowRepay) {
            _log.error(thisMessName + "未获取到标的还款计划，borrowNid:" + borrowNid + " repayPeriod:" + repayPeriod);
            return false;
        }
        ...
        if (isMonth) {
            ...
            for (BorrowRecoverPlan borrowRecoverPlan : borrowRecoverPlanlist) {
                ...
                if (null == borrowRecoverList || borrowRecoverList.size() <= 0) {
                    _log.error(thisMessName + "未获取到用户放款信息，borrowNid:" + borrowNid + " repayPeriod:" + repayPeriod);
                    return false;
                }
                ...
                // 统一社会信用代码
                nifaReceivedPayments.setPlatformNo(comSocialCreditCode);
                // 项目编号
                ...
                if (Validator.isNumber(borrowRecoverPlan.getRecoverYestime())) {
                    try {
                        ...
                        _log.error(thisMessName + "还款日格式化失败，borrowNid:" + borrowNid + " repayPeriod:" + repayPeriod);
                        e.printStackTrace();
                        return false;
                    }
                } else {
                ...
                if (!result) {
                    _log.error(thisMessName + "出借人回款记录生成失败，borrowNid:" + borrowRepay.getBorrowNid() + " repayPeriod:" + repayPeriod);
                }
            }
        } else {
            // 获取放款记录
            ...
            for (BorrowRecover borrowRecover : borrowRecoverList) {
                ...
                if (Validator.isNumber(borrowRepay.getRepayActionTime())) {
                    try {
                        nifaReceivedPayments.setReturnDate(GetDate.getDateMyTimeInMillis(Integer.parseInt(borrowRepay.getRepayActionTime())));
                    } catch (NumberFormatException e) {
                        _log.error(thisMessName + "还款日格式化失败，borrowNid:" + borrowNid + " repayPeriod:" + repayPeriod);
                        e.printStackTrace();
                        return false;
     ...
     * @return
     */
    private List<BorrowRecover> selectBorrowRecoverList(String borrowNid, Integer repayPeriod) {
        BorrowRecoverExample example = new BorrowRecoverExample();
     ...
     * @return
     */
    private List<BorrowRecoverPlan> selectBorrowRecoverPlanList(String borrowNid, Integer repayPeriod) {
