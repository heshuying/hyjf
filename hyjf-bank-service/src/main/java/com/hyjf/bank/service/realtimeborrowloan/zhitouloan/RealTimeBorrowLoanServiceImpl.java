...
//					BigDecimal serviceFee = getUserFee(serviceFeeRate, txAmount, borrowStyle, borrowPeriod,borrow,curUserFee);// 放款服务费
	...
	public BankCallBean requestLoans(BorrowApicron apicron, Map map) {
		...
		try {
			...
			_log.info("==============cwyang 放款异常:" + e.getMessage());
		}
		return null;
	}
	...
	private boolean borrowLoans(BorrowApicron apicron, BorrowWithBLOBs borrow, BankCallBean bean) throws Exception {
		...
		if (tenderList != null && tenderList.size() > 0) {
				...
				try {
					...
					if (bean == null) {//放款失败
			           ...
			           // actBalloonTender(borrowTender);
						//  处理纳觅返现活动mq
						_log.info("纳觅返现活动标的加入时间更新");
						sendRrturnCashActivity(borrowNid,nowTime);
					}
				} catch (Exception e) {
			...
			try {
				...
				_log.info("==================放款时，上市活动放款失败，错误：" + e.getMessage());
				e.printStackTrace();
			}
	 ...
	 * @param nowTime
	 */
	private void sendRrturnCashActivity(String borrowNid,int nowTime){
		// 加入到消息队列
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("borrowNid", borrowNid);
		params.put("nowTime", nowTime);//放款时间
		rabbitTemplate.convertAndSend(RabbitMQConstants.EXCHANGES_NAME, RabbitMQConstants.RETURN_CASH1_ACTIVITY, JSONObject.toJSONString(params));
	}
//	/**
//	 * 生成投资居间服务协议
//	 * @param borrow
...
//		}catch (Exception e){
//			_log.info("-----------------生成居间服务协议失败，ordid:" + nid + ",异常信息：" + e.getMessage());
//		}
//	}
	 ...
	 * 双十二活动发送mq任务
	 */
	private void actBalloonTender(BorrowTender borrowTender) {
		if(checkActivityStatus(actId) == 3){
			Map<String, String> params = new HashMap<String, String>();
			params.put("mqMsgId", GetCode.getRandomCode(10));
			// 借款项目编号
			params.put("tenderNid", borrowTender.getNid());
			params.put("tenderType", "0");
...
//			this.sendMail(borrowRecover);
	...
	public List<WebUserInvestListCustomize> selectUserInvestList(String borrowNid, int userId, String nid, int limitStart, int limitEnd) {
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("borrowNid", borrowNid);
	...
	public int countProjectRepayPlanRecordTotal(String borrowNid, int userId, String nid) {
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("userId", userId);
		params.put("borrowNid", borrowNid);
		params.put("nid", nid);
	...
	public List<WebProjectRepayListCustomize> selectProjectRepayPlanList(String borrowNid, int userId, String nid, int offset, int limit) {
