...
package com.hyjf.bank.service.user.withdraw;
...
public class WithdrawServiceImpl extends BaseServiceImpl implements WithdrawService {
	...
	@Override
	public AccountBank getBankCard(Integer userId, String cardNo) {
		if (Validator.isNotNull(userId) && Validator.isNotNull(cardNo)) {
			// 取得用户银行卡信息
			AccountBankExample accountBankExample = new AccountBankExample();
			accountBankExample.createCriteria().andUserIdEqualTo(userId).andAccountEqualTo(cardNo);
			List<AccountBank> listAccountBank = this.accountBankMapper.selectByExample(accountBankExample);
			if (listAccountBank != null && listAccountBank.size() > 0) {
				return listAccountBank.get(0);
			}
