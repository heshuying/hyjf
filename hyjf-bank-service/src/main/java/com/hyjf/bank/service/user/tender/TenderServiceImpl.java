...
@Service
public class TenderServiceImpl extends BaseServiceImpl implements TenderService {
	private Logger logger = LoggerFactory.getLogger(TenderServiceImpl.class);
	@Autowired
	 ...
	 */
	@Override
	public boolean updateTenderLog(String borrowNid, String orderId, Integer userId, String account, String ip, String couponGrantId, String userName) throws Exception {
		...
		temp.setCouponGrantId(Integer.parseInt(couponGrantId));// add by cwyang
																// 为出借完全掉单优惠券出借时修复做记录
...
//				}
				// del by liuyang 2017-09-15 删除crm出借同步接口
				...
				result.put("status", 1);
				logger.info("******************收到请求没"+accountDecimal.intValue());
				...
				//logger.info("=============出借成功，标的号：" + borrowNid + ",用户ID：" + userId + ",出借金额：" + txAmount + ",同/异步标识：" + isBg);
				//end
	...
	public boolean checkIsNewUserCanInvest(Integer userId) {
		//汇计划新家逻辑，如果用户标识已变更则不再进行判断
		Users users = this.getUsersByUserId(userId);
		Integer investflag = users.getInvestflag();
		if (1 == investflag){//已经是老用户
			return false;
		}
		// 新的判断是否为新用户方法
	...
	public JSONObject checkParam(String borrowNid, String account, Integer userIdInt, String platform, String couponGrantId) {
		 ...
		 */
		UsersInfo usersInfo = this.getUserInfo(userIdInt);
		if (null != usersInfo) {
	...
	public boolean bidCancel(int investUserId, String productId, String orgOrderId, String txAmount) throws Exception {
		// 出借人的账户信息
		LogUtil.infoLog(this.getClass().getName(), "bidCancel","出借失败,开始撤销订单号为:" + orgOrderId + "的出借!");
		...
		if (queryTransStatBean == null) {
			...
			LogUtil.infoLog(this.getClass().getName(), "bidCancel","出借失败交易接口查询接口返回码：" + queryRespCode);
  			// 调用接口失败时(000以外)
	...
	public void extraUeldInvest(Borrow borrow, BankCallBean bean) {
		...
		if (tender != null) {
			...
			if (!incinvflag) {
				logger.info("融通宝出借额外利息出借失败，插入额外出借信息失败,出借订单号:" + tenderOrderId);
			}
		} else {
			logger.info("融通宝出借额外利息出借失败，获取出借信息失败,出借订单号:" + tenderOrderId);
		}
...
//		Integer isAnswer = (Integer) jsonObject.get("userEvaluationResultFlag");
		if (user != null) {
			isSetPwd = user.getIsSetPassword();
			isBankOpen = user.getBankOpenAccount();
			isAnswer = user.getIsEvaluationFlag();
		}
		// modify by liuyang 20180411 用户是否完成风险测评标识 end
		if (isBankOpen == 0) {
			return CustomConstants.BANK_TENDER_RETURN_OPENACCOUNT_FAIL;// 未开户
		}
		if (isSetPwd == 0) {
			return CustomConstants.BANK_TENDER_RETURN_SETPWD_FAIL;// 未设置交易密码
		}
		if (isAnswer == 0) {
			return CustomConstants.BANK_TENDER_RETURN_ANSWER_FAIL;// 未测评
		} else {
			if(isAnswer==1 && null != user.getEvaluationExpiredTime()){
				...
				if (lCreate <= lNow) {
					//已过期需要重新评测
					return CustomConstants.BANK_TENDER_RETURN_ANSWER_EXPIRED;// 测评过期
				}
	        } else {
				return CustomConstants.BANK_TENDER_RETURN_ANSWER_FAIL;// 未测评
	        }
	...
	public JSONObject getCouponIsUsed(String orderId,String couponGrantId, int userId) {
		...
		if (couponUser != null) {
			if (CustomConstants.USER_COUPON_STATUS_USED == couponUser.getUsedFlag()) {// 优惠券状态为已使用
				...
				LogUtil.infoLog(this.getClass().toString(), "tender", "==============cwyang 优惠券出借利率是 " + borrowApr);
				if (couponTypeInt == 1) {// 体验金的预期收益
					...
					LogUtil.infoLog(this.getClass().toString(), "tender", "============cwyang borrow.getBorrowStyle() is " + borrow.getBorrowStyle());
				...
				info.setCouponEarnings(earnings);
				LogUtil.infoLog(this.getClass().toString(), "tender", "===================cwyang 优惠券预期收益为: " + couponInterest);
				// 优惠券类别
	...
	public BorrowWithBLOBs selectBorrowByNid(String borrowId) {
		...
		cra.andBorrowNidEqualTo(borrowId);
	...
	public String checkHjhErrCode(Integer userId) {
		if (userId == null) {
			return CustomConstants.SIGN_ERROR;
		}
		// TODO Auto-generated method stub
		Users user = this.getUsers(userId);
		Integer isSetPwd = -1;
		Integer isBankOpen = -1;
		// del by liuyang 20180124 风险测评修改 start
//		JSONObject jsonObject = CommonSoaUtils.getUserEvalationResultByUserId(userId + "");
//		Integer isAnswer = (Integer) jsonObject.get("userEvaluationResultFlag");
		// del by liuyang 20180124 风险测评修改 end
		Integer isAnswer = user.getIsEvaluationFlag();
		if (user != null) {
			isSetPwd = user.getIsSetPassword();
			isBankOpen = user.getBankOpenAccount();
		}
		...
		boolean autoPaymentStatus=authService.checkPaymentAuthStatus(userId);
		boolean autoCreditStatus=authService.checkCreditAuthStatus(userId);
		boolean autoInvesStatus=authService.checkInvesAuthStatus(userId);
		if (!autoPaymentStatus&&!autoCreditStatus&&!autoInvesStatus) {
			return CustomConstants.BANK_TENDER_RETURN_AUTH_ALL_FAIL;// 未授权
		}

		if(!autoInvesStatus) {
			return CustomConstants.BANK_TENDER_RETURN_AUTH_FAIL;// 未授权
		}
		if(!autoCreditStatus) {
			return CustomConstants.BANK_TENDER_RETURN_AUTH_CREDIT_FAIL;// 自动债转未授权(返回码一致，因为二者是走相同的接口)
		}
		if(!autoPaymentStatus) {
			return CustomConstants.BANK_TENDER_RETURN_AUTH_PAYMENT_FAIL;// 服务费授权
		}
		if (isAnswer == 0) {
			return CustomConstants.BANK_TENDER_RETURN_ANSWER_FAIL;// 未测评
		} else {
		...
		return "";
//		else  if(map.get("autoiStatus").equals(false)&&map.get("autocStatus").equals(false)) {
//			return CustomConstants.BANK_TENDER_RETURN_AUTH_ALL_FAIL;// 未授权--dzs 增加全部未授权713
//		}
	}
	...
	private HjhUserAuth getUserAuthInfo(Integer userId) {
		HjhUserAuthExample hjhUserAuthExample = new HjhUserAuthExample();
		HjhUserAuthExample.Criteria hjhUserAuthExampleCriteria = hjhUserAuthExample.createCriteria();
		hjhUserAuthExampleCriteria.andUserIdEqualTo(userId);
		List<HjhUserAuth> userAuths = hjhUserAuthMapper.selectByExample(hjhUserAuthExample);
		HjhUserAuth hjhUserAuth = null;
		if (userAuths != null && !userAuths.isEmpty()) {
			hjhUserAuth = userAuths.get(0);
		}
		return hjhUserAuth;
	 ...
	 * @author Administrator
	 */
	public JSONObject checkAutoTenderParam(String borrowNid, String account, String bizAcount, String platform, String couponGrantId) {
		CouponConfigCustomizeV2 cuc = null;
		// -1:有可用的优惠券，但是出借时不选择优惠券 空或null：用户没有可用优惠券
		/*
		 * if(StringUtils.isNotEmpty(couponGrantId) && !StringUtils.equals("-1",
		...
		BankOpenAccount accountChinapnrTender = this.getBankOpenAccount(bizAcount);
		// 用户未在平台开户
		if (accountChinapnrTender == null) {
			return jsonMessage("用户开户信息不存在", "1");
		}
		...
		// 判断是否自动出借授权
		HjhUserAuth hjhUserAuth = this.getUserAuthInfo(userIdInt);
		if (hjhUserAuth == null || hjhUserAuth.getAutoInvesStatus() == null
			...
			return jsonMessage("您没有开通自动投标授权", "1");
		}
...
//				}
	 ...
	 * 产品加息收益计算
	 * @author sunss
	 * @see com.hyjf.bank.service.user.tender.TenderService#increaseCalculate()
	 */
    @Override
