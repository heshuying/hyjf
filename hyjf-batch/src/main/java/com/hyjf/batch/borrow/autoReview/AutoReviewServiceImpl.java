...
@Service
public class AutoReviewServiceImpl extends BaseServiceImpl implements AutoReviewService {
	@Autowired
	...
	public void updateBorrow(Integer afterTime) throws Exception {
		...
		List<BorrowWithBLOBs> borrowList = borrowFullCustomizeMapper.selectAutoFullList(new BorrowCommonCustomize());
		if (borrowList != null && borrowList.size() > 0) {
				...
				try {
					...
					if (borrow.getBorrowFullTime() != null &&borrow.getBorrowFullTime() != 0) {
						...
						if (borrow.getBorrowFullTime() + afterTime * 60 < nowTime) {
							...
							boolean borrowFlag = this.borrowMapper.updateByPrimaryKeySelective(borrow)>0?true:false;
							if(borrowFlag){
