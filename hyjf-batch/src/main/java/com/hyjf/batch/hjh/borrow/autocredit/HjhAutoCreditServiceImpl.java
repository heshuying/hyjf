...
public class HjhAutoCreditServiceImpl extends BaseServiceImpl implements HjhAutoCreditService {
    ...
    Logger _log = LoggerFactory.getLogger(HjhAutoCreditServiceImpl.class);
...
//        cra.andAccedeOrderIdIn(ordidList);
    ...
    public List<String> updateAutoCredit(HjhAccede hjhAccede, Integer creditCompleteFlag) throws Exception {
        ...
        _log.info("清算开始:计划加入订单号:[" + accedeOrderId + "].");
        // 根据加入订单号,当前日期进行清算
        ...
        _log.info("清算完成:计划加入订单号:[" + accedeOrderId + "].");
        return creditList;
    }
    ...
    private List<String> updateLiquidation(String accedeOrderId, Integer liquidationShouldTime, Integer creditCompleteFlag) throws Exception {
        ...
        if (debtDetails != null && debtDetails.size() > 0) {
            ...
            for (HjhDebtDetail hjhDebtDetail : debtDetails) {
                _log.info("清算计划编号:[" + hjhDebtDetail.getPlanNid() + "],计划加入订单号:" + hjhDebtDetail.getPlanOrderId() + "],出借订单号或承接订单号:[" + hjhDebtDetail.getOrderId() + "].");
                // 债权原标编号
                ...
                if (repayApicronList != null && repayApicronList.size() > 0) {
                    _log.info("标的正在还款中,稍后清算,标的编号:[" + borrowNid + "]");
                    isRepayFlag = 1;
                    continue;
                }
                ...
                if (isMonth) {
                    ...
                    if (borrowRepayPlanList != null && borrowRepayPlanList.size() > 0) {
                        _log.info("分期标的.应还日期在当天的债权,暂时不进行清算,标的编号:[" + borrowNid + "]");
                        isRepayFlag = 1;
                        continue;
                    }
                } else {
                    ...
                    if (borrowRepayList != null && borrowRepayList.size() > 0) {
                        _log.info("不分期标的,应还日期在当天的债权,暂时不进行清算,标的编号:[" + borrowNid + "].");
                        isRepayFlag = 1;
                        continue;
                    }
                }
                ...
                if (hjhDebtCreditYes != null) {
                    ...
                    _log.info("债权已被清算出,债权出借订单号:[" + hjhDebtDetail.getOrderId() + "],项目编号:[" + hjhDebtDetail.getBorrowNid() + "].");
                    continue;
                }
                // 查询完全承接的债权
                ...
                if (creditCompleteFlag == 2 && assignComplete != null) {
                    ...
                    _log.info("债权已被清算出,债权出借订单号:[" + hjhDebtDetail.getInvestOrderId() + "],项目编号:[" + hjhDebtDetail.getBorrowNid() + "].");
                    continue;
                }
                // 如果上次清算债转编号不为空
                ...
                // 项目类型
                hjhDebtCredit.setProjectType(borrow.getProjectType());
                // 原标还款方式
                hjhDebtCredit.setBorrowStyle(borrow.getBorrowStyle());
                // 原标项目期限
                hjhDebtCredit.setBorrowPeriod(borrow.getBorrowPeriod());
                // 原标机构编号
                hjhDebtCredit.setInstCode(borrow.getInstCode());
                // 原标资产编号
                hjhDebtCredit.setAssetType(borrow.getAssetType());
                // 生成债转编号
                ...
                if (CustomConstants.BORROW_STYLE_ENDDAY.equals(hjhDebtDetail.getBorrowStyle()) || CustomConstants.BORROW_STYLE_END.equals(hjhDebtDetail.getBorrowStyle())) {
                    ...
                    if (hjhDebtDetailCur != null) {
                        ...
                        _log.info("未逾期不分期的债权清算");
                        _log.info("当前期未还款期次的待收收益:" + interest);
                        _log.info("当前期计息天数:" + holdDays);
                        _log.info("当前期计息天数:" + duringDays);
                        _log.info("债转编号:[" + creditNid + "].");
                        // 承接所在期数
                    ...
                    if (hjhDebtDetailCur != null) {
                        ...
                        if (borrowRepayPlan != null) {
                            // 剩余期限 当前日期 到 最后一期还款日
                            try {
                                remainDays = GetDate.daysBetween(GetDate.timestamptoStrYYYYMMDD(liquidationShouldTime), GetDate.timestamptoStrYYYYMMDD(Integer.parseInt(borrowRepayPlan.getRepayTime()))) + 1;
                            } catch (ParseException e) {
                                e.printStackTrace();
                            }
                        }
                        ...
                        _log.info("未逾期分期的债权清算");
                        _log.info("当前期未还款期次的待收收益:" + hjhDebtDetailCur.getRepayInterestWait());
                        _log.info("当前期计息天数:" + holdDays);
                        _log.info("当前期计息天数:" + duringDays);
                        _log.info("债转编号:[" + creditNid + "].");
                        // 检索是否有之前期有逾期
                        ...
                        if (lastTermDebtDetail != null) {
                            ...
                            _log.info("逾期分期的债权清算");
                            _log.info("当前期未还款期次的待收收益:" + lastTermDebtDetail.getRepayInterestWait());
                            _log.info("当前期计息天数:" + holdDays);
                            _log.info("当前期计息天数:" + duringDays);
                            _log.info("债转编号:[" + creditNid + "].");
                            // 检索是否有之前期有逾期
                ...
                hjhDebtCredit.setCreateTime(liquidationShouldTime);
     ...
     * @return
     */
    private List<BorrowRepayPlan> searchRepayDetail(String borrowNid, Integer repayPeriod) {
        BorrowRepayPlanExample debtRepayDetailExample = new BorrowRepayPlanExample();
        BorrowRepayPlanExample.Criteria cri = debtRepayDetailExample.createCriteria();
        cri.andBorrowNidEqualTo(borrowNid);
        cri.andRepayPeriodEqualTo(repayPeriod);
        List<BorrowRepayPlan> debtRepayDetails = borrowRepayPlanMapper.selectByExample(debtRepayDetailExample);
     ...
     * @return
     */
    private List<BorrowRepay> searchRepay(String borrowNid) {
        BorrowRepayExample debtRepayDetailExample = new BorrowRepayExample();
        BorrowRepayExample.Criteria cri = debtRepayDetailExample.createCriteria();
        cri.andBorrowNidEqualTo(borrowNid);
        List<BorrowRepay> debtRepayDetails = borrowRepayMapper.selectByExample(debtRepayDetailExample);
        return debtRepayDetails;
    }
    ...
    public void sendToMQ(MQBorrow mqBorrow, String routingKey) {
        ...
        _log.info("加入到消息队列:债转编号:[" + mqBorrow.getCreditNid() + "].");
     ...
     */
    @Override
    public void sendHjhCalculateFairValueMQ(HjhCalculateFairValueBean hjhCalculateFairValueBean, String routingkeyBorrowIssue) {
        // 加入到消息队列
        Map<String, String> params = new HashMap<String, String>();
        params.put("mqMsgId", GetCode.getRandomCode(10));
        ...
        _log.info("发送计算加入订单的公允价值MQ处理,计划加入订单号:[" + hjhCalculateFairValueBean.getAccedeOrderId() + "].");
        rabbitTemplate.convertAndSend(RabbitMQConstants.EXCHANGES_COUPON, routingkeyBorrowIssue, JSONObject.toJSONString(params));
    }
    ...
    private void updateAccedeDate(String accedeOrderId) {
        ...
        if (accedeList != null && accedeList.size() > 0) {
            ...
            if (3 == orderStatus) {//锁定中再处理
                ...
                if (count > 0) {
                    _log.info("============cwyang 更新计划加入明细状态,计划加入订单号:" + accedeOrderId);
                }
            }
            // 更新计划订单的还款信息
            ...
            if (repayList != null && repayList.size() > 0) {
                ...
                _log.info("============cwyang 更新计划还款信息状态,计划加入订单号:" + accedeOrderId + ",变更前状态:" + repayOrderStatus);
    ...
    private void updateAccedeCreditCompleteFlag(String accedeOrderId,Integer creditCompleteFlag) {
        ...
        if (hjhAccedeList != null && hjhAccedeList.size() == 1) {
            ...
            if (isUpdate) {
                _log.info("更新加入计划订单是否完成清算状态:已清算,加入订单号:[" + accedeOrderId + "].");
            }
        }
    }
    ...
    /**
     * 根据项目编号,出借订单号,清算日期查询当前时间节点未还款的债权
     *
     * @param borrowNid
     * @param investOrderId
     * @param liquidationShouldTime
     * @return
     */
    private List<HjhDebtDetail> selectRepayHjhDebtDetails(String borrowNid, String investOrderId, Integer liquidationShouldTime) {
        ...
        // 应还日期
        cra.andRepayTimeLessThan(liquidationShouldTime);
        // 债权是否有效（0失效 1有效）
        ...
        return this.hjhDebtDetailMapper.selectByExample(example);
    }
     ...
     * @return
     */
    private HjhDebtDetail selectLastPeriodDebtDetail(String orderId, int repayPeriod) {
        HjhDebtDetailExample example = new HjhDebtDetailExample();
        HjhDebtDetailExample.Criteria cra = example.createCriteria();
    ...
    private List<BorrowRepayPlan> selectBorrowRepayPlanList(String borrowNid) {
        ...
        cra.andBorrowNidEqualTo(borrowNid);
        // 应还时间 >= 当天开始时间
        cra.andRepayTimeGreaterThanOrEqualTo(String.valueOf(GetDate.getDayStart10(GetDate.getDate())));
        // 应还时间 <= 当天结束时间
        cra.andRepayTimeLessThanOrEqualTo(String.valueOf(GetDate.getDayEnd10(GetDate.getDate())));
        // 还款状态未还款
        cra.andRepayStatusEqualTo(0);
