...
/**
 * 汇计划自动结束前一天未完全承接的债权定时任务Service实现类
 *
 * @author liuyang
 */
@Service
public class HjhAutoEndCreditServiceImpl extends BaseServiceImpl implements HjhAutoEndCreditService {
    ...
    Logger _log = LoggerFactory.getLogger(HjhAutoEndCreditServiceImpl.class);
...
//        cra.andPlanOrderIdIn(ordidList);
        cra.andCreateTimeLessThanOrEqualTo(GetDate.getDayEnd10(GetDate.getDate()));
    ...
    public void updateHjhDebtCreditStatus(HjhDebtCredit hjhDebtCredit) throws Exception {
        ...
        if (StringUtils.isBlank(planNid)) {
            _log.info("清算出的债转未绑定新的计划:债转编号:[" + hjhDebtCredit.getCreditNid() + "]");
            return;
        }
        // 如果计划编号不为空
        // 根据计划编号查询计划
        ...
        if (hjhAccede.getOrderStatus() != 5) {
            _log.info("计划加入订单不为退出中,计划清算的债转暂不结束,计划加入订单号:[" + accedeOrderId + "]");
            return;
        }
        // 债转编号
        ...
        if (hjhPlanBorrowTmpList != null && hjhPlanBorrowTmpList.size() > 0) {
            _log.info("转让记录出借存在异常,不予结束转让,转让编号:[" + creditNid + "].");
            return;
        }
        ...
        if (hjhDebtCredit.getCreditStatus() == 0) {
            ...
            _log.info("加入订单号:[" + accedeOrderId + "],债转编号:[" + hjhDebtCredit.getCreditNid() + "],清算时债权价值为:[" + hjhDebtCredit.getLiquidationFairValue()
                    + "],已承接债转的购买价格:[" + liquidationFairValue + "].");
        }
        // 1.更新债转状态未停止
        hjhDebtCredit.setCreditStatus(3);
        hjhDebtCredit.setEndTime(now);
        hjhDebtCredit.setUpdateTime(now);
        ...
        _log.info("债权停止后，更新计划开放额度，计划编号:[" + planNid + "],计划加入订单号:[" + accedeOrderId +
                ",Redis原开放额度:" + oldOpenAmount + "，应减额度:" + liquidationFairValue.toString());
        // 3.减扣redis相应计划可投金额
