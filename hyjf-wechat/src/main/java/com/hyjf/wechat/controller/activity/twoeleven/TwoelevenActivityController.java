...
public class TwoelevenActivityController extends BaseController {
    ...
    @Autowired
    @Qualifier("wechatTwoelevenActivityServiceImpl")
    ...
    public JSONObject queryActivityList(HttpServletRequest request) {
        ...
        Integer userId = null;
        try {
            JSONObject jsonUserId = getUserId(request);
            if(jsonUserId != null && "000".equals(jsonUserId.get("status"))){
                userId = Integer.valueOf(String.valueOf(jsonUserId.get("userId")));
                if(userId != null){
