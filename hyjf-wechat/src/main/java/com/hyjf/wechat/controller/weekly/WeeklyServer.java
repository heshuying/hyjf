...
import com.hyjf.wechat.service.weekly.WeeklyService;
import org.apache.commons.lang3.StringUtils;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Controller;
import org.springframework.web.bind.annotation.RequestMapping;
...
public class WeeklyServer extends BaseController{
    ...
    public WeeklyResultBean getWeeklyData(HttpServletRequest request, HttpServletResponse response) {
        ...
        if(user.getBankOpenAccount()==1) {
        	resultBean.setXingming(weeklyService.getUsersInfo(Integer.valueOf(userId)).getTruename());
        }
...
//        if(wse2.isEmpty()) {
       ...
       /* }else {
        	...
        	resultBean.setTouzie(df.format(wse2.get(0).getTouzie()));
        	resultBean.setShouyi(df.format(wse2.get(0).getShouyi()));
        	resultBean.setHuankuanzonge(df.format(wse2.get(0).getHuankuanzonge()));
        	List<String> list1 = new ArrayList<String>();
