 ...
 */
@Service
public class WithdrawHFServiceImpl extends BaseServiceImpl implements WithdrawHFService{
	 ...
	 */
	@Override
	public AccountBank getBankInfoHF(Integer userId, String bankId) {
		if (Validator.isNotNull(userId) && Validator.isNotNull(bankId)) {
			// 取得用户银行卡信息
			AccountBankExample accountBankExample = new AccountBankExample();
			accountBankExample.createCriteria().andUserIdEqualTo(userId).andAccountEqualTo(bankId);
	...
	public String getWithdrawFee(Integer userId, String bankId, BigDecimal amount, String type) {
		AccountBank accountBank = getBankInfoHF(userId, bankId);
		if (accountBank != null) {
			...
			if (listFeeConfig != null && listFeeConfig.size() > 0) {
				...
				// }
				// } else {
				if (Validator.isNotNull(feeConfig.getNormalTakeout())
						&& NumberUtils.isNumber(feeConfig.getNormalTakeout())) {
