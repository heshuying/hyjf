 ...
 * 
 * Date: Mon Dec 07 2015 16:56:31 +800
 */

;~function($, fillOptions, region) {
	$.fn.extend({
		regionSelect: function() {
			return $.each(this, function() {
				// 递归事件绑定
				~function(select) {
					select = $(select.change(function(childsel) {
						(childsel = $($(this).data("childsel")))[0] && (
								fillOptions($(this).data("datas")[$(this).val()], childsel[0]),
								childsel.val("").change());
					}).data("childsel")),
					select[0] && arguments.callee(select);
				}($(this)),
				// 递归填充options
				~function(select, region, defval) {
					fillOptions(region, select[0]),
					select.val(defval = select.data("default") || ""),
					defval && (
							select = $(select.data("childsel")),
							select[0] && arguments.callee(select, region[defval]));
				}($(this), region);
			});
		}
	});
}(jQuery, function(obj, select, flag, opts) {
	obj && select && (
		flag = $.isArray(obj),
		flag || $(select).data("datas", obj),
		opts = select.options,
		opts.length = 0,
		opts[0] = new Option("", ""),
		$.each(obj, function(name, opt) {
			flag && (name = this),
			Array.prototype.push.call(opts, opt = new Option(name, name));
		}));
}, {
	"北京市": {
