package com.hyjf.mqreceiver.borrow.planloan;
...
import com.hyjf.bank.service.borrowloan.plan.BatchHjhBorrowLoanService;
 ...
 *
 */
@Component(value="borrowLoanPlanMessageHadnle")
public class BorrowLoanPlanMessageHadnle implements ChannelAwareMessageListener {
    Logger _log = LoggerFactory.getLogger(BorrowLoanPlanMessageHadnle.class);

    @Autowired
    BatchHjhBorrowLoanService batchLoanService;
    ...
    public void onMessage(Message message, Channel channel) throws Exception {
        _log.info("----------------------计划放款任务开始----------------------服务类:" + new BorrowLoanPlanMessageHadnle().toString());
        if(message == null || message.getBody() == null){
            _log.error("【计划放款任务】接收到的消息为null");
            // 消息队列的指令不消费
        ...
        //_log.info("【计划放款请求】接收到的消息：" + msgBody);
		...
		_log.info("标的编号："+borrowNid+"，计划类开始放款！");
        // 生成任务key 校验并发请求
        String redisKey = RedisConstants.PLAN_LOAN_TASK + ":" + borrowApicron.getBorrowNid() + "_" + borrowApicron.getPeriodNow();
