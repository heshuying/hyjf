package com.hyjf.mqreceiver.borrow.planrealtimeloan;
...
import com.hyjf.bank.service.realtimeborrowloan.planloan.BatchHjhRealTimeBorrowLoanService;
 ...
 *
 */
@Component(value="borrowLoanPlanRealTimeMessageHadnle")
public class BorrowLoanPlanRealTimeMessageHadnle implements ChannelAwareMessageListener {
    Logger _log = LoggerFactory.getLogger(BorrowLoanPlanRealTimeMessageHadnle.class);

	@Autowired
	private MqService mqService;

	@Autowired
	BatchHjhRealTimeBorrowLoanService batchLoanService;
    ...
    public void onMessage(Message message, Channel channel) throws Exception {
        _log.info("----------------------计划实时放款任务开始------------------------");
        if(message == null || message.getBody() == null){
            _log.error("【计划实时放款任务】接收到的消息为null");
            // 消息队列的指令消费
			...
			if (loanStatus == CustomConstants.BANK_BATCH_STATUS_SENDING ) {
				...
				if (requestLoanBean == null) {
					...
					try {
                        ...
                        mqService.convertAndSend(RabbitMQConstants.EXCHANGES_NAME, RabbitMQConstants.LOAN_SUCCESS_DELAY_KEY, JSONObject.toJSONString(params));

                        // 推送数据到MQ 智投添加新的标的,标的放款成功后。
                        params = new JSONObject();
                        params.put("planNid", null);
                        params.put("borrowNid", borrowNid);
                        // 推送数据到MQ 智投放款成功
                        mqService.convertAndSend(RabbitMQConstants.EXCHANGES_NAME, RabbitMQConstants.LOAN_HJH_SUCCESS_DELAY_KEY, JSONObject.toJSONString(params));
