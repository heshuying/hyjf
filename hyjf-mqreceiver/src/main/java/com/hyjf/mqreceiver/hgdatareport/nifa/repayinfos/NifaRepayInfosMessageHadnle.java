...
import com.hyjf.mqreceiver.hgdatareport.nifa.tenderinfo.NifaTenderInfoService;
...
/**
 * 合规数据上报 NIFA 还款信息上报（延时队列）
 *
 * @author LIUSHOUYI
 * @version CertUserInfoMessageHadnle, v0.1 2018/11/22 10:11
 */
public class NifaRepayInfosMessageHadnle implements ChannelAwareMessageListener {
    ...
    @Autowired
    NifaRepayInfosService nifaRepayInfosService;
    ...
    public void onMessage(Message message, Channel channel) throws Exception {
        ...
        try {
            ...
            NifaBorrowInfoEntity nifaRepayInfoEntity = this.nifaRepayInfosService.selectNifaBorrowInfoByBorrowNid(borrowNid, msgBody);
            ...
            // --> 拉数据
            // 获取标的的借款详情
            Borrow borrow = this.nifaRepayInfosService.getBorrowByBorrowNid(borrowNid);
            if (null == borrow) {
                throw new Exception(logHeader + "未获取到相关项目信息！！");
            }
            // 获取标的还款表数据
            BorrowRepay borrowRepay = this.nifaRepayInfosService.selectBorrowRepay(borrowNid);
            if (null == borrowRepay) {
                throw new Exception(logHeader + "未获取到相关项目还款信息！！");
            }
            // 获取标的放款表数据
            List<BorrowRecover> borrowRecoverList = this.nifaRepayInfosService.selectBorrowRecoverListByBorrowNid(borrowNid);
            ...
            if ("1".equals(borrow.getCompanyOrPersonal())) {
                // 获取借款公司信息
                BorrowUsers borrowUsers = this.nifaRepayInfosService.selectBorrowUsersByBorrowNid(borrowNid);
                if (null == borrowUsers) {
                ...
                // 获取CA认证信息
                CertificateAuthority certificateAuthority = this.nifaRepayInfosService.selectCAInfoByUsername(borrowUsers.getUsername(), cardNo);
                ...
                // 获取借款人信息
                BorrowManinfo borrowManinfo = this.nifaRepayInfosService.selectBorrowMainfo(borrowNid);
                ...
                // 获取借款人CA认证信息
                CertificateAuthority certificateAuthority = this.nifaRepayInfosService.selectCAInfoByUsername(borrowManinfo.getName(), borrowManinfo.getCardNo());
                if (null != certificateAuthority && StringUtils.isNotBlank(certificateAuthority.getCustomerId())) {
                    borrower = certificateAuthority.getCustomerId();
                } else {
                    borrower = borrow.getId().toString();
                ...
                if (null != nifaBorrowerInfoEntity && "1".equals(nifaBorrowerInfoEntity.getReportStatus())) {
                    ...
                    if (result) {
                        ...
                        _log.error(logHeader + " 该出借人信息处理错误。" + msgBody);
                    }
                }
            }
            // --> 开始处理投资人数据
            ...
            if (null == nifaRepayInfoEntity) {
                nifaRepayInfoEntity = new NifaBorrowInfoEntity();
            }
            // 初始化数据类型
            ...
            result = this.nifaRepayInfosService.selectDualNifaRepayInfo(historyData, repayPeriod, borrow, borrowRepay, borrowRecoverList, recoverFee, lateCounts, nifaRepayInfoEntity);
            if (result) {
                // 保存借款详情
                this.nifaTenderInfoService.insertNifaBorrowInfo(nifaRepayInfoEntity);
            } else {
                _log.error(logHeader + " 该出借人信息处理错误。" + msgBody);
            }
