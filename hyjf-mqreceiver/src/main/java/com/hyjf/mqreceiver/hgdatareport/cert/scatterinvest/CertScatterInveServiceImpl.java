...
public class CertScatterInveServiceImpl extends BaseHgCertReportServiceImpl implements CertScatterInveService {
    ...
    public Map<String, Object> getSendData(String borrowNid,String idcardHash) throws Exception {
        ...
        BorrowProjectType borrowProjectType = getBorrowProjectType(borrow.getProjectType().toString());
        CertUser certUser = this.getCertUserByUserIdBorrowNid(borrow.getUserId(),borrowNid);
        if(certUser ==null){
     ...
     * @return
     */
    private String getServiceCost(Borrow borrow) {
        List<BorrowRecover> borrowRecovers = getBrorrowRecoverByBorrowNid(borrow.getBorrowNid());
        BigDecimal serviceCost = BigDecimal.ZERO;
        if (borrowRecovers != null && borrowRecovers.size() > 0) {
            for (BorrowRecover item : borrowRecovers) {
                serviceCost = serviceCost.add(item.getRecoverServiceFee());
            }
            BorrowRepay repay = getBorrowRepayByBorrowNid(borrow.getBorrowNid());
            if(repay!=null){
     ...
     * @return
     */
    private BorrowRepay getBorrowRepayByBorrowNid(String borrowNid) {
     ...
     * @return
     */
    private List<BorrowRecover> getBrorrowRecoverByBorrowNid(String borrowNid) {
