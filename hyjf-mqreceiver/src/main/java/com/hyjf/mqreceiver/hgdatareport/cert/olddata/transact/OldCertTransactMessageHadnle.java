 ...
 */
public class OldCertTransactMessageHadnle implements ChannelAwareMessageListener {

    Logger logger = LoggerFactory.getLogger(OldCertTransactMessageHadnle.class);
    ...
    public void onMessage(Message message, Channel channel) throws Exception {
        ...
        RedisUtils.set("oldCertTransactMqIsRun","1");
        String oldCertTransactId = jsonObject.getString("oldCertTransactId");
        // --> 消息处理
        try {
        	String pageStr=RedisUtils.get("page");
            ...
            // --> 调用service组装数据
        	List<Map<String, Object>> list=oldCertTransactService.getTransactMap(param);
        	if(list==null||list.size()==0){
        		RedisUtils.set("oldCertTransactMqIsRun","0");
    			return;
    		}
    		...
    		for (CertAccountList certAccountList : entitys) {
    			try {
                	oldCertTransactService.insertAndSendPostOld(certAccountList);
                    // 批量修改状态  end
                } catch (Exception e) {
                    throw e;
                }
