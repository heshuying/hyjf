...
/**
 * @Description 合规数据上报 CERT 国家互联网应急中心 标的老数据上报
 * @Author sunss
 * @Date 2018/11/26 17:57
 */
public class CertOldBorrowMessageHadnle implements ChannelAwareMessageListener {

    Logger logger = LoggerFactory.getLogger(CertOldBorrowMessageHadnle.class);

    private String thisMessName = "标的老数据上报";
    private String logHeader = "【" + CustomConstants.HG_DATAREPORT + CustomConstants.UNDERLINE + CustomConstants.HG_DATAREPORT_CERT + " " + thisMessName + "】";
    private static boolean isRun =false;
    @Autowired
    private CertOldBorrowService certOldBorrowService;
    @Autowired
    ...
    public void onMessage(Message message, Channel channel) throws Exception {
        ...
        try {
            for (int z = 0; z < 2; z++) {
                ...
                // 查询前多少条标的数据
                List<CertBorrow> users = this.certOldBorrowUserService.insertCertBorrow();
                logger.info(logHeader + "等待处理的散标数量:" + users == null ? "0" : users.size() + " 当前时间：" + GetDate.getNowTime10());
                if (users != null && users.size() > 0) {
                    JSONArray result = new JSONArray();
                    for (CertBorrow item : users) {
                        Map<String, Object> param = certScatterInveService.getSendData(item.getBorrowNid(), "");
                        result.add(param);
                    }
                    ...
                    List<CertReportEntity> entitys = CertCallUtil.groupByDate(result, thisMessName, CertCallConstant.CERT_INF_TYPE_SCATTER_INVEST);

                    for (CertReportEntity entity : entitys) {
                        // 上送数据
                        try {
                            entity = certScatterInveService.insertAndSendPost(entity);
                            logger.info(logHeader + "上报完成,数量为:" + entity.getDataList().size() + "，结果为:" + entity.getRetMess() + " 当前时间：" + GetDate.getNowTime10());
                        } catch (Exception e) {
                            throw e;
                        }
                        // 批量修改状态  start
                        ...
                        if (ids.size() > 0) {
                            ...
                            if (entity != null && CertCallConstant.CERT_RETURN_STATUS_SUCCESS.equals(entity.getReportStatus())) {
                                // 成功
                                certBorrow.setIsScatter(1);
                            } else {
                                // 失败
                                certBorrow.setIsScatter(99);
                            }
