...
/**
 * @Description 合规数据上报 CERT 历史数据转让项目信息推送上报（延时队列）
 * @Author pcc
 * @Date 2018/11/26 17:57
 */
public class OldCertTransferProjectMessageHadnle implements ChannelAwareMessageListener {

    Logger logger = LoggerFactory.getLogger(OldCertTransferProjectMessageHadnle.class);

    private String thisMessName = "历史数据转让项目信息";
    private String logHeader = "【" + CustomConstants.HG_DATAREPORT + CustomConstants.UNDERLINE + CustomConstants.HG_DATAREPORT_CERT + " " + thisMessName + "】";


    @Autowired
    private OldCertTransferProjectService oldcertTransferProjectService;
    ...
    public void onMessage(Message message, Channel channel) throws Exception {
        ...
        try {
            ...
            // --> 调用service组装数据
        	List<CertBorrow> certBorrowEntityList = oldcertTransferProjectService.insertCertBorrowEntityList();
            if(null!=certBorrowEntityList&&certBorrowEntityList.size()>0){
                Map<String, List<Map<String, Object>>> map = new HashMap<String, List<Map<String, Object>>>();
                for (CertBorrow certBorrow : certBorrowEntityList) {
                	List<Map<String, Object>> list=oldcertTransferProjectService.createList(certBorrow.getBorrowNid());
                ...
                for (CertBorrow certBorrow : certBorrowEntityList) {
                	...
                	if((list.size()+entityList.size())>3000){
                		...
                		List<CertReportEntity> entitys = CertCallUtil.groupByDate(data,thisMessName,CertCallConstant.CERT_INF_TYPE_TRANSFER_PROJECT);
                		for (CertReportEntity certReportEntity : entitys) {
            				try {
                            	oldcertTransferProjectService.insertAndSendPost(certReportEntity);
                                // 批量修改状态  start
                ...
                if (successIds.size() > 0) {
                    ...
                    certBorrow.setIsTransfer(1);
                    update.setCertBorrow(certBorrow);
                    // 批量修改
                    oldcertTransferProjectService.updateCertBorrowStatusBatch(update);
                }
                if (errorIds.size() > 0) {
                    CertBorrowUpdate update = new CertBorrowUpdate();
