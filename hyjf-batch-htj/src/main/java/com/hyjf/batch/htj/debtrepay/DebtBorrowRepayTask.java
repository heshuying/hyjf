...
public class DebtBorrowRepayTask {

	/** 类名 */
	private static final String THIS_CLASS = DebtBorrowRepayTask.class.getName();
	/** 运行状态 */
	...
	@Autowired
	DebtBorrowRepayService borrowRepayService;
	...
	private boolean repay() {
		...
		if (isRun == 0) {
			...
			try {
				// 取得未还款任务
				List<DebtApicron> listApicron = borrowRepayService.getBorrowApicronList(STATUS_WAIT, 1);
				if (listApicron != null && listApicron.size() > 0) {
					...
					for (DebtApicron apicron : listApicron) {
						...
						try {
							...
							// 取得还款明细列表
							List<DebtLoan> listRecover = borrowRepayService.getBorrowRecoverList(borrowNid);
							// 还款总件数
							...
							if (listRecover != null && size > 0) {
								...
								// 取得借款详情
								DebtBorrowWithBLOBs borrow = borrowRepayService.getBorrow(borrowNid);
								if (borrow == null) {
									throw new Exception(
								...
								// 借款人可用余额小于还款金额时报错
								BigDecimal returnAccount = borrowRepayService.getBorrowAccountWithPeriod(borrowNid,
										borrow.getBorrowStyle(), apicron.getPeriodNow());
								...
								if (chinapnrBalance.compareTo(returnAccount) < 0) {
									System.out.println("借款人在汇付平台的金额不足。[用户ID：" + borrowUserId + "]，" + "[借款编号："
											+ borrowNid + "]，" + "[可用余额：" + chinapnrBalance + "]，" + "[还款金额："
											+ returnAccount + "]");
									throw new Exception("借款人在汇付平台的金额不足。[用户ID：" + borrowUserId + "]，" + "[借款编号："
											+ borrowNid + "]，" + "[可用余额：" + chinapnrBalance + "]，" + "[还款金额："
											+ returnAccount + "]");
								}
								DebtLoan borrowRecover = null;
								/** 循环出借详情列表 */
								for (int i = 0; i < size; i++) {
									...
									// 出借订单号
									String tenderOrdId = borrowRecover.getInvestOrderId();
									try {
										...
										if (isMonth) {
											...
											// 取得分期还款计划表
											borrowRecoverPlan = borrowRepayService.getBorrowRecoverPlan(borrowNid,
													periodNow, tenderUserId, borrowRecover.getInvestId());
											if (borrowRecoverPlan != null) {
												// 保存还款订单号
												if (StringUtils.isBlank(borrowRecoverPlan.getRepayOrderId())) {
													// 还款订单号
													repayOrderId = GetOrderIdUtils
															.getOrderId2(borrowRecover.getUserId());
													// 还款订单日期
													...
													// 更新还款信息
													boolean recoverPlanFlag = this.borrowRepayService
													...
													// 设置还款订单号
													borrowRecover.setRepayOrderId(repayOrderId);
													// 设置还款时间
													borrowRecover.setRepayOrderDate(repayOrderDate);
													// 更新还款信息
													boolean recoverFlag = this.borrowRepayService
															.updateBorrowRecover(borrowRecover) > 0 ? true : false;
													if (!recoverFlag) {
												...
												throw new RuntimeException("分期还款计划表数据不存在。[借款编号：" + borrowNid + "]，"
														+ "[出借订单号：" + tenderOrdId + "]，" + "[期数：" + periodNow + "]");
											}
										}
										// [endday: 按天计息, end:按月计息]
										else {
											// 保存还款订单号
											if (StringUtils.isBlank(borrowRecover.getRepayOrderId())) {
												// 还款订单号
												...
												borrowRecover.setRepayOrderDate(repayOrderDate);
												// 更新还款信息
												boolean flag = this.borrowRepayService
														.updateBorrowRecover(borrowRecover) > 0 ? true : false;
												if (!flag) {
													throw new RuntimeException("添加还款订单号，更新borrow_recover表失败"
															+ "，[出借订单号：" + tenderOrdId + "]");
										...
										// 自动还款
										borrowRepayService.updateBorrowRepay(apicron, borrowRecover, borrowUserCust);
										// 还款之后冻结相应的资金
										boolean flag = this.borrowRepayService.repayBackUpdate(
												borrowRecover.getUserId(), borrowRecover.getUserName(),
												borrowRecover.getPlanNid(), borrowRecover.getPlanOrderId());
										if (!flag) {
											throw new Exception("还款后后续操作失败，加入订单号：" + borrowRecover.getPlanOrderId());
										}
									} catch (Exception e) {
										errorCnt++;
								...
								if (errorCnt > 0) {
									throw new Exception(
											"还款时发生错误。" + "[借款编号：" + borrowNid + "]，" + "[错误件数：" + errorCnt + "]");
								} else {
									// 更新最后还款状态
									this.borrowRepayService.updateBorrowStatus(borrowNid, apicron.getPeriodNow(),
											borrowUserId);
								}
							} else {
							...
							if (runCnt >= 3) {
								...
								// 更新任务API状态为错误
								borrowRepayService.updateBorrowApicron(apicron.getId(), STATUS_ERROR,
										sbError.toString());
							} else {
								// 更新任务API状态为重新执行
							...
							LogUtil.endLog(THIS_CLASS, methodName, "自动还款任务结束。[订单号：" + apicron.getBorrowNid() + "]， 耗时："
