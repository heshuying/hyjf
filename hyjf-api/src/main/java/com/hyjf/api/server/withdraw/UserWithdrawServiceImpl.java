...
public class UserWithdrawServiceImpl extends BaseServiceImpl implements UserWithdrawService {
	...
	Logger _log = LoggerFactory.getLogger(UserWithdrawServiceImpl.class);
				...
				if (accountWithdraw.getStatus() == WITHDRAW_STATUS_SUCCESS) {
					...
					if (accountlistCnt == 0) {
						...
						try {
							...
							_log.info("银联行号:" + payAllianceCode);
	 ...
	 * @param instCode
	 * @return
	 * @see com.hyjf.api.server.withdraw.UserWithdrawService#getUserFee(java.lang.String)
	 */
    @Override
    public String getUserFee(String instCode) {
        ...
        HjhInstConfigExample.Criteria cra = configExample.createCriteria();
           ...
           if (list != null && list.size() > 0) {
               return list.get(0).getCommissionFee().toString();
           }
    ...
    @Override
    public List<UserWithdrawRecordCustomize> getThirdPartyUserWithdrawRecord(Map<String, Object> param) {
        List<UserWithdrawRecordCustomize> list = withdrawCustomizeMapper.getThirdPartyUserWithdrawRecord(param);
        return list;
    }
