...
public class ApiUserBindServer extends ApiBaseController {
    ...
    public ModelAndView bindApi(HttpServletRequest request, HttpServletResponse response, ApiUserPostBean apiUserPostBean){
    	...
    	ModelAndView modelAndView = new ModelAndView(ApiUserBindDefine.FAST_AUTH_LOGIN_PTAH);
		// 验证
    ...
    public JSONObject bind(HttpServletRequest request, HttpServletResponse response, 
		...
		if(!loginBean.getReadAgreement()){
		    ...
		    jsonObj.put(ApiUserBindDefine.STATUS, ApiUserBindDefine.STATUS_FALSE);
		    jsonObj.put(ApiUserBindDefine.ERROR, "授权失败，请仔细阅读并同意《汇盈金服授权协议》");
            return jsonObj;
		}
		// 用户手机号码验证
		if(!StringUtils.isNotBlank(loginBean.getLoginUserName())){
			jsonObj = new JSONObject();
			jsonObj.put(ApiUserBindDefine.STATUS, ApiUserBindDefine.STATUS_FALSE);
			jsonObj.put(ApiUserBindDefine.ERROR, "授权失败，请输入正确的手机号码");
			return jsonObj;
		}
		...
		if (users != null) {
	        ...
	        if(userid != null){
                // 纳觅财富
                if (ApiUserBindDefine.NMCF_PID.equals(apiUserPostBean.getPid())) {
                    JSONObject jsonResult = new JSONObject();
                    jsonResult.put(ApiUserBindDefine.STATUS, ApiUserBindDefine.STATUS_TRUE);
                    jsonResult.put("statusCode", ApiUserBindDefine.ERROR_SUCCESS);
					...
					jsonResult.put(ApiUserBindDefine.RETURL, apiUserPostBean.getRetUrl() + "?datajson=" + URLEncoder.encode(str, "UTF-8"));
                    return jsonResult;
                }
	        	jsonObj = new JSONObject();
	            jsonObj.put(ApiUserBindDefine.STATUS, ApiUserBindDefine.STATUS_FALSE);
	            jsonObj.put(ApiUserBindDefine.ERROR, ValidatorCheckUtil.getErrorMessage("user.repeatbound"));
	            return jsonObj;
	        }
	        ...
	        if(binduniqueid != null){
	        	jsonObj = new JSONObject();
	            jsonObj.put(ApiUserBindDefine.STATUS, ApiUserBindDefine.STATUS_FALSE);
	            jsonObj.put(ApiUserBindDefine.ERROR, ValidatorCheckUtil.getErrorMessage("user.useridbound"));
	            return jsonObj;
	        }
		}
        ...
        if (jsonObj.get(ApiUserBindDefine.STATUS).equals(ApiUserBindDefine.STATUS_FALSE)) {
            // 登陆失败，返回失败信息
            return jsonObj;
        }
        ...
        if(!result){
        	jsonObj = new JSONObject();
            jsonObj.put(ApiUserBindDefine.STATUS, ApiUserBindDefine.STATUS_FALSE);
            jsonObj.put(ApiUserBindDefine.ERROR, "授权失败，请联系汇盈金服客服。");
            return jsonObj;
        }
        ...
        jsonResult.put(ApiUserBindDefine.RETURL, apiUserPostBean.getRetUrl());
        jsonResult.put("bindUniqueIdScy", apiUserPostBean.getBindUniqueIdScy());
        jsonResult.put("hyjfUserName",userName ); 
        jsonResult.put("userId",users.getUserId() ); 
        Long timestamp = System.currentTimeMillis();
