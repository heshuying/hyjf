...
public class InvesAuthPagePlusController extends BaseController {
    ...
    public ModelAndView page(HttpServletRequest request, HttpServletResponse response) {
        LogUtil.startLog(InvesAuthPagePlusDefine.THIS_CLASS, InvesAuthPagePlusDefine.INVES_AUTH_ACTION);
        ModelAndView modelAndView = new ModelAndView();
        // 用户id
        WebViewUser user = WebUtils.getUser(request);
        if (user == null) {
            modelAndView = new ModelAndView(InvesAuthPagePlusDefine.USER_AUTH_ERROR_PATH);
        ...
        // 拼装参数 调用江西银行
        // 同步调用路径
        String retUrl = PropUtils.getSystem(CustomConstants.HYJF_WEB_URL) + InvesAuthPagePlusDefine.REQUEST_MAPPING
                + InvesAuthPagePlusDefine.RETURL_SYN_ACTION + ".do?authType=" + AuthBean.AUTH_TYPE_AUTO_BID;
        // 异步调用路
        String bgRetUrl = PropUtils.getSystem(CustomConstants.HYJF_WEB_URL) + InvesAuthPagePlusDefine.REQUEST_MAPPING
        ...
        try {
            ...
            authService.insertUserAuthLog(authBean.getUserId(), orderId, Integer.parseInt(authBean.getPlatform()), BankCallConstant.QUERY_TYPE_1);
            LogUtil.endLog(InvesAuthPagePlusDefine.THIS_CLASS, InvesAuthPagePlusDefine.INVES_AUTH_ACTION);
        } catch (Exception e) {
            e.printStackTrace();
            modelAndView = new ModelAndView(InvesAuthPagePlusDefine.USER_AUTH_ERROR_PATH);
            modelAndView.addObject("message", "调用银行接口失败！");
            LogUtil.errorLog(InvesAuthPagePlusDefine.THIS_CLASS, InvesAuthPagePlusDefine.INVES_AUTH_ACTION,
                    e);
        }
        return modelAndView;
    }
    ...
    public ModelAndView invesAuthReturn(HttpServletRequest request, HttpServletResponse response,
        @ModelAttribute BankCallBean bean) {

        LogUtil.startLog(InvesAuthPagePlusDefine.THIS_CLASS, InvesAuthPagePlusDefine.RETURL_SYN_ACTION,
                "[自动出借授权同步回调开始]");
        ModelAndView modelAndView = new ModelAndView();
        bean.convert();
        ...
        // 出借人签约状态查询
        _log.info("自动出借授权同步回调调用查询接口查询状态,userId:" + bean.getLogUserId());
        ...
        if (retBean != null && BankCallStatusConstant.RESPCODE_SUCCESS.equals(retBean.getRetCode())
                && "1".equals(retBean.getAutoBid())) {
            try {
            	if(authService.checkDefaultConfig(retBean, AuthBean.AUTH_TYPE_AUTO_BID)){
                	modelAndView = new ModelAndView(InvesAuthPagePlusDefine.AUTH_TENDER_AGAIN_ERROR_PATH);
                    modelAndView.addObject("message", "授权期限过短或额度过低，<br>请重新授权！");
                    return modelAndView;
                }
                retBean.setOrderId(bean.getLogOrderId());
                // 更新签约状态和日志表
                this.authService.updateUserAuth(Integer.parseInt(bean.getLogUserId()), retBean,AuthBean.AUTH_TYPE_AUTO_BID);
            } catch (Exception e) {
                e.printStackTrace();
                LogUtil.errorLog(InvesAuthPagePlusDefine.THIS_CLASS, InvesAuthPagePlusDefine.RETURL_ASY_ACTION, e);
            }
            // 成功
        ...
        modelAndView = new ModelAndView(InvesAuthPagePlusDefine.USER_AUTH_SUCCESS_PATH);
        modelAndView.addObject("message", "自动出借授权成功");
        LogUtil.endLog(InvesAuthPagePlusDefine.THIS_CLASS, InvesAuthPagePlusDefine.RETURL_SYN_ACTION,
                "[自动出借授权完成后,回调结束]");
        return modelAndView;
    }
    ...
    public BankCallResult invesAuthBgreturn(HttpServletRequest request, HttpServletResponse response,
        @ModelAttribute BankCallBean bean) {
        BankCallResult result = new BankCallResult();
        _log.info("[自动出借授权异步回调开始]");
        LogUtil.startLog(InvesAuthPagePlusDefine.THIS_CLASS, InvesAuthPagePlusDefine.RETURL_ASY_ACTION,
                "[自动出借授权异步回调开始]");
        bean.convert();
        ...
        LogUtil.endLog(InvesAuthPagePlusDefine.THIS_CLASS, InvesAuthPagePlusDefine.RETURL_ASY_ACTION,
                "[用户自动出借授权完成后,回调结束]");
        _log.info("[用户自动出借授权完成后,回调结束]");
