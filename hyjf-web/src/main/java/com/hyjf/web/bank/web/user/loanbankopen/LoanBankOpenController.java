 ...
 * @see 上午9:49:17
 */
@Controller(LoanBankOpenDefine.CONTROLLER_NAME)
@RequestMapping(value = LoanBankOpenDefine.REQUEST_MAPPING)
public class LoanBankOpenController extends BaseController {
	Logger _log = LoggerFactory.getLogger(LoanBankOpenController.class);
	@Autowired
	...
	/** 当前controller名称 */
	public static final String THIS_CLASS = LoanBankOpenController.class.getName();
	 ...
	 * @return
	 */
	@RequestMapping(LoanBankOpenDefine.BANKOPEN_INIT_ACTION)
	public ModelAndView initOpenAccount(HttpServletRequest request, HttpServletResponse response) {

		LogUtil.startLog(THIS_CLASS, LoanBankOpenDefine.BANKOPEN_INIT_ACTION);
		ModelAndView modelAndView = new ModelAndView(LoanBankOpenDefine.BANKOPEN_INIT_PATH);
	 ...
	 */
	@ResponseBody
	@RequestMapping(value = LoanBankOpenDefine.MOBILE_CHECK_ACTION, method = RequestMethod.POST, produces = "application/json; charset=utf-8")
	...
	// 开户
	@ResponseBody
    @RequestMapping(method = RequestMethod.POST,value =LoanBankOpenDefine.BANKOPEN_OPEN_ACTION)
    public ModelAndView openAccount(HttpServletRequest request, HttpServletResponse response,
            @ModelAttribute LoanBankOpenBean accountBean) {
	    ModelAndView modelAndView = new ModelAndView(LoanBankOpenDefine.BANKOPEN_ERROR_PATH);
	    try {
	        // 获取登陆用户userId
            ...
            // 拼装参数 调用江西银行
            // 同步调用路径
            String retUrl = PropUtils.getSystem(CustomConstants.HYJF_WEB_URL) + LoanBankOpenDefine.REQUEST_MAPPING
                    + LoanBankOpenDefine.RETURL_SYN_ACTION + ".do?phone="+accountBean.getMobile()+"&uid="+userId;
            // 异步调用路
            String bgRetUrl = PropUtils.getSystem(CustomConstants.HYJF_WEB_URL) + LoanBankOpenDefine.REQUEST_MAPPING
                    + LoanBankOpenDefine.RETURL_ASY_ACTION + ".do?phone="+accountBean.getMobile();
     ...
     * @return
     */
    @RequestMapping(LoanBankOpenDefine.RETURL_SYN_ACTION)
     ...
     */
    @ResponseBody
    @RequestMapping(LoanBankOpenDefine.RETURL_ASY_ACTION)
