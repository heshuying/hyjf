 ...
 * @see 下午12:52:16
 */
public class BankOpenAccountInterceptor extends HandlerInterceptorAdapter {
		...
		if (StringUtils.isBlank(result)) {// 如果用户没有登录
			...
			if (null == webViewUser) {// 如果用户没有登录
				...
				if (webViewUser.isBankOpenAccount()) {
				    ...
				    // 重定向到开户页面
					WebUtils.redirectTargetPage(response, xmlHttpRequest, "/bank/web/user/bankopen/init.do", "708");
				}
			}
		}
		return false;
