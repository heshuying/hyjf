...
<body>
	...
	<form action="${ctx}/credit/tendertocreditdetail.do" id="tenderToCreditForm" name="tenderToCreditForm" method="post">
...
function tendertocredit(borrowNid, tenderNid){
	jQuery.ajax({
		type: "POST",
		async: "async",
		url: "${ctx}/credit/tenderabletocredit.do",
...
function tenderToCreditAjax(currPage, limitPage){
	jQuery.ajax({
		type: "POST",
		async: "async",
		url: "${ctx}/credit/tendertocreditlist.do",
		contentType:"application/x-www-form-urlencoded;charset=utf-8",
		dataType: "json",
		data: {
			"currPage":currPage,
			...
			if(data.resultFlag==0){
				...
				if(recordList.length>0){
					for(i=0;i<recordList.length;i++){
						...
						recordHtml += '<tr>';
						recordHtml += '<td><a href="${ctx}/project/getProjectDetail.do?borrowNid='+record.borrowNid+'" class="title"> <span class="id">'+record.borrowNid+'</span><br><span class="date">投资时间 ：'+record.tenderTime.substring(0,10)+'</span></a></td>';
					...
					recordHtml += '<tr class="data-empty">';
					recordHtml += '<td class="empty-icon" colspan="8"><p class="align-center">咦，您所找的页面暂无数据～</p></td>';
					recordHtml += '</tr>';
				}
...
function creditInprogressAjax(currPage, limitPage){
	jQuery.ajax({
		type: "POST",
		async: "async",
		url: "${ctx}/credit/creditinprogresslist.do",
		contentType:"application/x-www-form-urlencoded;charset=utf-8",
		dataType: "json",
		data: {
			"currPage":currPage,
			...
			if(data.resultFlag==0){
				...
				if(recordList.length>0){
					for(i=0;i<recordList.length;i++){
						var record = recordList[i];
						recordHtml += '<tr>';
						recordHtml += '<td><a href="${ctx}/project/getProjectDetail.do?borrowNid='+record.bidNid+'" class="title"><span class="id">'+record.bidNid+'</span><br> <span class="date">转让时间 ：'+record.addTime.substring(0,10)+'</span></a></td>';
					...
					recordHtml += '<tr>';
					recordHtml += '<td colspan="5">没有进行中的债转数据</td>';
					recordHtml += '</tr>';
				}
...
function creditStopAjax(currPage, limitPage){
	jQuery.ajax({
		type: "POST",
		async: "async",
		url: "${ctx}/credit/creditstoplist.do",
		contentType:"application/x-www-form-urlencoded;charset=utf-8",
		dataType: "json",
		data: {
			"currPage":currPage,
...
function creditAssignAjax(currPage, limitPage){
	jQuery.ajax({
		type: "POST",
		async: "async",
		url: "${ctx}/credit/creditassignlist.do",
		contentType:"application/x-www-form-urlencoded;charset=utf-8",
		dataType: "json",
		data: {
			"currPage":currPage,
			...
			if(data.resultFlag==0){
				...
				if(recordList.length>0){
					for(i=0;i<recordList.length;i++){
						...
						recordHtml += '<td><span class="end-date">'+record.assignRepayEndTime+' <br><a href="${ctx}/credit/usercreditcontract.do?bidNid='+record.bidNid+'&creditNid='+record.creditNid+'&creditTenderNid='+record.creditTenderNid+'&assignNid='+record.assignNid+'" class="show-term" target="_blank">查看协议</a><br><a href="javascript:;" onclick="creditRepayPlanList(\''+record.creditNid+'\',\''+record.assignNid+'\');">还款计划</a></span></td>';
...
function creditAssignTenderList(creditNid){
	jQuery.ajax({
		type: "POST",
		async: "async",
		url: "${ctx}/credit/creditassigntenderlist.do",
		contentType:"application/x-www-form-urlencoded;charset=utf-8",
		dataType: "json",
		data: {
			"creditNid":creditNid
			...
			if(data.resultFlag==0){
				...
				if(recordList!=null && recordList!="" && recordList.length>0){
					for(i=0;i<recordList.length;i++){
						...
						recordHtml += '<td><a href="${ctx}/credit/usercreditcontract.do?bidNid='+record.bidNid+'&creditNid='+record.creditNid+'&creditTenderNid='+record.creditTenderNid+'&assignNid='+record.assignNid+'" target="_blank">协议</a></td>';
...
function creditRepayPlanList(creditNid,assignNid){
	jQuery.ajax({
		type: "POST",
		async: "async",
		url: "${ctx}/credit/creditrepayplanlist.do",
		contentType:"application/x-www-form-urlencoded;charset=utf-8",
		dataType: "json",
		data: {
			"creditNid":creditNid,
