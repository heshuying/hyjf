			...
			<div class="container-1200">
				<h2>
					<span class="fl-l title">${creditResult.data.borrowName}</span> <span
						class="fl-l id">${creditResult.data.borrowNid}</span> <a href="${ctx}/credit/usercreditlist.do" class="back fl-r">&lt;&lt;返回</a>
				</h2>
				<div class="info">
					<div class="item">
						<div class="num">
							...
							<td>
								<div class="perc-calc">
									...
									<div class="ctrl-number">
										<input type="text" value="0.5" maxlength="3" id="creditDiscount" name="creditDiscount">
										<div class="unit">%</div>
									</div>
	...
	<script type="text/javascript">
		function creditPriceAndFee(){
			...
			jQuery.ajax({
				type: "POST",
				async: "async",
				url: "${ctx}/credit/expectcreditfee.do",
...
$(document).ready(function(){
	...
	jQuery("#vercode").blur(function(){
		...
		if(vercode!=null && vercode!=""){
			jQuery.ajax({
				type: "POST",
				async: "async",
				url: "${ctx}/credit/checkcaptcha.do",
				contentType:"application/x-www-form-urlencoded;charset=utf-8",
				dataType: "json",
				data: {
					"captchaCode":vercode
	...
	jQuery("#confirmBtn").click(function(){
		...
		$.ajax({
			cache: true,
			type: "POST",
			url:"${ctx}/credit/savetendertocredit.do",
		    ...
		    success: function(data) {
			    if(data.resultFlag==0){
			    	//跳转到投资列表
			    	window.location.href="${ctx}/credit/tendertocreditresult.do?creditNid="+data.data;
			    }else{
			    	popupWin(data.msg);
			    	jQuery("#confirmWait").hide();
			    	jQuery("#confirmBtn").show();
