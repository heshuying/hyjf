...
<body>
	...
	<div class="vip-buy-pop hide">
		...
		<div class="vip-buy-pop-div">
			...
			<div class="vip-buy-buy">
				<a href="${returl}"></a>
			</div>
		</div>
	</div>
	<div class="vip-buy-header">
		<a href="${returl}"></a>
	</div> --%>
			...
			<div class="new-detail-hd">
				...
				<div class="hd2">
					...
					<c:if test="${projectDeatil.status eq 10}">
						...
						<form action="${ctx}/user/invest/appointment.do" id="detailForm">
							...
							<div class="hd2-item con1">
								<c:choose>
									<c:when test="${projectDeatil.type eq 11 and (projectDeatil.bookingStatus eq 0 or projectDeatil.bookingStatus eq 1 or projectDeatil.bookingStatus eq 2 ) and ( projectDeatil.bookingBeginTime ne 0 and projectDeatil.bookingEndTime ne 0)}">
										<input type="hidden" id="projectData" data-total="<fmt:formatNumber value="${projectDeatil.borrowAccountWaitAppoint}" pattern="####" />"
											data-tendermax="${projectDeatil.tenderAccountMax}"
											data-tendermin="${projectDeatil.tenderAccountMin}"/>
										<span class="title">可预约金额</span>
										<span class="total"> 
										<fmt:formatNumber value="${projectDeatil.borrowAccountWaitAppoint}" pattern="#,###" /> <span class="total-font">元</span>
										</span>
									</c:when>
									<c:otherwise>
							...
							<div class="hd2-item con2">
								<c:if test="${loginFlag eq 1}">
									...
									<span class="balance" id="balance"> 
										<fmt:formatNumber value="${userBalance}" pattern="#,##0.00" /> 元
									</span>
									<a href="${ctx}/recharge/rechargePage.do" class="recharge" onclick="setCookie()">充值</a>
								</c:if>
								...
								<c:when test="${projectDeatil.type eq 11 and (projectDeatil.bookingStatus eq 0 or projectDeatil.bookingStatus eq 1 or projectDeatil.bookingStatus eq 2 ) and (projectDeatil.bookingBeginTime ne 0 and projectDeatil.bookingEndTime ne 0)}">
									...
									<div class="hd2-item con3">
										<div class="money">
											...
											<input type="text" name="money" id="money" class="money-input" maxlength="9"
												...
												<c:if test="${projectDeatil.tenderAccountMin ge borrowAccountWaitAppoint }"> value="${borrowAccountWaitAppoint}"  readonly="readonly"</c:if>
												<c:if test="${projectDeatil.bookingStatus eq 2 }">  readonly="readonly"</c:if>/>
												
											<input id="nid" name="nid" type="hidden" value="${projectDeatil.borrowNid}" />
											<c:if test="${loginFlag eq 1 and userBalance ne '0.00'}">
									...
									<div class="hd2-item con4">
										<div class="appoint-term">
											...
											<div class="terms-box">
												<a href="#" onclick="openNew('${ctx}/user/invest/contract.do')" class="highlight">《预约投标服务需知》</a> 
												<a href="#" onclick="openNew('${ctx}/user/regist/goDetail.do?type=invest')" class="highlight">《居间服务借款协议》</a> 
												<a href="#" onclick="openNew('${ctx}/user/regist/goDetail.do?type=contract')" class="highlight">《投资风险确认书》</a>
											</div>
										</div>
									</div>
									<div class="hd2-item con5">
										<c:if test="${projectDeatil.bookingBeginTime gt nowTime and projectDeatil.bookingStatus eq 0}">
											<a href="#" class="confirm-btn disabled">${projectDeatil.onAppointTime}预约</a>
										</c:if>
										<c:if test="${projectDeatil.bookingBeginTime le nowTime and projectDeatil.bookingEndTime ge nowTime and projectDeatil.bookingStatus eq 1}">
											<a href="javascript:;" class="confirm-btn avaliable">立即预约</a>
										</c:if>
										<c:if test="${(projectDeatil.bookingEndTime lt nowTime and projectDeatil.bookingStatus eq 2) or(projectDeatil.bookingBeginTime le nowTime and projectDeatil.bookingEndTime ge nowTime  and projectDeatil.bookingStatus eq 2)}">
											<a href="#" class="confirm-btn disabled">预约完成</a>
										</c:if>
									</div>
									<!-- 预约投标结束-->
								</c:when>
										...
										<c:otherwise>
											<div class="hd2-item con4 coupon-txt couponDiv">
												<c:if test="${isThereCoupon==1}">
													<span> 已选择 <span class="num">${couponConfig.couponQuotaStr }
					...
					<c:if test="${projectDeatil.status eq 11}">
						<input type="hidden" id="projectData" data-total="${projectDeatil.investAccount}" data-tendermax="${projectDeatil.tenderAccountMax}" data-tendermin="${projectDeatil.tenderAccountMin}"/>
						<input type="hidden" id="userData" data-balance="${userBalance}" data-openflag="${openFlag}"/>
						<form action="${ctx}/user/invest/invest.do" id="detailForm">
							<input type="hidden" name="tokenCheck" id="tokenCheck" value="" />
							<input type="hidden" name="tokenGrant" id="tokenGrant" value="${tokenGrant}" /> 
				...
				<ul class="new-detail-tab-panel">
					...
					<li panel="5">
						<div class="new-detail-tb-t">
							...
							</span> <span>加入人次 <strong id="investTimes">10</strong> 人次
							</span>
						</div>
						<table class="new-detail-tb">
	...
	<script type="text/javascript"
		src="${cdn}/js/jquery.validate.js?version=${version}" charset="utf-8"></script>
	<script type="text/javascript"
		src="${cdn}/js/messages_cn.js?version=${version}" charset="utf-8"></script>
	<script type="text/javascript"
		src="${cdn}/js/jquery.metadata.js?version=${version}" charset="utf-8"></script>
	<script type="text/javascript"
		src="${cdn}/js/baguetteBox.min.js?version=${version}" charset="utf-8"></script>
	<script type="text/javascript"
		src="${cdn}/js/jquery.cookie.min.js?version=${version}"
		charset="utf-8"></script>
	<c:choose>
		<c:when
			test="${projectDeatil.status eq 10 and  projectDeatil.type eq 11 and (projectDeatil.bookingStatus eq 0 or projectDeatil.bookingStatus eq 1 or projectDeatil.bookingStatus eq 2 ) and ( projectDeatil.bookingBeginTime ne 0 and  projectDeatil.bookingEndTime ne 0)}">
			<script type="text/javascript"
				src="${cdn}/js/project/appointDetail.js?version=${version}"></script>
			<script>
				var lastTime = ${endMins}; // 剩余毫秒数 
				renderTimer();
			</script>
		</c:when>
		<c:otherwise>
			<script type="text/javascript"
				src="${cdn}/js/project/projectDetail.js?version=${version}"></script>
		</c:otherwise>
	</c:choose>
