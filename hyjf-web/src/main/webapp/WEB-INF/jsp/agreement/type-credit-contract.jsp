        ...
        <div class="container">
            <section class="about-detial creditcontract content">
                <div class="main-title">
                    汇盈金服互联网金融服务平台债权转让协议
                    <%--原  <a class="download-txt" href="${ctx}/bank/user/credit/usercreditcontractsealpdf.do?bidNid=${creditResult.data.borrow.borrowNid}&creditNid=${creditResult.data.creditTender.creditNid}&creditTenderNid=${creditResult.data.creditTender.creditTenderNid}&assignNid=${creditResult.data.creditTender.assignNid}" target="blank">下载协议</a> --%>
                    <c:choose>
	                    <c:when test="${creditResult.data.borrow.borrowNid == null || creditResult.data.borrow.borrowNid == '' }">
	            		<!-- 不显示 下载协议 -->
	                    </c:when>
	                    <c:otherwise>
	                    	<a class="download-txt" href="${ctx}/createAgreementPDF/planusercreditcontractsealpdf.do?bidNid=${creditResult.data.borrow.borrowNid}&creditNid=${creditResult.data.creditTender.creditNid}&creditTenderNid=${creditResult.data.creditTender.assignOrderId}" target="blank">下载协议</a>
	                    </c:otherwise>
                    </c:choose>
                </div>
                <div class="detial-list">
                	<p style="text-align: left;">合同编号：${creditResult.data.creditTender.assignOrderId}</p>
                    ...
                    <table border="1" class="table-info">
                      ...
                      <tr>
                        <td>预计还款日</td>
                        <td>
                        自 <%--  ${creditResult.data.creditTender.addTime} --%>${creditResult.data.creditTender.updateUserName}月起，每月${creditResult.data.creditTender.addip}   日（24:00前）
                         </td>
                      </tr>
                       <tr>
                        <td>借款期限</td>
                    ...
                    <table border="1">
                      <tr>
                        <td>转让债权本金</td>
                        <td>${creditResult.data.creditTender.assignCapital}元 </td>
                      </tr>
                      <tr>
                        <td>转让价款</td>
                        <td>${creditResult.data.creditTender.assignPay}元  </td>
                      </tr>
                      <tr>
                        <td>转让剩余期限</td>
                    ...
                    <p>1.2.2  双方同意，本协议通过汇盈金服平台审核通过时，本协议成立，在乙方转让价款支付完成时生效.协议成立的同时，甲方不可撤销地授权汇盈金服平台贷委托第三方支付平台将债权标的转让价款在扣除甲方应支付给汇盈金服平台的相关款项后（包括但不限于转让管理费）划转给甲方，上述债权标的转让价款划转完成即视为本协议生效且债权标的转让成功.</p>
