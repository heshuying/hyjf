...
var productForm = $("#productForm").validate({
    ...
    "messages": {
        "money": {
            ...
            "maxlength":"您输入的金额太大了",
            "min" : "出借金额应" + (isLast() ? "等于" : "大于 ") + getMinMoney() + "元",
            "max" : "出借金额不能大于项目剩余！"
        },
...
function projectInvestPageSuccessCallback(data) {
    ...
    if (projectInvestList.length == 0) {
        projectInvestListStr = projectInvestListStr + '<tr>' + '<td colspan="4"><div class="data-empty"><div class="empty-icon"></div><p class="align-center">暂无出借记录</p></div></td>' + '</tr>'
    } else {
        for (var i = 0; i < projectInvestList.length; i++) {
            ...
            if(projectInvest.redbag==1){
            	projectInvestListStr = projectInvestListStr 
                + '<tr>'
                + '   <td style="position:relative"><i class="i-get-gift"></i><span>' + projectInvest.userName + '</span>'+vipflg+'</td>' 
...
function getEndTime(now,date){
	var endTime  = "";
	var serverNow = now*1000 || coutdownEle.data("now")*1000;
	var end = date*1000 || coutdownEle.data("date")*1000;
...
var getopsition =function (time){
	 var options = {
        ...
        onFinish: function() {
        	 $.ajax({
    	        type : "GET",
    	        url :webPath + "/bank/web/borrow/"+borrowNid+"/ontimeBorrow.do",
    	        success : function(data) {
    		        if (data.status == 0) {
    		        	$("div.product-form").hide();
    		        	$("form.product-form").show();
    		        }
    		        else{
    		        	 if(data.status == -2 ){
    		        		 $('.product-form').hide();
    		        	 ...
    		        	 setTimeout(function(){
    		        		 var time = getEndTime(data.nowtime,data.ontime);
        		        	 coutdownEle.children(".countdown").jCountdown(getopsition(time));
    		        	 },500)
