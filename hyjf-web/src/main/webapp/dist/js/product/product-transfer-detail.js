...
var productForm = $("#productForm").validate({
    ...
    "messages": {
        "assignCapital": {
            "required": "亲，您还没有填写承接金额",
            "number": "亲，只能填写数字",
            "maxlength":"您输入的金额太大了",
            "min" : "承接金额应" + (isLast() ? "等于" : "大于 ") + getMinMoney() + "元",
            "max" : "承接金额超过项目剩余！"
        },
        "termcheck": "请先阅读并同意相关协议",
        "increase" : "承接递增金额须为" + $("#increase").val() + "元的整数倍"
    },
    errorElement: "div",
    "ignore": ".ignore",
    ...
    $.ajax({
        ...
        success : function(data) {
            if (data.resultFlag==0) {
				...
				// 实际支付
				$("#act_pay_num_confirm").html(data.creditAssign.assignPay);
...
function getAssignListPage() {
    ...
    doRequest(assignPageSuccessCallback, assignPageErrorCallback, webPath + "/bank/user/credit/webCreditTenderList.do?creditNid=" + $("#creditNid").val(), $("#listForm").serialize(), true, "assignClass", "assignListPage");
}
/**
 * 获取投资记录成功回调
 */
function assignPageSuccessCallback(data) {
    ...
    var assignTimes = "承接总人次 : ";
    var assignTotal = "承接金额 : ";
    // 挂载数据
    var assignListStr = "";
    if (assignList.length == 0) {
        ...
        for (var i = 0; i < assignList.length; i++) {
            ...
            var vipflg = "";
            if (assign.isVip != "0") {
                vipflg = '<span class="tag">VIP</span>';
            }
            assignListStr = assignListStr 
                ...
                + '   <td>' + assign.client + '</td>'
                + '   <td><span class="dark">' + assign.assignTime + '</span></td>' 
                + ' </tr>'
        }
        assignTimes += data.assignTimes;
        assignTotal += data.assignTotal+"元";
    }
