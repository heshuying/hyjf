...
$(document).ready(function() {
    ...
    if(roleId == 2){
		 ...
		 * 待授权标的列表
		 */
	    getBorrowAuthListPage();
	    
		/**
	     * 待授权标的分页点击
	     */
		$(document).on("click", ".authClass", function() {
			getBorrowAuthListPage($(this).data("page"));
		});
		
		/**
		 * 待授权标的列表
		 */
		getBorrowAuthedListPage();
	    
		/**
	     * 待授权标的分页点击
	     */
		$(document).on("click", ".authedClass", function() {
			getBorrowAuthedListPage($(this).data("page"));
		});
		...
		if (repayList != null) {
			for (var i = 0; i < repayList.length; i++) {
                ...
                if (repayList[i].projectType != '13' && repayList[i].fddStatus=='1') {
					agreementStr += '<a data-href="' + webPath + '/bank/web/user/repay/downloadBorrowerPdf.do?borrowNid='+ repayList[i].borrowNid + '" class="font-bg downloadargrement">下载协议</a>&nbsp;&nbsp;';
				}
				if (roleId == "3") {
					strRepayList +=
						'<tr class="loan-div-a">'
						...
						+		'<span  class="font-bg font-nocolor">'+agreementStr+'</span>'
						+ '	</td>'
                    ...
                    if (repayList[i].listCount > 0){
                        agreementStr += '<a href="'+ webPath + '/bank/web/user/repay/userRepayDetail.do?borrowNid='+ repayList[i].borrowNid + '" class="font-bg ">详情</a>';
                    }
					strRepayList +=
						'<tr class="loan-div-a">'
						/*应还时间*/
						...
						// +'	<td class="item5 loan-div-row">第' + repayList[i].repayPeriod + '期</td>'/*当前还款期数*/
