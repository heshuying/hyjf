...
public class AnRongCallController extends BaseController {
	Logger _log = LoggerFactory.getLogger(AnRongCallController.class);
	@Autowired
	AnRongCallService anRongCallService;
	...
	public String callApiBg(HttpServletRequest request, @ModelAttribute AnRongBean bean) throws Exception {
		...
		LogUtil.startLog(AnRongCallDefine.CONTROLLOR_CLASS_NAME, methodName, "[调用接口开始, 消息类型:" + (bean == null ? "" : bean.getTxCode()) + "]");
		String ret = "";
		...
		try {
			...
			LogUtil.errorLog(AnRongCallDefine.CONTROLLOR_CLASS_NAME, methodName, "订单号：" + logOrderId, e);
		} finally {
			LogUtil.endLog(AnRongCallDefine.CONTROLLOR_CLASS_NAME, methodName, "[调用接口结束, 消息类型:" + (bean == null ? "" : bean.getTxCode()) + "]");
		}
		return ret;
	}
}