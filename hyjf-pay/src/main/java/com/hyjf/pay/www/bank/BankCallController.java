...
public class BankCallController extends BaseController {
    ...
    public ModelAndView callPageApi(@ModelAttribute BankCallBean bean) throws Exception {
        ...
        LogUtil.startLog(BankCallDefine.CONTROLLOR_CLASS_NAME, methodName, "[调用接口开始, 消息类型:" + (bean == null ? "" : bean.getTxCode()) + "]");
        ...
        try {
            ...
            LogUtil.errorLog(BankCallDefine.CONTROLLOR_CLASS_NAME, methodName, e);
            throw e;
        } finally {
            LogUtil.endLog(BankCallDefine.CONTROLLOR_CLASS_NAME, methodName, "[调用接口结束, 消息类型:" + (bean == null ? "" : bean.getTxCode()) + "]");
        }
        return modelAndView;
    }
    ...
    public ModelAndView callPageReturn(HttpServletRequest request, @ModelAttribute BankCallBean bean) {
        ...
        String methodName = "callPageReturn";
...
//			}
            ...
            if (Validator.isNull(result) || !result.isLogVerifyFlag()) {
                ...
                LogUtil.debugLog(BankCallDefine.CONTROLLOR_CLASS_NAME, methodName, "验证签名失败");
        ...
        LogUtil.endLog(BankCallDefine.CONTROLLOR_CLASS_NAME, methodName, "[交易完成后,回调结束]");
        return modelAndView;
    }
...
//			BankCallBean result = api.verifyChinaPnr(bean);
            ...
            if (Validator.isNotNull(logOrderId)) {
                ...
                if (record != null) {
                    ...
                    if (result.isLogVerifyFlag()) {
                        try {
                            if (Validator.isNotNull(notifyUrl)) {
                                ...
                                logger.info("调用notifyUrl:"+notifyUrl+"   返回结果为:"+content);
                    ...
                    LogUtil.endLog(BankCallDefine.CONTROLLOR_CLASS_NAME, methodName, "[接收异步返回的消息结束, 消息类型:" + (bean == null ? "" : bean.getTxCode()) + "]");
                }
            }
        }
        // 当业务请求认为本次回调成功
        ...
        try {
...

            {
                ...
                if(StringUtils.isEmpty(result)) {
                    _log.error("txtCode：" + bean.getTxCode()+",订单号："+logOrderId);
                    return ret;
                }
...
//				BankCallBean verifyResult = api.verifyChinaPnr(backBean);
            ...
            LogUtil.errorLog(BankCallDefine.CONTROLLOR_CLASS_NAME, methodName, "订单号：" + logOrderId, e);
        } finally {
            LogUtil.endLog(BankCallDefine.CONTROLLOR_CLASS_NAME, methodName, "[调用接口结束, 消息类型:" + (bean == null ? "" : bean.getTxCode()) + "]");
        }
        return ret;
    ...
    public String callApiAjax(@RequestBody BankCallBean bean) throws Exception {
        ...
        try {
            ...
            LogUtil.debugLog(BankCallDefine.CONTROLLOR_CLASS_NAME, methodName, "转换成JSON时失败");
        } catch (Exception e) {
            LogUtil.errorLog(BankCallController.class.getName(), methodName, e);
        } finally {
            LogUtil.endLog(BankCallDefine.CONTROLLOR_CLASS_NAME, methodName, "[调用接口结束, 消息类型:" + (bean == null ? "" : bean.getTxCode()) + "]");
        }
        return ret;
    ...
    public void callBack(HttpServletRequest request, HttpServletResponse response, @ModelAttribute BankCallBean bean) {
        ...
        if (StringUtils.isNotBlank(bgData)) {
            bean = JSONObject.parseObject(bgData, BankCallBean.class);
            LogUtil.startLog(BankCallDefine.CONTROLLOR_CLASS_NAME, methodName, "[接收异步返回的消息开始, 消息类型:" + (bean == null ? "" : bean.getTxCode()) + "]");
            ...
            bean.convert();
            // 输出debug日志
            LogUtil.debugLog(BankCallDefine.CONTROLLOR_CLASS_NAME, methodName, "参数: " + bean == null ? "无" : bean.getAllParams() + "]");
            bean.setLogOrderId(logOrderId);
            bean.setLogUserId(logUserId);
            // 写入银行接口接收记录
