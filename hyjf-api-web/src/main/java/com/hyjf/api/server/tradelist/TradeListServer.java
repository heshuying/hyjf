 ...
 */
@RestController
@RequestMapping(value = TradeListDefine.REQUEST_MAPPING)
public class TradeListServer extends BaseController {
	 ...
	 * @return
	 */
    @RequestMapping(value = TradeListDefine.TRADELIST_ACTION)
    public ResultApiBean<List<AccountDetailCustomize>> srchTradeList(@RequestBody TradeListBean bean){
    	/**必传为空校验 start*/
    	// 常规参数验证
		...
		//CheckUtil.check(Validator.isNotNull(bean.getLimitStart()), MsgEnum.STATUS_CE000001);
		//CheckUtil.check(Validator.isNotNull(bean.getLimitEnd()), MsgEnum.STATUS_CE000001);
		/**必传为空校验 end*/
		...
		CheckUtil.check(this.verifyRequestSign(bean, TradeListDefine.TRADELIST_ACTION),MsgEnum.ERR_SIGN);
    	// 返回查询结果
