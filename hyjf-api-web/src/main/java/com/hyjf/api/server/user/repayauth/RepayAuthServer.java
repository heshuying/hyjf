package com.hyjf.api.server.user.repayauth;
...
@Controller(RepayAuthDefine.CONTROLLER_NAME)
@RequestMapping(value = RepayAuthDefine.REQUEST_MAPPING)
public class RepayAuthServer extends BaseController {
     ...
     * @return
     */
    @RequestMapping(value = RepayAuthDefine.USER_REPAY_AUTH_ACTION)
    @ResponseBody
    public ModelAndView repayAuth(HttpServletRequest request, HttpServletResponse response, @RequestBody RepayAuthRequestBean payRequestBean){
        ModelAndView modelAndView = new ModelAndView(RepayAuthDefine.PATH_TRUSTEE_PAY_ERROR);
        ...
        // 验签  暂时去掉验签
       if (!this.verifyRequestSign(payRequestBean, BaseDefine.METHOD_REPAY_AUTH)) {
        ...
        // 根据电子账户号查询用户ID
        BankOpenAccount bankOpenAccount = this.repayAuthService.selectBankOpenAccountByAccountId(payRequestBean.getAccountId());
        ...
        // 检查用户是否存在
        Users user = repayAuthService.getUsers(bankOpenAccount.getUserId());//用户ID
        if (user == null) {
        ...
        UsersInfo usersInfo=repayAuthService.getUsersInfoByUserId(userId);
    ...
    // 还款授权同步回调
    @RequestMapping(RepayAuthDefine.RETURL_SYN_ACTION)
    public ModelAndView repayAuthReturn(HttpServletRequest request, HttpServletResponse response,
        ...
        if (retBean!=null&& BankCallStatusConstant.RESPCODE_SUCCESS.equals(retBean.getRetCode()) && "1".equals(retBean.getRepayAuth())) {
            ...
            // 失败
            modelAndView.addObject("statusDesc", "还款授权申请失败,失败原因：" + repayAuthService.getBankRetMsg(bean.getRetCode()));
    ...
    // 异步回调
    @ResponseBody
    @RequestMapping(RepayAuthDefine.RETURL_ASY_ACTION)
    public BankCallResult repayAuthBgreturn(HttpServletRequest request, HttpServletResponse response,
        ...
        if (user != null && BankCallConstant.RESPCODE_SUCCESS.equals(bean.get(BankCallConstant.PARAM_RETCODE))) {
         ...
         // 失败
            message = "还款授权失败,失败原因：" + repayAuthService.getBankRetMsg(bean.getRetCode());
            status = ErrorCodeConstant.STATUS_CE999999;
        }
        // 返回值
