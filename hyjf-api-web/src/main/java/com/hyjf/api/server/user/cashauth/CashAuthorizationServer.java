...
@Controller(CashAuthDefine.CONTROLLER_NAME)
@RequestMapping(value = CashAuthDefine.REQUEST_MAPPING)
public class CashAuthorizationServer extends BaseController {
    
    Logger _log = LoggerFactory.getLogger("代提现授权");
    ...
    public ModelAndView cashWithdrawalByFrom(HttpServletRequest request, HttpServletResponse response){
        ...
        CashAuthRequestBean cashAuthRequestBean = new CashAuthRequestBean();
            ...
            CashAuthRequestBean bean = cashAuthRequestBean;
     ...
     * 代提现授权接口
     */
    @RequestMapping(value = CashAuthDefine.CASH_WITHDRAWAL)
    public ModelAndView cashWithdrawal(HttpServletRequest request, HttpServletResponse response,@RequestBody CashAuthRequestBean cashAuthRequestBean){
       ...
       //参数校验
        ...
        _log.info("授权接口返回CE999999  请求参数[" + JSONObject.toJSONString(cashAuthRequestBean, true) + "]");
        // 此接口暂时停用 返回CE999999
        if(true){
            return sendErrorMsg("CE999999","同步通知地址不能为空",retnotifyUrl,returnUrl,baseBean,modelAndView,cashAuthRequestBean);
        }
        ...
        _log.info("验证成功"+cashAuthRequestBean.getAccountId());
        
        // 暂时去掉验签功能
       if(StringUtils.isEmpty(cashAuthRequestBean.getChkValue())||!this.verifyRequestSign(cashAuthRequestBean, BaseDefine.METHOD_SERVER_CASHWITHDRAWAL)){
            
            return sendErrorMsg("CE000002","验签失败",retnotifyUrl,returnUrl,baseBean,modelAndView,cashAuthRequestBean);
        }
        _log.info("验签成功"+cashAuthRequestBean.getAccountId());
       ...
       // 根据电子账户号查询用户ID
        ...
        String retUrl = PropUtils.getSystem(CustomConstants.HYJF_WEB_URL)+CashAuthDefine.PROJECT_NAME+CashAuthDefine.REQUEST_MAPPING+CashAuthDefine.USER_AUTH_INVES_RETURN_ACTION+"?acqRes="+acqRes+"&callback="+returnUrl.replace("#", "*-*-*");
        String notifyUrl = PropUtils.getSystem(CustomConstants.HYJF_WEB_URL)+CashAuthDefine.PROJECT_NAME+CashAuthDefine.REQUEST_MAPPING+CashAuthDefine.USER_AUTH_INVES_BGRETURN_ACTION+"?acqRes="+acqRes+"&callback="+retnotifyUrl.replace("#", "*-*-*");
        ...
        if("0".equals(bitMap)){
            modelAndView = new ModelAndView(CashAuthDefine.AUTH_ERROR_PATH);
            ...
            cashBean.setTxCode(CashAuthDefine.TXCODE_AUTH);
            ...
            boolean insAuthLog = this.suuserCashAuthService.insertUserAuthLog(userId, cashBean,"2");
            if(!insAuthLog){
    ...
    public static ModelAndView sendErrorMsg(String status,String statusDesc , String retnotifyUrl,String returnUrl,BaseResultBean baseBean,ModelAndView modelAndView, CashAuthRequestBean cashAuthRequestBean){
        Map<String, String> params = new HashMap<String, String>();
        CashAuthResultBean resultBean = new CashAuthResultBean();
    ...
    public static ModelAndView checkParams (CashAuthRequestBean cashAuthRequestBean,ModelAndView modelAndView,BaseResultBean baseBean){
        String channel = cashAuthRequestBean.getChannel();//渠道
        String accountId = cashAuthRequestBean.getAccountId();//电子账户accountId
        String bitMap = cashAuthRequestBean.getBitMap();//维护标识
        String autoBid = cashAuthRequestBean.getAutoBid();//维护标识
     ...
     * @return
     */
    @RequestMapping(CashAuthDefine.USER_AUTH_INVES_RETURN_ACTION)
    public ModelAndView userAuthInvesReturn(HttpServletRequest request, HttpServletResponse response,
        ...
        System.out.println("同步回调--------------------"+request.getParameter("callback"));
        System.out.println("同步参数"+bean.getLogUserId());
     ...
     * @return
     */
    @RequestMapping(CashAuthDefine.USER_AUTH_INVES_BGRETURN_ACTION)
    @ResponseBody
