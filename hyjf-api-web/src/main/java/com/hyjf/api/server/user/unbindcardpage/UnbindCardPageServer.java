...
@Controller
@RequestMapping(UnbindCardPageDefine.REQUEST_MAPPING)
public class UnbindCardPageServer extends BaseController {
     ...
     */
    @ResponseBody
    @RequestMapping(UnbindCardPageDefine.DELETE_CARD)
    public ModelAndView deleteCard(@RequestBody UnbindCardPageRequestBean bankCardRequestBean, HttpServletRequest request, HttpServletResponse response) {
        _log.info("-----------------------------"+bankCardRequestBean.getAccountId() + "用户解除绑定银行卡开始-----------------------------");
        _log.info("第三方请求参数：" + JSONObject.toJSONString(bankCardRequestBean));
        LogUtil.startLog(UnbindCardPageDefine.THIS_CLASS, UnbindCardPageDefine.DELETE_CARD);
        ModelAndView modelAndView = new ModelAndView(UnbindCardPageDefine.PATH_OPEN_ACCOUNT_PAGE_ERROR);
        try {
            ...
            //验签
            if(!this.verifyRequestSign(bankCardRequestBean, UnbindCardPageDefine.REQUEST_MAPPING+UnbindCardPageDefine.DELETE_CARD)){
            ...
            String retUrl = PropUtils.getSystem(CustomConstants.HYJF_WEB_URL) + request.getContextPath()
                    + UnbindCardPageDefine.REQUEST_MAPPING + UnbindCardPageDefine.RETURL_SYN_ACTION+".do?acqRes="
            ...
            String bgRetUrl = PropUtils.getSystem(CustomConstants.HYJF_WEB_URL) + request.getContextPath()
                    + UnbindCardPageDefine.REQUEST_MAPPING + UnbindCardPageDefine.RETURL_ASY_ACTION + ".do?acqRes="
            ...
            bean.setForgotPwdUrl(bankCardRequestBean.getForgotPwdUrl());
            ...
            LogUtil.endLog(UnbindCardPageDefine.THIS_CLASS, UnbindCardPageDefine.DELETE_CARD);
            return modelAndView;
        } catch(Exception e){
            e.printStackTrace();
     ...
     * @return
     */
    @RequestMapping(UnbindCardPageDefine.RETURL_SYN_ACTION)
    public ModelAndView unbindCardReturn(HttpServletRequest request, HttpServletResponse response,@ModelAttribute BankCallBean bean) {
        ...
        LogUtil.startLog(UnbindCardPageDefine.THIS_CLASS, UnbindCardPageDefine.RETURL_SYN_ACTION, "[解卡同步回调开始]");
        _log.info("==========解卡同步回调开始==============");
        ...
        try {
            ...
            if (BankCallConstant.RESPCODE_SUCCESS.equals(retCode)||"1".equals(isSuccess)) {
                ...
                LogUtil.endLog(UnbindCardPageDefine.THIS_CLASS, UnbindCardPageDefine.RETURL_SYN_ACTION, "[交易完成后,回调结束]");
     ...
     */
    @ResponseBody
    @RequestMapping(UnbindCardPageDefine.RETURL_ASY_ACTION)
