...
@Controller
@RequestMapping(BankOpenEncryptPageDefine.REQUEST_MAPPING)
public class BankOpenEncryptPageServer extends BaseController {

    Logger _log = LoggerFactory.getLogger(BankOpenEncryptPageServer.class);
     ...
     */
    @ResponseBody
    @RequestMapping(BankOpenEncryptPageDefine.OPEN_ACCOUNT_ACTION)
    public ModelAndView openAccont(@RequestBody BankOpenEncryptPageRequestBean requestBean, HttpServletRequest request,
                                   HttpServletResponse response) {
        LogUtil.startLog(BankOpenEncryptPageDefine.THIS_CLASS, BankOpenEncryptPageDefine.OPEN_ACCOUNT_ACTION);
        ModelAndView modelAndView = new ModelAndView(BankOpenEncryptPageDefine.PATH_OPEN_ACCOUNT_PAGE_ERROR);
        try {
            // 验证请求参数
            // 机构编号
            ...
            if (!this.verifyRequestSign(requestBean, BankOpenEncryptPageDefine.REQUEST_MAPPING + BankOpenEncryptPageDefine.OPEN_ACCOUNT_ACTION)) {
                _log.info("----验签失败----");
            ...
            String retUrl = PropUtils.getSystem(CustomConstants.HYJF_WEB_URL) + request.getContextPath()
                    + BankOpenEncryptPageDefine.REQUEST_MAPPING + BankOpenEncryptPageDefine.RETURL_SYN_ACTION + ".do?acqRes="
            ...
            String bgRetUrl = PropUtils.getSystem(CustomConstants.HYJF_WEB_URL) + request.getContextPath()
                    + BankOpenEncryptPageDefine.REQUEST_MAPPING + BankOpenEncryptPageDefine.RETURL_ASY_ACTION + ".do?acqRes="
    ...
    private ModelAndView getErrorMV(BankOpenEncryptPageRequestBean userOpenAccountRequestBean, ModelAndView modelAndView,
                                    String status, String des) {
        BankOpenEncryptPageResultBean repwdResult = new BankOpenEncryptPageResultBean();
