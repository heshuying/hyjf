 ...
 *
 */
@Controller(MergeAuthPagePlusDefine.CONTROLLER_NAME)
@RequestMapping(value = MergeAuthPagePlusDefine.REQUEST_MAPPING)
public class MergeAuthPagePlusServer extends BaseController {

	Logger _log = LoggerFactory.getLogger(MergeAuthPagePlusServer.class);
	...
	public ModelAndView page(HttpServletRequest request,
			HttpServletResponse response,@RequestBody MergeAuthPagePlusRequestBean requestBean) {

		LogUtil.startLog(MergeAuthPagePlusDefine.THIS_CLASS, MergeAuthPagePlusDefine.MERGE_AUTH_ACTION);
        ModelAndView modelAndView = new ModelAndView(MergeAuthPagePlusDefine.PATH_AUTH_PAGE_ERROR);
        ...
        try {
	        ...
	        if (Validator.isNull(requestBean.getAuthType())) {
	            getErrorMV(requestBean, modelAndView, ErrorCodeConstant.STATUS_SQ000001, "授权类型不能为空");
	        ...
	        if (!Arrays.asList(AuthBean.AUTH_TYPE_AUTO_CREDIT,
	            ...
	            getErrorMV(requestBean, modelAndView, ErrorCodeConstant.STATUS_SQ000002, "授权类型不是指定类型");
	        ...
	        String retUrl = PropUtils.getSystem(CustomConstants.HYJF_WEB_URL) + request.getContextPath()
	                + MergeAuthPagePlusDefine.REQUEST_MAPPING + MergeAuthPagePlusDefine.RETURL_SYN_ACTION + ".do?acqRes="
	                + requestBean.getAcqRes()  + "&isSuccess=&authType="
	        ...
	        String bgRetUrl = PropUtils.getSystem(CustomConstants.HYJF_WEB_URL) + request.getContextPath()
	                + MergeAuthPagePlusDefine.REQUEST_MAPPING + MergeAuthPagePlusDefine.RETURL_ASY_ACTION + ".do?acqRes="
			...
			LogUtil.endLog(MergeAuthPagePlusServer.class.toString(),
	 ...
	 private ModelAndView getErrorMV(MergeAuthPagePlusRequestBean userOpenAccountRequestBean, ModelAndView modelAndView,
		        String status, String des) {
		MergeAuthPagePlusResultBean repwdResult = new MergeAuthPagePlusResultBean();
    ...
    public ModelAndView mergeauthReturn(HttpServletRequest request, HttpServletResponse response,
        ...
        MergeAuthPagePlusRetBean repwdResult = new MergeAuthPagePlusRetBean();
        ...
        if (retBean != null
        	...
        	try {
				...
				this.authService.updateUserAuth(
						Integer.parseInt(retBean.getLogUserId()), retBean,authType);
			} catch (Exception e) {
				e.printStackTrace();
	...
	public BankCallResult mergeauthBgreturn(HttpServletRequest request, HttpServletResponse response,
        ...
        try {
            ...
            e.printStackTrace();
            _log.info("多合一授权出错,userId:【"+userId+"】错误原因："+e.getMessage());
            message = "多合一授权失败";
            status = ErrorCodeConstant.STATUS_CE999999;
        }
        // 返回值
