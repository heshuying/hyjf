package com.hyjf.api.web.activity.actdec2018.listact;

import com.alibaba.fastjson.JSONObject;
import com.hyjf.activity.actdec2018.listact.ListActDefine;
import com.hyjf.activity.actdec2018.listact.ListActResultBean;
import com.hyjf.activity.actdec2018.listact.ListActService;
...
public class ListActServer extends BaseController {
	...
	public ListActResultBean select(@ModelAttribute BaseBean requestBean, HttpServletRequest request,
		...
		// 验签
		if (!this.checkSign(requestBean, ListActDefine.REQUEST_MAPPING)) {
...
//		}
	...
	public ListActResultBean select1(@ModelAttribute BaseBean requestBean, HttpServletRequest request,
		...
		resultBean.setAlo(listActService.getAct1List(Integer.valueOf(userId)));
	 ...
	 * @return
	 */
	private void giveMoney(String userId) {
		ActivityList ayl = listActService.getActivityDate(48);
		// 检测活动id
		if (ayl==null) {
			return ;
		}
		...
		_log.info("送钱活动活动开始中,正在开户userId:" + userId);
		List<UsersInfo> uio = listActService.getUserInfo(Integer.valueOf(userId));
		if (uio.isEmpty()) {
			return ;
		}
		...
		listActService.insertMoney(Integer.valueOf(userId),uio.get(0).getTruename());
