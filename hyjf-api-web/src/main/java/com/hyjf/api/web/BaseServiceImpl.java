...
public class BaseServiceImpl extends CustomizeMapper implements BaseService {
    ...
    public int updateChinapnrExclusiveLog(ChinapnrExclusiveLogWithBLOBs record) {
        ...
        example.createCriteria().andIdEqualTo(record.getId()).andUpdatetimeEqualTo(record.getUpdatetime())
                .andUpdateuserNotEqualTo("callback2");
	...
	public int countUserPlat(String fromCode) throws Exception {
		...
		example.createCriteria().andSourceIdEqualTo(Integer.valueOf(fromCode));
		int cnt = utmPlatMapper.countByExample(example);
		return cnt;
	}
