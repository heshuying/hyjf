...
<html>
	...
	<body class="bg_grey couponDetailBody">
		...
		<div class="specialFont response">
			...
			<c:if test="${detail.usedFlag == '已使用'}">
			<c:choose>
			    <c:when test="${detail.borrowTenderStatus == '0'}">
			      <div class="detailContent bg_white">
					<ul>
						<li>
							<span class="left">预期收益：</span>
							<span class="right"><c:out value="${earnings }"></c:out></span>
						</li>
						<li>
			    ...
			    <c:otherwise>
					<c:if test="${detail.borrowStyleName == '按月计息，到期还本还息' or detail.borrowStyleName == '按天计息，到期还本息'}">
					...
					<div class="detailContent bg_white">
						<ul>
						   ...
						   <c:if test="${item.recoverStatus == '已还款' and item.receivedFlg == '未领取'}">
								<li>
									<span class="left">待收收益：</span>
									<span class="right"><c:out value="${item.recoverInterest}"></c:out></span>
								</li>
						   ...
						   <c:if test="${item.recoverStatus == '已还款' and item.receivedFlg == '已领取'}">
								<li>
									<span class="left">已得收益：</span>
									<span class="right"><c:out value="${item.recoverInterest}"></c:out></span>
								</li>
								<li>
									<span class="left">领取时间：</span>
					...
					<!--item end-->
					<c:if test="${detail.borrowStyleName == '等额本息' or detail.borrowStyleName == '等额本金' or detail.borrowStyleName == '先息后本'}">
