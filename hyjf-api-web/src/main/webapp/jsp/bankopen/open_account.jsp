...
<head>
	...
	<form id="subForm" method="POST" onsubmit="return false;">
		...
		<div class="wap-reg-div">
				...
				<div class="wap-reg-item">	
					...
					<input class="wap-reg-input wap-reg-input-code" placeholder="短信验证码" name="verificationCode" id="msg_code" type="tel" oninput="if(value.length>6)value=value.slice(0,6)"  data-describedby="vercode-description" data-validate="vercode">
					<input class="wap-reg-input wap-reg-input-getcode wap-reg-getcode" value="获取验证码" type="button" >
				</div>
		</div>
		<input class="wap-reg-btn " id="subutton" value="开通江西银行账号" type="submit">
	</form>
	<div class="wap-reg-bottom">
		<p class="tac">点击“开通江西银行账号”按钮，即表示您同意</p>
		<p class="tac"><a href="" class="color-blue hy-jumpH5">《江西银行网络交易资金账户服务三方协议》</a></p>
	</div>
	...
	$(function(){
	jumpH5("/hyjf-api-web/jsp/bankopen/open_contract.jsp")
  ...
  //   	var orderId = $("#orderid").val();
            	...
            	$.ajax({
					...
					success : function(result) {
						...
						if(result.status == 1){//失败
							...
							window.location.href = "${ctx}/bank/user/bankopen/openSuccess?userId="+$("#userid").val();
						}
					},
				});
	...
	$(function(){
	    ...
	    var getver = function(){
      		var phone = $("#phone").val();
      		//var checkVer = /^(((13[0-9]{1})|(14[0-9]{1})|(15[0-3,5-9]{1})|(17[0,1,3,5-8]{1})|(18[0-9]{1}))+\d{8})$/.test(phone);
      		if(verIsMobile(phone)){
      			if(check){
      				$.ajax({
